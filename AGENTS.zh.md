...
| 决策                                                | 行动                                                                                                                                 |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 架构 / spec 内部范围 / 技术选型                     | **自决**。记录到 spec 的 `plan.md` 或 `docs/decisions/<short-kebab-case-title>.md`。                                                 |
...
否则保持扁平。举例：只有当 `stack.md` 同时变得超过 ~300 行**且**其各节都各自写成长文时，才拆成子目录；同时更新 §2 的链接。
