...
**定位校准**：Coffer 是**个人工具，非企业工具**——不做企业级安全/治理。"safe" = 凭据隔离 + 审计（per-agent server 策展已于 2026-06-20 删除，见 1.6）。RAG 是**战略投资**方向，不砍检索栈。

**实删（10）**：`/cwd`（+工作区白名单）· commands 目录管理 · 4.8 skill catalog（回退 #116）· 3.9 instructions hub（回退 #112）· 1.7 MCP Registry 浏览（回退 #114）· 5.7 KB 逐文档锁 · 2.9 开机自启 · 10.5 死 ADR · **4.3 从 Git URL 拉 skill**（用户不用,2026-06-20 加删）· **1.6 per-agent MCP 作用域**（用户 2026-06-20 复核裁定删,回退 #108/ADR-026-per-agent）

**简化/部分（3）**：3.8→删 commands 留 subagents/cron · 1.11 backup 简化 · 8.4→删 /cwd 留其余（3.5 撤回——读写保留,不简化）

**留**：其余全部（检索栈 / agent 框架 / 插件 / Sync / Channels 含 SeaTalk / Chat / skills 4.4–4.7,4.9 / 审计 / 2.10 / 批量 8 条）
...
- [x] 3.9 — 删 Master-instructions hub（回退 PR #112：instructions_service / instructions.py / routes / CLI / 前端 AgentConfigFilesEditor）
  - 执行说明:删后端 hub（instructions_service / domain instructions.py / managed_block.py / routes / CLI + 4 audit enum + 2 error）+ spec 004（FR-041–046 / US13 / SC-011 / 8 验收场景 / data-model / OpenAPI / quickstart）。前端 tab UI 早被 PR #119 删（只剩孤儿 `agentInstructions.ts` + 2 处 i18n)。**保留**:`descriptor.py` 的 agent-enabled gating（#112 顺带做的,非 hub)、config-file `instructions` 键（FR-013 改名,非 hub）、`managed_block` 无其它消费者（memory projection 已由 ADR-026 删）。无 migration。
...
- [x] 4.5 — 删 Skill 内容信任扫描（风险分级 + acknowledge 闸）
  - 执行说明（决策表 4.5「重审=✅保留」的理由是「4.3 Git skill 仍是远程源，扫描有据」；但 4.3 已删 → 该理由作废，第二轮 line 37「❌ 删」+ backlog 一致，用户「continue 4.5」授权）。删启发式扫描器 `content_scan.py`（Severity 枚举 + 8 条规则 + scan_skill_folder）+ `scan_ops.py`；剥 `SkillConfig` 5 个 trust-layer 字段（scan_verdict/scan_findings_count/scan_ruleset_version/last_scanned_at/risk_acknowledged）；删 enable 的 risk-gate（`skip_risk_gate` + `SkillRiskNotAcknowledged`）→ 启用无条件；删 `POST /skills/{name}/scan` + `/acknowledge-risk` 路由 + CLI `scan`/`acknowledge-risk` 命令 + 2 审计事件（SKILL_SCANNED/SKILL_RISK_ACKNOWLEDGED）；前端去 Content-scan 块 + i18n；spec 005 删 FR-028(scan)/FR-029(ack) + 3 验收场景（spec↔marker 锁步）；ADR-027 立碑（Reverted）；migration 0032 剥存量 skill config 的 5 字段。**保留**:FR-022 非托管 skill 扫描（无关功能）、SKILL_AUTOBIND_SKIPPED（通用）。

### 阶段四 · 简化

- [x] 4.4 — 简化 skill 包管理：留 import（本地文件夹/zip）+ 重新导入覆盖；删 rename 检测 / pin / check-update / Git re-fetch（Git 导入已删）
  - 执行说明（Explore 复核:rename 检测**从未实现**、pin/check-update/Git-refetch **已随 4.3 删净** → "删"项已全部完成,无可再删;唯一未做的是「**重新导入覆盖**」—— 原 import 对重名以 409 拒绝。用户裁定:**加 flag-gated overwrite**）。`register_from_validated(overwrite=False)`:重名 + overwrite → `atomic_replace` 主库 + `update_config` 既有 Resource + `SKILL_UPDATED` 审计,**保留 binding/symlink**(主库名不变),overwrite 路径跳过 auto_bind(已绑);无 overwrite 仍 409。HTTP `SkillImportRequest.overwrite` + CLI `import --force` + 前端 409→「替换?」确认卡片→overwrite 重试。spec 005 FR-005 + 新验收场景「re-import a skill with overwrite replaces it」+ edge case;**无 migration**(version_hash/last_synced 保留作内容指纹)。
...
| 3.9  | Master-instructions hub                              | 🔴   | 小型双向同步引擎（managed block + drift + adopt-back），场景小众。⚠️PR #112 刚做                                                                                                         | 简化/删                      | ❌ **删除**（确认 code 仍在：instructions_service/instructions.py/routes/CLI/前端 AgentConfigFilesEditor 全在 → 删除为**待执行任务**，回退 PR #112）                                                                            | 已执行 |
| 4.4  | Skill 包管理（更新/rename 检测/pin/check-update）    | 🔴   | 给小文本文件夹套 npm 级版本管理                                                                                                                                                          | 砍到"重 fetch 覆盖"          | ✂️ **已简化**（rename 检测从未实现 + pin/check-update/Git-refetch 已随 4.3 删净；本轮加 flag-gated「重新导入覆盖」补齐唯一更新机制，CLI `--force`）                                                                              | 已执行 |
| 4.5  | Skill 内容信任扫描（风险分级 + acknowledge 闸）      | 🔴   | 启发式扫描误报高；但 4.3 Git skill 仍是远程源，扫描有据                                                                                                                                  | 删                           | ❌ **删除**（保留理由系于 4.3 Git 远程源，4.3 已删 → 理由作废；回退 ADR-027，migration 0032 剥 5 字段）                                                                                                                          | 已执行 |
...
| 1.6  | 逐 agent MCP 作用域（auto/selected）                 | 🟡   | 重审曾拟保留作**策展**；2026-06-20 用户复核裁定删除（推翻"重审保留"，与 backlog/line 35 一致）                                                                                            | 删                           | ❌ **删除**（用户 2026-06-20 裁定；回退 PR #108 / ADR-026-per-agent；migration 0030 drop 两表）                                                                                                                                  | 已执行 |
...
| P11  | per-agent 逐工具启用（扩展 1.6）             | 配置每个 agent 开启哪些 MCP server + 哪些**工具**。⚠️定位=**策展/便利**（减噪聚焦），**非** G1 拒绝的安全强制闸。难点：UI 避免 toggle 爆炸（agent×server×tool）→ 需设计（默认全开 + 按需收窄 / 分组 / 搜索选择 / 复用 1.6 的 auto/selected 模式延伸到工具层）。**⚠️1.6 已于 2026-06-20 删除 → 本项前提不存在，取消**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 待做 |
