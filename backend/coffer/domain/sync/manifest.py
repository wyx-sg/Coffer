...
#: would install the literal token into configs). The too-new gate is enforced
#: from version 2 onward — a v1 build predates the gate entirely, so a mixed
#: v1/v2+ fleet is unprotected and must upgrade together.
SCHEMA_VERSION = 3
