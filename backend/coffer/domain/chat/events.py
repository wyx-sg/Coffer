...
``QueueChanged`` is a conversation-level event (the pending-message queue, spec channels FR-050)
...
@dataclass(frozen=True)
class QueueChanged:
    """The conversation's pending-message queue changed (spec channels FR-052).
