...
directories declared in their own config file. For those agents Coffer
folder-delivers the enabled skills into a Coffer-owned directory and registers
that directory in the agent's config.
...
class ExternalDirRegistration:
    ...
    - ``config_path`` — the agent's own config file to edit.
