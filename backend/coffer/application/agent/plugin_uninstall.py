...
def cache_dir_for(cfg_dir: pathlib.Path, plugin_id: str) -> pathlib.Path | None:
    """Resolve a Codex plugin's cache directory, or None when the id is unsafe.

    ``plugin_id`` arrives from the request path and its two halves become path
    segments of a directory this module deletes. A half carrying a separator or
    a ``..`` would point the delete outside the cache root, so the resolved path
    is checked to lie strictly inside it and anything else is refused — the
    caller treats that as "no such plugin" and removes nothing.
    """
    at = plugin_id.rfind("@")
    plugin_name, marketplace = (
        (plugin_id[:at], plugin_id[at + 1 :]) if at != -1 else (plugin_id, "")
    )

    root = cfg_dir / "plugins" / "cache"
    candidate = root / marketplace / plugin_name if marketplace else root / plugin_name
...
async def uninstall_codex(
    *,
    ...
    # plugin_id = "<name>@<marketplace>"; both halves become path segments, so
    # the cache dir is resolved through a guard that refuses anything landing
    # outside the cache root.
    cache_dir = cache_dir_for(cfg_dir, plugin_id)
    cache_removed = cache_dir is not None and dir_exists(cache_dir)
    ...
    if cache_removed and cache_dir is not None:
        rmtree(cache_dir)
