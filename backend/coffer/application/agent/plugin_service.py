...
- **Claude Code** writes only ``settings.json`` (``settings`` key); the two
  internal inventory files are read by path and never written, so uninstall is
  delegated to the agent's own ``claude plugin uninstall`` CLI.
"""
...
class AgentPluginService:
    ...
    async def set_enabled(
        ...
        if cap.model is PluginModel.CODEX:
            ...
            # Claude's enabledPlugins map accepts any key, so an id the agent
            # has never installed would be written in as a settings-only orphan
            # — a 204 that quietly edits the user's settings.json. Check the id
            # against the listing first, matching the Codex path's 404.
            if plugin_id not in {
                p.id for p in self._list_claude(cfg, cfg.resolved_config_dir()).items
            }:
                raise PluginNotFound(plugin_id)
    ...
    async def uninstall(self, uid: str, plugin_id: str, *, actor: str = "api") -> None:
        ...
        if cap.uninstall_strategy is UninstallStrategy.CLI:
            ...
            # and surface as a 422. A missing CLI still answers first.
            if self._cli_runner is not None and self._cli_runner.available():
                listed = self._list_claude(cfg, cfg.resolved_config_dir()).items
                if plugin_id not in {p.id for p in listed}:
                    raise PluginNotFound(plugin_id)
