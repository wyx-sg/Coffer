...
:class:`AgentConfig`, reads the descriptor's :class:`HookInjectionSpec`, and
...
agent's session-context / session-end endpoints.

An agent type that declares no ``hooks`` injection (descriptor ``hooks is None``)
raises :class:`HookInstallUnsupported` (→ 422) on install/uninstall; ``status``
reports ``installed=False`` for it (a not-installable agent is simply not
installed, never an error to inspect).
"""
...
from coffer.domain.agent.hook_injection import HookInjectionSpec
...
def _hook_command(hook_binary: str, agent_name: str) -> str:
    """The exact command string the agent execs: ``<binary> --agent <name>``.

    Both parts are shell-quoted so a binary path with spaces (e.g. a macOS app
...
class AgentHookService:
    ...
    async def _hook_spec(self, name: str) -> tuple[ConfigFileSpec, HookInjectionSpec, AgentConfig]:
        ...
        injection = descriptor_for(cfg.type).hooks
    ...
    async def status(self, name: str) -> HookInstallStatus:
        ...
        command = _hook_command(self._resolve_hook(), name) if installed else None
    ...
    async def install(self, name: str, *, actor: str = "api") -> HookInstallStatus:
        ...
        command = _hook_command(hook, name)
        text = self._store.read_text(spec.path) or ""
        new_text = apply_install(text, command=command, events=inj.events, fmt=spec.format)
