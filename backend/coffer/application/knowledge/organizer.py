...
class OrganizerService:
    ...
    async def organize(self, *, scope_name: str) -> OrganizeResult:
        """Drain ``scope_name``'s inbox into topic docs (one LLM call per item).
    ...
    async def _retrieve_candidates(
        ...
        for passage in result.passages:
            doc_row = await self._documents.get_document(
                KIND_KNOWLEDGE, ref.resource_name, passage.document_id
            )
            if doc_row is None:
                continue
