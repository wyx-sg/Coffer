"""Read-path helpers for ``KnowledgeService`` — fact scans + store metrics.
...
from coffer.application.knowledge.service_helpers import du_bytes
...
def find_entry_scope(
    resolved_scopes: Iterable[ResolvedScope],
    fact_id: str,
    scope_name_for: Callable[[ResolvedScope], str],
) -> str:
    ...
    for resolved in resolved_scopes:
        scan = scan_scope_dir(resolved.store_dir)
        if fact_id in scan.files:
            return scope_name_for(resolved)
    raise MemoryNotFound("memory", fact_id)


async def scope_metrics(store_dir: Path, config: KnowledgeConfig) -> dict[str, object]:
