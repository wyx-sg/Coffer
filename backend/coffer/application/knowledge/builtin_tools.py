"""Knowledge built-in MCP tools — retrieval + working-state handoff.

Registered under the reserved ``coffer__`` prefix (added by the gateway). This
module contributes ``search``, ``grep``, ``read``, ``set_handoff`` and
``resume``; the sibling ``document_tools`` contributes ``list``, ``write`` and
``delete``. Eight tools over one kind, where there used to be twelve over two:
an agent no longer has to decide whether what it is after is "memory" or
"knowledge" before it can ask for it.
...
def is_document(svc: KnowledgeService, scope_name: str, item_id: str) -> bool:
    """Whether an id names an ingested document rather than a written entry.
    ...
    a file under the scope's ``inbox/``; an entry lives in ``knowledge/``.
    Getting this wrong would drop a row while leaving its markdown behind, and
    the reconciler would simply put the row back."""
    try:
...
def register_knowledge_builtin_tools(
    ...
    *,
    ...
    """Wire ``search`` / ``grep`` / ``read`` + the two handoff tools."""
    ...
    registry.register(
        BuiltinTool(
            ...
            description=(
                ...
                "both the facts and rules agents have written down and the "
