"""Async batch re-embed for ingested documents.

The slow part of an ingest is convert+chunk+embed; this module moves the
...
OP_TYPE = "kb_embed"
_SEP = "\x00"  # scope_name / document_id separator in registry keys (neither contains NUL)
...
class KnowledgeBaseBatchService:
    ...
    @staticmethod
    def _key(scope_name: str, document_id: str) -> str:
        return f"{scope_name}{_SEP}{document_id}"

    def inflight(self, scope_name: str) -> dict[str, InflightEntry]:
        """In-flight entries for *scope_name*, keyed by document_id."""
        prefix = f"{scope_name}{_SEP}"
    ...
    async def enqueue_documents(self, scope_name: str, document_ids: list[str]) -> EnqueueResult:
        """Enqueue an explicit set of documents (skip those not pending an embed)."""
        docs = await self._list_documents(scope_name)
        wanted = set(document_ids)
        chosen = [d for d in docs if d.id in wanted]
        return self._enqueue(scope_name, chosen)

    async def enqueue_all(self, scope_name: str) -> EnqueueResult:
        """Enqueue every document still pending an embed (re-embed-all)."""
        docs = await self._list_documents(scope_name)
        return self._enqueue(scope_name, docs)

    def _enqueue(self, scope_name: str, docs: list[Document]) -> EnqueueResult:
        queued = 0
        skipped = 0
        for doc in docs:
            if not doc.embed_pending:
                ...
                continue
            key = self._key(scope_name, doc.id)
            ...
            self._runner.enqueue(OP_TYPE, key, self._factory(scope_name, doc.id))
            queued += 1
        return EnqueueResult(queued=queued, skipped=skipped, total=len(docs))

    def _factory(self, scope_name: str, document_id: str) -> Callable[[], Awaitable[None]]:
        async def _run() -> None:
            await self._reembed(scope_name, document_id)
