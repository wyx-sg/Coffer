"""External-source tracking for ingested documents (FR-021..024).
...
if TYPE_CHECKING:
    # Only the document operations are needed here, so depend on the mixin
    # rather than the concrete service: narrower, and it keeps the dependency
    # pointing one way (``documents`` -> ``source_tracking``, never back).
...
async def check_sources(service: DocumentOps, *, scope_name: str, actor: str) -> list[SourceStatus]:
            ...
            if doc.source_mode == "edited":
                ...
                try:
                    await update_from_source(
                        service, scope_name=scope_name, document_id=doc.id, actor=actor
                    )
                    status = "updated"
...
async def update_from_source(
    service: DocumentOps, *, scope_name: str, document_id: str, actor: str
) -> Document:
    ...
    await service.get_config(scope_name)
    doc = await service._require_document(scope_name, document_id)
    if doc.source_mode == "edited":
