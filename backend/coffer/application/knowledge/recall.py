"""Retrieval helpers for ``KnowledgeService`` — lazy reindex-on-read recall.
...
single resolved store; ``KnowledgeService.recall`` / ``recall_in_scope`` orchestrate
...
from coffer.application.knowledge.service_helpers import (
...
#: ``scope_name -> KnowledgeConfig`` (validates the store exists).
...
async def recall_across(
    ...
    *,
    ...
    for resolved in resolved_scopes:
        scope_name = deps.scope_name_for(resolved)
        config = await deps.get_config(scope_name)
...
ExtraStore = tuple[ResolvedScope, str, KnowledgeConfig]
...
#: ``scope_name -> ResolvedScope`` (recovers a store's scope + on-disk dir).
...
async def recall_in_store_scoped(
    ...
    *,
    ...
    if scope == "both" and scope_name != GLOBAL_SCOPE_NAME:
        g_resolved = await resolved_for(GLOBAL_SCOPE_NAME)
        extra = [(g_resolved, GLOBAL_SCOPE_NAME, await get_config(GLOBAL_SCOPE_NAME))]
...
async def recall_store_with_mode(
    ...
    *,
    ...
    async def _get_doc(fact_id: str):  # type: ignore[no-untyped-def]
        return await deps.documents.get_document(KIND_KNOWLEDGE, store_ref.resource_name, fact_id)
