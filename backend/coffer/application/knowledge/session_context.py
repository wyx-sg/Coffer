"""Rules / SessionStart-context helpers for ``KnowledgeService`` (spec 007 Slice 6).

Split out of :mod:`coffer.application.knowledge.service` for the file-size budget.
...
from coffer.domain.knowledge.scope import KnowledgeScope, ResolvedScope
...
async def assemble_session_context(
    *,
    ...
    """
    from coffer.application.knowledge.rules_bundle import RulesBundleAssembler

    async def _get_rules(scope_name: str) -> str | None:
        try:
            return await get_rules_for(scope_name)
        except Exception:
            return None
...
def render_memory_digest(facts: list[KnowledgeEntry], *, max_chars: int) -> str:
...
async def assemble_memory_digest(*, cwd: str | None, memory: Any, max_chars: int) -> str:
    ...
    ``memory`` is the KnowledgeService (duck-typed to avoid an import cycle through
    this module, which the service imports)."""
    if cwd is None or max_chars <= 0:
        return ""
    try:
        resolved = await memory.resolve_scope(scope=KnowledgeScope.PROJECT, cwd=cwd)
    ...
    try:
        facts, _total = await memory.list_facts(
            scope_name=scope_name_for(resolved), limit=_DIGEST_FACTS
        )
    except Exception:
