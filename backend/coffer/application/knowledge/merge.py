"""AI-assisted merge of same-project knowledge scopes (FR-056-059).
...
from coffer.application.knowledge.consolidate import MergeOutcome, StoreConsolidator
...
class StoreMergeService:
    ...
    async def merge(
        ...
        await self._audit.record(
            ...
            ref=ResourceRef(KIND_KNOWLEDGE, target),
            actor=actor,
            details={
                "source": source,
                "target": target,
    ...
    def _validate_pair(source: str, target: str) -> None:
        for name in (source, target):
            if name == GLOBAL_SCOPE_NAME:
                raise MemoryStoreMergeInvalid("the global scope is never mergeable")
            if not is_project_scope_name(name):
                raise MemoryStoreMergeInvalid(f"not a project scope name: {name!r}")
        if source == target:
            raise MemoryStoreMergeInvalid("source and target are the same scope")
