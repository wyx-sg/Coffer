...
hook for an agent the developer drives themselves (spec memory FR-054/FR-055).
...
#: FR-054 covers exactly the two agents Coffer already reads native memory
...
class DeliveryService:
    ...
    def _with_path(spec: ConfigFileSpec, fn: Callable[..., str | None], *args: str) -> str | None:
        ...
        (spec memory FR-054's "fails loudly" requirement)."""
        try:
            return fn(*args)
