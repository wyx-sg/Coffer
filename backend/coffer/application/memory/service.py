...
class MemoryService:
    ...
    async def add(self, *, store_name: str, text: str, actor: Actor) -> MemoryRecord:
        ...
        with self._tracer.span(
            "memory.add",
            attributes={"store_name": store_name, "char_size": len(text)},
        ):
            try:
    ...
    async def update(
        ...
        await self._store.open(store_name, config)
        with self._tracer.span(
            "memory.update", attributes={"store_name": store_name, "memory_id": memory_id}
        ):
    ...
    async def delete(self, *, store_name: str, memory_id: str, actor: str) -> None:
        ...
        await self._store.open(store_name, config)
        with self._tracer.span(
            "memory.delete", attributes={"store_name": store_name, "memory_id": memory_id}
        ):
    ...
    async def clear(self, *, store_name: str, actor: str) -> int:
        config = await self.get_store_config(store_name)
        await self._store.open(store_name, config)
        with self._tracer.span("memory.clear", attributes={"store_name": store_name}):
