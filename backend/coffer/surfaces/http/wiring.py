...
def wire_kb_kind(
    ...
    Each KB picks its backend by ``config.search_mode``: ``keyword`` (plain
    text search, no embeddings — the default) or ``semantic`` (LlamaIndex
    vectors). The dispatcher routes per-KB.
    """
    store = DispatchingKnowledgeBaseStore(
        keyword=KeywordKnowledgeBaseStore(text_dir=kb_text_dir),
        semantic=LlamaIndexKnowledgeBaseStore(),
    )
