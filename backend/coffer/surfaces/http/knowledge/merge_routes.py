...
``/merge``) — they cannot collide with ``/{name}`` because the ``/{name}`` route only ever
resolves an existing scope, and no scope may be named for an HTTP verb.
"""
...
from coffer.application.knowledge.merge import MergeProposal, StoreMergeService
from coffer.application.knowledge.merge_prompt import StoreEvidence
from coffer.domain.errors import MemoryStoreNotFound, ResourceNotFound
from coffer.domain.knowledge.entry import Actor
from coffer.surfaces.http.knowledge.merge_state import get_merge_service
from coffer.surfaces.http.knowledge.routes import _actor, router
