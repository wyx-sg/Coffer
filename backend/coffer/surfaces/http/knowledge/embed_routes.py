"""Async document re-embed routes (the non-blocking ingest-tail slice).

The slow part of ingest is convert+chunk+embed; a document indexed
...
from coffer.surfaces.http.knowledge.batch_state import get_batch_service
...
async def documents_status(
    name: str,
    batch: KnowledgeBaseBatchService = Depends(get_batch_service),  # noqa: B008
) -> DocumentStatusResponse:
    """In-flight re-embed state (queued / running / error) for a scope's documents."""
