"""``coffer knowledge merge-scan`` / ``merge`` (FR-056-059).

Split out to keep each module under the file-size ceiling; registers its
commands on the SAME ``knowledge`` Typer app via import side effect (imported
by ``main.py`` right after ``knowledge_cmd``).
"""
...
def merge_scan(
    ...
    """Scan for per-project scopes that describe the SAME project: pairs
    ...
    with c:
        ...
        r = c.post("/knowledge/merge_scan", timeout=600.0)
...
def merge(
    ctx: typer.Context,
    source: str = typer.Argument(..., help="Scope merged away (retired on success)."),
    target: str = typer.Argument(..., help="Scope that survives and absorbs the source."),
    no_organize: bool = typer.Option(
        False, "--no-organize", help="Skip the post-merge reorg pass on the target."
    ),
    output_json: bool = typer.Option(False, "--json"),
) -> None:
    """Merge one project scope into another (additive — memory is gained,
