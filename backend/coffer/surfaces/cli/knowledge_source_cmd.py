"""``coffer knowledge check-sources`` / ``update-source`` — external source tracking.

Split out to keep each module under the file-size ceiling. These commands
register onto the SAME ``knowledge`` Typer ``app`` (imported from
``knowledge_cmd``); importing this module (done by the CLI root) is what
attaches them.
"""
...
def check_sources(
    ...
    with c:
        r = c.post(f"/knowledge/{name}/check-sources")
...
def update_source(
    ...
    c, _info = _cli_client.client_or_exit()
    with c:
        r = c.post(f"/knowledge/{name}/documents/{document_id}/update-source")
        _cli_client.check(r, verbose=_verbose(ctx))
    out = r.json()
