...
class _Bridge:
    ...
    def __init__(self, info: DaemonInfo, *, agent_uid: str | None = None) -> None:
        ...
        # The shim's self-reported agent identity — the agent resource's uid,
        # from `--agent-uid <uid>`, written into installed MCP-server entries by
        # AgentMcpService.install. None for a launch that reported none — the
        # `coffer/agent-uid` meta key is simply omitted in that case, and the
        # gateway treats the session as unidentified.
    ...
    async def _pump_stdin(self, client: httpx.AsyncClient) -> None:
        ...
        while not self._stop.is_set():
            ...
            # report the agent's launch cwd
            # AND, when known, its self-reported `--agent-uid` identity at
            # session handshake. Both ride ``initialize`` params under ``_meta``
            # (an MCP-reserved extension key); the gateway threads cwd into tools
            ...
            if envelope.get("method") == "initialize":
                _inject_meta(envelope, self._agent_uid)
...
async def _async_main(argv: list[str] | None = None) -> int:
    ...
    bridge = _Bridge(info, agent_uid=args.agent_uid)
