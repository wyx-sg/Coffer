...
# Carrier-grade NAT (RFC 6598). Python's `is_private` does NOT flag this range
# before 3.13, yet it is a shared, non-public space that must never be a fetch
# target. We block it explicitly so the guard behaves identically across
...
def is_blocked_host(host: str) -> bool:
    ...
    private (RFC1918: 10/8, 172.16/12, 192.168/16, fc00::/7), CGNAT
    (100.64.0.0/10, RFC 6598), and reserved (multicast, unspecified). DNS
    resolution failures count as blocked, because we don't know what they map
    to.
...
def _ip_is_blocked(ip: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    if (
        ...
        return True
    # An IPv4-mapped IPv6 address (::ffff:a.b.c.d) carries an IPv4 payload that
    # the IPv6 predicates above don't classify; unwrap and re-check it so a
    # mapped private/CGNAT address can't slip through.
