...
This module belongs to the migration, not to the knowledge layer: a migration
describes one moment in history and must not track the product's live code.
It imports the standard library, ``yaml``, ``sqlalchemy`` and its own frozen
helpers (``knowledge_tree_0066_text``) — never ``coffer.*`` outside this
package.
"""
...
from coffer.infrastructure.persistence.migrations.knowledge_tree_0066_text import (
    README_NAME,
    knowledge_root,
    render_frontmatter,
    slugify,
...
def _write_readme(collection_dir: pathlib.Path) -> None:
    readme = collection_dir / README_NAME
    if readme.exists():
        return
