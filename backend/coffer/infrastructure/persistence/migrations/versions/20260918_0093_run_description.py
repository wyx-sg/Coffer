...
Neither is part of the projection: the run's status, its stage and its position
are folded from ``workflow_events`` and only the engine writes them. A title is
a label the person put on the work, which is why this column sits outside the
optimistic-lock cycle exactly as ``inputs`` does.
