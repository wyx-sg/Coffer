...
def upgrade() -> None:
    ...
    # Single internal-default invariant (spec provider-switching "Keep at most
    # one internal-engine default": normalise on import). The legacy registry
    # kept at most one is_default row via the app layer, but the table had no
    # UNIQUE guard — so if a divergent DB holds >1, keep only the
    # most-recently-updated as internal_default (ISO timestamps sort
    # lexically).
