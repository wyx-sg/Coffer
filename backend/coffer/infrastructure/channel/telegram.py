...
_CHUNK_LIMIT = 4000
# spec channels/telegram "Debounce an album into one turn": how long to wait for more album items
# sharing a media_group_id before flushing them as one turn. Small — they arrive back-to-back.
...
class TelegramAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._seen = SeenIds()  # "Process each inbound event once"
        # One turn per media_group_id (spec channels/telegram "Debounce an album into one turn").
        self._albums = AlbumBuffer(_ALBUM_DEBOUNCE_SECONDS, self._flush_album)
        self._task: asyncio.Task[None] | None = None
        self._profile_task: asyncio.Task[None] | None = None
        # "Probe platform capabilities and latch off rejected ones": what getMe said about this
        # bot, filled in start(). Defaults are the "could not introspect" state, never a guess.
        self._identity = BotIdentity()
        # Bot API 10.x capabilities, each live until the platform
    ...
    def identity(self) -> BotIdentity:
        """What ``getMe`` reported. Read by the channel health surface to diagnose a
        privacy-mode/configuration contradiction (spec channels/telegram "Report privacy mode
    ...
    async def start(self, callbacks: AdapterCallbacks) -> None:
        ...
        # Probe, never assume — identity for @mention matching, and the
        # privacy-mode flag the health surface reports on. Awaited,
        # because parsing a group message needs the bot's own id.
        self._identity = await probe_identity(self._call)
        # Register the command menu and fill an empty profile. NOT
    ...
    async def _dispatch(self, update: dict[str, Any]) -> None:
        if self._callbacks is None:
            return
        # "Process each inbound event once": the poll offset normally prevents replays, but a
        # reconnect race can re-deliver — drop an already-seen update_id (covers messages + taps).
        ...
        if isinstance(message, dict):
            ...
            if media_group_id:
                # An album arrives as separate updates sharing a media_group_id
                # — buffer them and flush ONE turn (dedup still runs per-update).
        ...
        if isinstance(query, dict):
            ...
            return
        # The user pressed the stop control on a streamed draft.
        ...
        if stopped is not None:
            ...
            return
        # The bot was removed from a chat ("Track the bot's own standing in a group") — the same
        # event SeaTalk reports directly, arriving here as a membership transition.
    ...
    async def _flush_album(
        ...
        """Emit ONE InboundMessage for a debounced album — all its attachments and the caption from
        whichever item carried it."""
        if self._callbacks is None:
            return
