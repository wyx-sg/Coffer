...
_CHUNK_LIMIT = 4000
# spec channels/telegram FR-015: how long to wait for more album items sharing
# a media_group_id before flushing them as one turn. Small — they arrive
# back-to-back.
...
class TelegramAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._seen = SeenIds()  # FR-040: drop a redelivered update
        # spec channels/telegram FR-015: one turn per media_group_id.
        self._albums = AlbumBuffer(_ALBUM_DEBOUNCE_SECONDS, self._flush_album)
        self._task: asyncio.Task[None] | None = None
        self._profile_task: asyncio.Task[None] | None = None
        # FR-044: what getMe said about this bot, filled in start(). Defaults
        # are the "could not introspect" state, never a guess.
        self._identity = BotIdentity()
        # FR-044: Bot API 10.x capabilities, each live until the platform
    ...
    def identity(self) -> BotIdentity:
        """What ``getMe`` reported (FR-044). Read by the channel health surface
        to diagnose a privacy-mode/configuration contradiction (spec channels/telegram FR-004)."""
    ...
    def capabilities(self) -> ChannelCapabilities:
        return ChannelCapabilities(
            supports_edit=True,
            supports_live_text=True,  # FR-039: the edit IS its live surface
            supports_typing=True,
            # FR-046: a rich message carries 32k characters against an ordinary
    ...
    async def start(self, callbacks: AdapterCallbacks) -> None:
        ...
        # FR-044: probe, never assume — identity for @mention matching, and the
        # privacy-mode flag the health surface reports on (spec channels/telegram FR-004). Awaited,
        # because parsing a group message needs the bot's own id.
        self._identity = await probe_identity(self._call)
        # FR-050: register the command menu and fill an empty profile. NOT
    ...
    async def stop(self) -> None:
        # spec channels/telegram FR-015: drop pending album timers/flush tasks so none leak past
        # stop.
    ...
    async def _dispatch(self, update: dict[str, Any]) -> None:
        ...
        if isinstance(message, dict):
            ...
            if media_group_id:
                # spec channels/telegram FR-015: an album arrives as separate updates sharing a
                # media_group_id
                # — buffer them and flush ONE turn (FR-040 dedup still runs per-update).
        ...
        if isinstance(query, dict):
            ...
            return
        # FR-048: the user pressed the stop control on a streamed draft.
        ...
        if stopped is not None:
            ...
            return
        # The bot was removed from a chat (FR-043) — the same event SeaTalk
    ...
    async def _flush_album(
        ...
        """spec channels/telegram FR-015: emit ONE InboundMessage for a debounced album — all its
    ...
    async def open_live_text(
        ...
        """FR-039/FR-047: a surface the reply grows on.
        ...
        afterwards, and it carries the stop control FR-048 routes back here.
    ...
    async def set_reaction(self, chat_id: str, message_id: str, emoji: str) -> None:
        # FR-038: react on the user's message (👀 receipt / ✅ completion, both in
