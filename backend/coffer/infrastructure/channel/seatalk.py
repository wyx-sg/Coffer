...
class SeaTalkAdapter:
    ...
    async def handle_event(self, envelope: dict[str, Any]) -> None:
        ...
        if event_type == "message_from_bot_subscriber":
            ...
            # A group card tap carries a ``group_id`` and a DM tap does not —
            # derive chat_kind from that so the core replies into the
            # group/thread and owner-gates on the right peer (FR-036). DMs are
            # 1:1 so the sender IS the employee_code; a group tap names the
            # tapper under ``sender``, like the @mention.
