...
the same files ripgrep would — every non-hidden file under the roots, in order
— and applies the same rules: a regex matched line by line, at most ``cap``
hits per file so "more exist" stays visible, hidden entries skipped (see "Hide
dot-prefixed entries except the inbox"), binary files skipped, and a wall-clock
budget that reports truncation rather than "no matches" when it runs out.
