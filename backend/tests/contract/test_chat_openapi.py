...
structurally matches openspec/specs/chat/contracts/api.openapi.yaml.
...
channel is the one the page continues.

Mirrors test_channel_contract.py, test_schemas_match_openapi.py and
test_openapi_shape.py. Does not do a full deep-diff — it checks that:
  - Every chat path declared in the yaml exists in the generated schema.
  - Every chat component schema declared in the yaml has corresponding required
...
def spec_doc() -> dict[str, Any]:
    """Parse openspec/specs/channels/contracts/api.openapi.yaml once per module."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/api/v1/chat/conversations/{id}/archive", "post"),
...
def test_chat_path_and_method_exists(
    ...
    """Every chat path+method declared in the OpenAPI yaml must exist in the
    generated schema."""
...
def test_every_yaml_path_is_implemented(
    ...
    """Drive the contract from the yaml itself: every chat path+method written in
    ...
    assert chat_paths, "the yaml declares no /api/v1/chat paths — wrong file?"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ChannelBindingOut", {"channel", "chat_id"}),
        ("MessageOut", {"id", "conversation_id", "seq", "role", "content", "status", "created_at"}),
        ("MessageListOut", {"messages"}),
        ("SendMessageRequest", set()),
        ("ChatAttachmentOut", {"id", "filename", "mime", "size"}),
        ("SendMessageAck", {"queued"}),
        ("PendingQueueIn", {"pending"}),
        ("PendingQueueOut", {"pending"}),
    ],
)
def test_chat_schema_required_fields(
    ...
    """Every chat schema component in the yaml must exist in the generated
...
def test_content_block_type_enum(generated_schema: dict[str, Any]) -> None:
    """ContentBlockOut must exist — it carries the block types declared in the
    yaml: text, tool_use, tool_result, attachment."""
