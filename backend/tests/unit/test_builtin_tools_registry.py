...
def test_register_and_lookup() -> None:
    r = BuiltinToolRegistry()
    r.register(_tool("search_knowledge_base"))
    assert len(r) == 1
    assert r.is_builtin(f"{COFFER_TOOL_PREFIX}search_knowledge_base")
    assert r.get(f"{COFFER_TOOL_PREFIX}search_knowledge_base") is not None
    assert r.get("search_knowledge_base") is None  # without prefix: no match
