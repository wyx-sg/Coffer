...
@pytest.mark.parametrize(
    ...
    [
        ...
        "[fe80::1]",
        "100.64.0.1",  # CGNAT (RFC 6598) — not flagged by is_private pre-3.13
        "100.127.255.254",  # CGNAT upper bound
        "[::ffff:100.64.0.1]",  # IPv4-mapped CGNAT must also be blocked
        "[::ffff:10.0.0.1]",  # IPv4-mapped RFC1918 must also be blocked
        "",
        "   ",
    ],
)
...
def test_check_url_rejects_cgnat():
    """CGNAT 100.64.0.0/10 (RFC 6598) is a shared, non-public range."""
    with pytest.raises(ValueError, match="SSRF"):
        check_url("http://100.64.0.1/x.git")
...
def test_allows_address_just_outside_cgnat():
    """100.128.0.0 is the first address above CGNAT and is public space."""
    assert is_blocked_host("100.128.0.0") is False
