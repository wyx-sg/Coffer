...
def test_inject_session_context_adds_cwd_for_memory_tool():
...
def test_inject_session_context_respects_caller_supplied_cwd():
    reg, _ = _registry_with_cwd_tool()
    session = _session_with(reg)
...
def test_inject_session_context_skips_tool_without_cwd_property():
    reg, _ = _registry_with_cwd_tool()
    session = _session_with(reg)
    session._session_cwd = "/work/repo"
    params = session._inject_session_context(
...
def test_inject_session_context_without_session_cwd_injects_nothing():
