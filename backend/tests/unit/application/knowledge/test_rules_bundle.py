...
from coffer.application.knowledge.rules_bundle import (
    SEEDED_RESUME_RULE,
...
_PROJECT = ResolvedScope(
    scope=KnowledgeScope.PROJECT,
    project_id="P" * 26,
    store_dir=Path("/p"),
    resource_name="project-" + "P" * 26,
)
_GLOBAL = ResolvedScope(
...
def _assembler(*, scopes, rules):
    ...
    return RulesBundleAssembler(
        ...
        get_rules=get_rules,
        scope_name_for=lambda rs: "global" if rs.is_global else f"project-{rs.project_id}",
    )
...
async def test_scope_resolution_failure_degrades_to_seeded():
    ...
    a = RulesBundleAssembler(
        ...
        get_rules=get_rules,
        scope_name_for=lambda rs: "global",
    )
