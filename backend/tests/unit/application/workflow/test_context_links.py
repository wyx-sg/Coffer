...
from coffer.application.workflow.context_lines import link_line
...
def test_a_recognised_link_says_what_it_is_and_an_ordinary_one_does_not() -> None:
    confluence = link_line(_link("https://mycorp.atlassian.net/wiki/spaces/ENG/pages/1/TD"), "")
    ordinary = link_line(_link("https://example.com/notes"), "")
...
def test_a_label_the_developer_gave_survives_either_way() -> None:
    assert "— the PRD" in link_line(_link("https://example.com/prd"), " — the PRD")
    assert "— the PRD" in link_line(_link("https://jira.internal/browse/COF-1"), " — the PRD")
