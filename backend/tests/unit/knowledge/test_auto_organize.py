"""Unit tests for KnowledgeAutoOrganizer (FR-035 session-end trigger)."""
...
class _FakeOrganizer:
    ...
    async def organize(self, *, scope_name: str) -> Any:
        if self._raises:
...
async def test_single_change_triggers_organize() -> None:
    fake = _FakeOrganizer()
    auto = KnowledgeAutoOrganizer(organize=fake, delay_seconds=0.02)
...
async def test_exception_in_organize_suppressed_and_scheduler_survives() -> None:
    ...
    auto = KnowledgeAutoOrganizer(organize=fake_raising, delay_seconds=0.02)
    # Should not raise
    await auto.on_change("global")
    await asyncio.sleep(0.1)
    ...
    auto2 = KnowledgeAutoOrganizer(organize=fake_ok, delay_seconds=0.02)
