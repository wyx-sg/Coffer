...
def test_memory_fact_has_no_free_form_type_field() -> None:
    ...
    assert "type" not in {f.name for f in fields(KnowledgeEntry)}
...
def test_fact_timestamps_persist_in_frontmatter(tmp_path) -> None:
    ...
    from coffer.domain.knowledge.entry import KnowledgeEntry
    from coffer.infrastructure.knowledge_scope.files import read_fact_file, write_fact_file
