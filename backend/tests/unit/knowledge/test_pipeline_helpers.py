"""Unit: the ingestion ``pipeline_helpers`` pure functions."""
...
def test_document_from_frontmatter_omits_absent_source_path() -> None:
    ...
    without = document_from_frontmatter(
        _STORE, "d1", {"title": "T", "source_filename": "a.md"}, path=_PATH, mtime=_MTIME
    )
...
def test_ingest_frontmatter_round_trips_timestamps() -> None:
    ...
    rebuilt = document_from_frontmatter(_STORE, "d1", fm, path=_PATH, mtime=_MTIME)
