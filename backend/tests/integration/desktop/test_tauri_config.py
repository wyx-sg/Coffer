...
def test_tray_and_close_interception_wired() -> None:
    ...
    # No redundant core:path:default — it is already implied by core:default
    # in Tauri 2.x, so listing it explicitly is a redundancy bug.
    assert "core:path:default" not in perms, (
        "core:path:default is implied by core:default; remove the redundant entry"
    )
