...
async def test_conversation_env_lookup_reaches_the_sdk_options(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    assert asked == [conv.id]  # looked up per turn, by conversation
    assert captured[0].env == {"COFFER_RUN_CONTEXT": "run_01J/att_07"}

    await engine.dispose()


@pytest.mark.asyncio
async def test_conversation_env_lookup_returning_none_changes_nothing(tmp_path) -> None:  # type: ignore[no-untyped-def]
    """An ordinary conversation — which is nearly all of them — must launch its
    agent exactly as it did before the seam existed."""
    repo, engine = await _repo(tmp_path)
    conv = await repo.create(_conv())
    factory, captured = _make_factory(_simple_messages())
...
@pytest.mark.asyncio
async def test_no_conversation_env_lookup_at_all_changes_nothing(tmp_path) -> None:  # type: ignore[no-untyped-def]
    """And with nothing wired — the default — the provider never asks."""
    repo, engine = await _repo(tmp_path)
    conv = await repo.create(_conv())
    factory, captured = _make_factory(_simple_messages())
    provider = ClaudeSdkProvider(conversations=repo, session_factory=factory)
