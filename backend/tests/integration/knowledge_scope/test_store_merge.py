...
from coffer.application.knowledge.scope import KnowledgeScope, ScopeResolver, project_scope_name
...
async def test_merge_consolidates_additively_and_retires_source(mem):
    ...
    src_dir = paths.scope_dir(project_scope_name(_SRC_ULID))
    dst_dir = paths.scope_dir(project_scope_name(_DST_ULID))
    ...
    assert await mem.documents.get_document(KIND_KNOWLEDGE, _SRC, "src-only") is None
...
async def test_merged_identity_resolves_to_survivor(mem, tmp_path):
    ...
    assert resolved.store_dir == paths.scope_dir(project_scope_name(_DST_ULID))
...
async def test_merge_retry_after_midway_failure_does_not_duplicate_files(mem, monkeypatch):
    ...
    cfg = KnowledgeConfig().model_dump(mode="json")
    src_dir, dst_dir = (
        paths.scope_dir(project_scope_name(_SRC_ULID)),
        paths.scope_dir(project_scope_name(_DST_ULID)),
    )
...
def _add_fact(c: TestClient, store: str, text: str) -> None:
    r = c.post(
        f"/api/v1/knowledge/{store}/entries",
...
def test_merge_route_merges_and_reports_no_model_organize(tmp_path, monkeypatch):
    ...
    with TestClient(app) as c:
        ...
        r = c.patch(f"/api/v1/knowledge/{_SRC}/label", json={"label": "老机器"}, headers=_HEADERS)
        ...
        names = {s["name"]: s for s in r.json()["scopes"]}
        assert _SRC not in names
        assert names[_DST]["entry_count"] == 3
        assert names[_DST]["label"] == "老机器"
        assert c.get(f"/api/v1/knowledge/{_SRC}", headers=_HEADERS).status_code == 404
...
def test_merge_route_validates_pairs_with_no_side_effects(tmp_path, monkeypatch):
    ...
    with TestClient(app) as c:
        ...
        for payload, expected in [
            ...
            r = c.post("/api/v1/knowledge/merge", json=payload, headers=_HEADERS)
            assert r.status_code == expected, (payload, r.text)

        # The failed merges left the source untouched.
        r = c.get(f"/api/v1/knowledge/{_SRC}", headers=_HEADERS)
        assert r.status_code == 200 and r.json()["entry_count"] == 1
...
def test_merge_scan_route_degrades_to_no_model_on_the_real_wiring(tmp_path, monkeypatch):
    ...
    with TestClient(app) as c:
        set_active_token(_TOKEN)
        _add_fact(c, _SRC, "src fact")
        r = c.post("/api/v1/knowledge/merge_scan", headers=_HEADERS)
        assert r.status_code == 200, r.text
        body = r.json()
