...
from coffer.application.knowledge.scope import GLOBAL_SCOPE_NAME, project_scope_name
...
async def test_remember_global_scope(mem) -> None:
    ...
    assert await mem.documents.count_documents(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME) == 1
...
async def test_project_scope_resolution_provisions_store(mem) -> None:
    ...
    assert resolved.scope is KnowledgeScope.PROJECT
    assert resolved.project_id != WORKSPACE_GLOBAL_PROJECT_ID
    # The backing Resource was auto-provisioned.
    listed = [r.name for r in await mem.resources.list(kind=KIND_KNOWLEDGE)]
    assert project_scope_name(resolved.project_id) in listed
...
async def test_update_fact_reflects_in_recall(mem) -> None:
    ...
    updated = await mem.service.update_fact(
        scope_name=store, fact_id=fact.id, new_body="replaced zebra text", actor="user"
    )
...
async def test_forget_removes_fact(mem) -> None:
    ...
    store = _project_store(mem)
    await mem.service.delete_fact(scope_name=store, fact_id=fact.id, actor="user")
...
async def test_user_add_sets_actor_user(mem) -> None:
    ...
    got = await mem.service.get_fact(scope_name=GLOBAL_SCOPE_NAME, fact_id=fact.id)
...
async def test_user_corrects_fact_via_write_api(mem) -> None:
    ...
    await mem.service.update_fact(
        scope_name=GLOBAL_SCOPE_NAME, fact_id=fact.id, new_body="dark mode preferred", actor="user"
    )
    got = await mem.service.get_fact(scope_name=GLOBAL_SCOPE_NAME, fact_id=fact.id)
...
async def test_user_delete_fact(mem) -> None:
    ...
    await mem.service.delete_fact(scope_name=GLOBAL_SCOPE_NAME, fact_id=fact.id, actor="user")
    facts, total = await mem.service.list_facts(scope_name=GLOBAL_SCOPE_NAME)
    assert total == 0
    assert facts == []
...
async def test_clear_scope_keeps_resource(mem) -> None:
    ...
    cleared = await mem.service.clear(scope_name=GLOBAL_SCOPE_NAME, actor="user")
    assert cleared == 3
    _, total = await mem.service.list_facts(scope_name=GLOBAL_SCOPE_NAME)
    assert total == 0
...
async def test_metrics(mem) -> None:
    ...
    m = await mem.service.metrics(scope_name=GLOBAL_SCOPE_NAME)
    assert m["fact_count"] == 1
    assert m["disk_bytes"] > 0


async def test_fact_count_uses_indexed_count_without_disk_scan(mem) -> None:
    """KB14: ``fact_count`` hits only the indexed DB count — no ``scan_scope_dir``
    ...
    disk_bytes."""
    from coffer.application.knowledge import queries as mem_queries_mod
    ...
    def _boom_scan(_path):
        raise AssertionError("scan_scope_dir must not run on the fact_count path")
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        mp.setattr(mem_queries_mod, "scan_scope_dir", _boom_scan)
        assert await mem.service.fact_count(scope_name=GLOBAL_SCOPE_NAME) == 2
...
async def test_vector_recall_falls_back_when_unconfigured(mem) -> None:
    ...
    hits, _fb = await mem.service.recall(
        cwd=None, query="okapi", scope=KnowledgeScope.GLOBAL, top_k=5, mode="vector"
    )
...
async def test_fact_too_long_rejected(mem) -> None:
    with pytest.raises(MemoryRejected) as exc:
        await mem.service.add_fact(
            ...
            title="x",
            description="d",
            body="x" * 10_000,  # default max_entry_chars is 8192
            actor="user",
        )
...
async def test_enabling_vector_backfills_existing_facts(mem) -> None:
    ...
    await mem.resources.update_config(
        ResourceRef(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME),
        new_config={
    ...
    hits, mode, fallback = await mem.service.recall_in_scope(
...
async def test_degraded_fact_is_retried_on_next_reconcile(mem) -> None:
    ...
    await mem.service.add_fact_to_scope(
        scope_name=GLOBAL_SCOPE_NAME, title="seed", description="", body="seed fact", actor="user"
    )
    await mem.resources.update_config(
        ResourceRef(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME),
        new_config={
    ...
    wombat = await mem.service.add_fact_to_scope(
        scope_name=GLOBAL_SCOPE_NAME,
        title="wombat",
        description="d",
    ...
    pending = await mem.documents.get_document(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME, wombat.id)
    ...
    cleared = await mem.documents.get_document(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME, wombat.id)
...
async def test_recall_hit_time_and_source_carry_fact_metadata(mem) -> None:
    ...
    assert fallback is False
    h = hits[0]
    doc = await mem.documents.get_document(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME, h.id)
...
async def test_grep_recall_budget_not_consumed_by_one_facts_lines(mem) -> None:
    ...
    hits, mode, _fb = await mem.service.recall_in_scope(
        scope_name="global", query="needle", mode="grep", top_k=3
    )
    assert mode == "grep"
...
async def test_recall_scope_global_returns_global_only(mem) -> None:
    ...
    resolved = await mem.service.resolve_scope(scope=KnowledgeScope.PROJECT, cwd=mem.project_cwd)
    project_store = project_scope_name(resolved.project_id)
    await mem.service.add_fact_to_scope(
        scope_name=project_store,
        title="p",
        description="",
    ...
    await mem.service.add_fact_to_scope(
        scope_name="global", title="g", description="", body="ocelot global fact", actor="user"
    )

    hits, _mode, _fb = await mem.service.recall_in_scope(
        scope_name=project_store, query="ocelot", scope="global", top_k=10
    )
...
async def test_new_fact_embeds_via_global_config_without_per_store_fields(mem) -> None:
    ...
    await mem.service.ensure_scope(GLOBAL_SCOPE_NAME)
    await mem.resources.update_config(
        ResourceRef(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME),
        new_config={
            "retrieval_modes": ["grep", "keyword", "vector"],
    ...
    vec = (mem.vec_stores or {}).get((KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME))
    assert vec is not None and len(vec._rows) == 1  # vec row written at remember() time
    hits, mode, fallback = await mem.service.recall_in_scope(
        scope_name=GLOBAL_SCOPE_NAME, query="quokka habitat", mode="vector", top_k=5
    )
    assert mode == "vector"
    assert fallback is False
...
async def test_store_delete_and_recreate_has_no_stale_vectors(mem) -> None:
    ...
    await mem.resources.delete(ResourceRef(KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME), actor="user")
    vec = (mem.vec_stores or {}).get((KIND_KNOWLEDGE, GLOBAL_SCOPE_NAME))
    ...
    await mem.service.ensure_scope(GLOBAL_SCOPE_NAME)
    hits, _mode, _fb = await mem.service.recall_in_scope(
        scope_name=GLOBAL_SCOPE_NAME, query="vanishing", mode="keyword", top_k=5
    )
    assert hits == []
