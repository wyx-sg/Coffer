...
from coffer.application.knowledge.consolidate import StoreConsolidator
...
async def wired(tmp_path: pathlib.Path, monkeypatch):  # type: ignore[no-untyped-def]
    ...
    resources._kinds[KIND_KNOWLEDGE] = make_knowledge_kind(None)  # type: ignore[attr-defined,arg-type]
...
async def test_legacy_store_adopted_under_portable_id(wired) -> None:  # type: ignore[no-untyped-def]
    ...
    legacy_name = project_scope_name(legacy_id)

    resolved = await resolver.resolve(scope=KnowledgeScope.PROJECT, cwd=str(project_root))
    ...
    names = {r.name for r in await resources.list(kind=KIND_KNOWLEDGE)}
    assert new_name in names
    assert legacy_name not in names

    again = await resolver.resolve(scope=KnowledgeScope.PROJECT, cwd=str(project_root))
...
async def test_surviving_legacy_store_merges_even_when_portable_exists(wired) -> None:  # type: ignore[no-untyped-def]
    ...
    await resolver.resolve(scope=KnowledgeScope.PROJECT, cwd=str(project_root))
...
async def test_boot_consolidator_converges_and_stays_stable(wired) -> None:  # type: ignore[no-untyped-def]
    ...
    names = {r.name for r in await resources.list(kind=KIND_KNOWLEDGE)}
    assert project_scope_name(PORTABLE) in names
...
async def test_fresh_project_provisions_directly_no_migration(wired) -> None:  # type: ignore[no-untyped-def]
    ...
    assert resolved.project_id == PORTABLE
    names = {r.name for r in await resources.list(kind=KIND_KNOWLEDGE)}
    assert project_scope_name(project_ulid(str(project_root))) not in names
