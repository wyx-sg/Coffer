...
async def test_set_handoff_then_resume(handoff) -> None:
    ...
    freshness note, AND the handoff is never returned by ``coffer__search``."""
    reg = BuiltinToolRegistry()
    register_knowledge_builtin_tools(
        reg, knowledge_service=handoff.knowledge_service, handoff_service=handoff.svc
    )
    set_tool = reg.get(f"{COFFER_TOOL_PREFIX}set_handoff")
    resume_tool = reg.get(f"{COFFER_TOOL_PREFIX}resume")
    search_tool = reg.get(f"{COFFER_TOOL_PREFIX}search")
    ...
    # ...and the handoff is NEVER returned by search (grep is the only lane that
    # recurses into handoff/).
    res = await search_tool.handler({"query": secret, "top_k": 20, "cwd": handoff.cwd})
    leaked = [h for h in res["hits"] if secret in h["text"]]
    assert leaked == [], f"handoff leaked into coffer__search: {leaked}"


async def test_handoff_never_returned_by_search(handoff) -> None:
    """The retrieval-isolation guarantee: a saved handoff body MUST NOT surface
    in ``coffer__search`` — not even when the scope's mode is ``grep``, which
    (unlike keyword/vector) descends into the scope's ``handoff/`` subdir.
    Regression for the leak where a handoff file parsed gracefully into a fake
    entry and was emitted as a hit."""
    reg = BuiltinToolRegistry()
    register_knowledge_builtin_tools(
        reg, knowledge_service=handoff.knowledge_service, handoff_service=handoff.svc
    )
    ...
    # grep is the ONLY lane that recurses into handoff/ — assert it stays clean
    # across every scope default_mode.
    res = await search_tool.handler({"query": secret, "top_k": 20, "cwd": handoff.cwd})
    leaked = [h for h in res["hits"] if secret in h["text"]]
    assert leaked == [], f"handoff leaked into coffer__search: {leaked}"
