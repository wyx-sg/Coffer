...
async def wired(tmp_path: pathlib.Path, monkeypatch):
    ...
    # kind registered. ``cleanup_scope`` (the only place ``service`` is used) is
...
async def test_resolving_project_scope_records_root(wired) -> None:
    ...
    resolved = await scope.resolve(scope=KnowledgeScope.PROJECT, cwd=str(project_root / "src"))
    store_name = project_scope_name(resolved.project_id)
    assert await roots.get(store_name) == str(project_root)


async def test_global_scope_has_no_project_root(wired) -> None:
    scope, roots, _ = wired
    await scope.resolve(scope=KnowledgeScope.GLOBAL, cwd=None)
