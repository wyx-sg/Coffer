"""Integration: KnowledgeService lane read methods (Slice 7 lane UI).
...
async def _global_store_dir(mem) -> Path:
    ...
    return (await mem.service.resolved_scope(GLOBAL_SCOPE_NAME)).store_dir
...
async def test_read_handoff_scenes_per_branch(mem) -> None:
    ...
    scenes = await lane_reads.handoff_for_store(GLOBAL_SCOPE_NAME, mem.service.resolved_scope)
...
async def test_read_handoff_empty_store(mem) -> None:
    ...
    assert await lane_reads.handoff_for_store(GLOBAL_SCOPE_NAME, mem.service.resolved_scope) == []
...
async def test_read_consolidation_log_present(mem) -> None:
    ...
    log = await lane_reads.consolidation_log_for_store(
        GLOBAL_SCOPE_NAME, mem.service.resolved_scope
    )
