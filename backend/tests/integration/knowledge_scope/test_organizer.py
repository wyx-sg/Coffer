...
async def test_organizer_drains_inbox_into_topic_doc(mem) -> None:
    ...
    result = await org.organize(scope_name="global")
...
async def test_organize_no_model_is_clean_noop(mem) -> None:
    ...
    from coffer.infrastructure.knowledge_scope.topic_files import list_topic_docs
...
async def test_second_organize_on_empty_inbox_is_noop(mem) -> None:
    ...
    org = _make_organizer(mem, llm, _Models(_model()))
    first = await org.organize(scope_name="global")
    ...
    second = await org2.organize(scope_name="global")
...
async def test_index_and_changelog_never_surface_in_recall(mem) -> None:
    ...
    org = _make_organizer(mem, llm, _Models(_model()))
    await org.organize(scope_name="global")
    ...
    hits, _mode, _fb = await mem.service.recall_in_scope(
        scope_name="global", query="ZEBRA", scope="global", mode="grep"
    )
    for h in hits:
...
async def test_organizer_routes_rule_into_rules_lane(mem) -> None:
    ...
    hits, _mode, _fb = await mem.service.recall_in_scope(
        scope_name="global", query="make verify", scope="global"
    )
    for h in hits:
...
async def test_organize_splits_oversized_rules_lane(mem) -> None:
    ...
    from coffer.infrastructure.knowledge_scope.rules_files import append_rule, count_rules
