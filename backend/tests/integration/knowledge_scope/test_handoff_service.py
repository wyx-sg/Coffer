...
async def test_set_then_resume_roundtrip(handoff) -> None:
    res = await handoff.svc.set_handoff(cwd=handoff.cwd, body="at step 3\nnext: step 4")
...
async def test_set_handoff_overwrites_same_branch(handoff) -> None:
    await handoff.svc.set_handoff(cwd=handoff.cwd, body="first")
    await handoff.svc.set_handoff(cwd=handoff.cwd, body="second")
    got = await handoff.svc.resume(cwd=handoff.cwd)
    assert got is not None
...
async def test_handoff_is_per_branch_independent(handoff) -> None:
    ...
    await handoff.svc.set_handoff(cwd=handoff.cwd, body="WORK_BODY")
    ...
    await handoff.svc.set_handoff(cwd=handoff.cwd, body="OTHER_BODY")
