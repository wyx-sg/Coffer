...
from coffer.infrastructure.knowledge_scope.scope_fs import git_branch, git_root, project_ulid
...
async def mem(tmp_path: pathlib.Path, monkeypatch):
    ...
    # Register the knowledge kind so auto-provision (resources.register) works.
    resources._kinds[KIND_KNOWLEDGE] = make_knowledge_kind(service)  # type: ignore[attr-defined]
...
async def handoff(tmp_path: pathlib.Path, monkeypatch):
    ...
    not. Reuses the same temp-SQLite + ``$COFFER_KNOWLEDGE_ROOT`` wiring as ``mem``.
    """
    monkeypatch.setenv("COFFER_KNOWLEDGE_ROOT", str(tmp_path / "knowledge"))
    ...
    memory_service = KnowledgeService(
        resource_service=resources,
        documents=documents,
        scope_resolver=scope,
        reconciler=reconciler,
    ...
    resources._kinds[KIND_KNOWLEDGE] = make_knowledge_kind(memory_service)  # type: ignore[attr-defined]
