...
The knowledge tidy pass rewrites files a human and an agent manage together,
with no diff anyone approved. That is defensible on one machine and not on
several, and the spec's ``## Unattended rewriters`` section states three
normative rules to make it safe:

1. **An unattended rewriter of synced content names one owner machine**, and is
   a no-op everywhere else — otherwise two machines merge the same pair of
   notes into two *different* topic documents, git merges that cleanly (both
   agree the sources are deleted, the two topics are additions at different
   paths), and the vault ends up holding the same knowledge twice with nothing
   in conflict.
...
async def test_a_round_waits_for_whatever_else_is_rewriting_the_vault(pair) -> None:
    ...
    content — the tidy pass — can take the same one. This holds it the way a
    pass in progress would and shows that a round genuinely waits rather than
...
async def test_tidy_names_one_owner_machine_and_is_a_no_op_elsewhere(pair) -> None:
    """The gate must answer True on exactly one machine of a converged pair.

    Asserting on ``tidy_runs_on`` rather than on a pass actually running is
    ...
    internal model configured would be testing the tidy pass instead.
    """
    ...
    enabled_here = GlobalInternalEngineConfig(
        model="m", updated_at=datetime.now(tz=UTC), auto_tidy_enabled=True
    )
    # No owner: a single-machine vault, where "here" is the only answer.
    assert enabled_here.tidy_runs_on(a.machine_id) is True
    assert enabled_here.tidy_runs_on(b.machine_id) is True

    owned_by_a = dataclasses.replace(enabled_here, tidy_owner_machine_id=a.machine_id)
    assert owned_by_a.tidy_runs_on(a.machine_id) is True
    assert owned_by_a.tidy_runs_on(b.machine_id) is False, (
    ...
    # An owner naming a machine nobody has heard of stops tidy everywhere.
    # No tidy costs housekeeping; tidy everywhere costs duplicated knowledge.
    orphaned = dataclasses.replace(enabled_here, tidy_owner_machine_id="0000000000000000")
    assert orphaned.tidy_runs_on(a.machine_id) is False
    assert orphaned.tidy_runs_on(b.machine_id) is False

    # Off beats ownership.
    off = dataclasses.replace(owned_by_a, auto_tidy_enabled=False)
    assert off.tidy_runs_on(a.machine_id) is False


@pytest.mark.acceptance(spec="vault-sync", scenario="an edit outlives a tidy deletion")
async def test_an_edit_beats_a_tidy_deletion_without_reporting_a_conflict(pair) -> None:
    a, b = pair
    a.write_knowledge("notes", "merged-away", "the original note\n")
    await settle(a, b)

    # A owns tidy: its pass merged this note elsewhere and deleted it.
