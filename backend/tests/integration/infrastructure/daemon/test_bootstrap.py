...
def test_acquire_writes_daemon_json_with_0600(
    ...
    assert isinstance(raw["token"], str) and len(raw["token"]) >= 32, "Token missing or too short"
...
def test_second_daemon_picks_free_port_and_clients_discover(
    ...
    # Hold one port in the range so allocate() must move on.
    holder = socket.socket()
    holder.bind(("127.0.0.1", 59500))
    holder.listen(1)
    try:
        info_a = bootstrap.acquire()
        ...
        # Now hold info_a.port too and acquire a second time — the allocator
        # must pick yet another free port and the daemon.json must update.
        holder2 = socket.socket()
        holder2.bind(("127.0.0.1", info_a.port))
        holder2.listen(1)
        try:
            info_b = bootstrap.acquire()
