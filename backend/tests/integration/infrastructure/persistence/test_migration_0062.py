"""Revision 0062: the curated ``models`` set comes back OFF every agent.
...
def test_0062_strips_the_key_and_leaves_everything_else(tmp_path, monkeypatch):
...
def test_0062_downgrade_restores_the_uncurated_default(tmp_path, monkeypatch):
...
def test_0062_leaves_an_unparseable_row_alone(tmp_path, monkeypatch):
