...
# ---------------------------------------------------------------------------
# Plugins — Cursor / OpenCode / OpenClaw / Hermes (capability manifest)
# ---------------------------------------------------------------------------


def _register_opencode(c: TestClient, tmp_path: pathlib.Path, config: dict) -> None:
    cfg_dir = tmp_path / ".config" / "opencode"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / "opencode.json").write_text(json.dumps(config), encoding="utf-8")
    r = c.post("/api/v1/agents", json={"type": "opencode", "name": "oc"})
    assert r.status_code == 201, r.text


def _register_openclaw(c: TestClient, tmp_path: pathlib.Path, config: dict) -> None:
    cfg_dir = tmp_path / ".openclaw"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / "openclaw.json").write_text(json.dumps(config), encoding="utf-8")
    r = c.post("/api/v1/agents", json={"type": "openclaw", "name": "ow"})
    assert r.status_code == 201, r.text


def _register_cursor(c: TestClient, tmp_path: pathlib.Path, extensions: list) -> None:
    cfg_dir = tmp_path / ".cursor"
    ext_dir = cfg_dir / "extensions"
    ext_dir.mkdir(parents=True, exist_ok=True)
    (ext_dir / "extensions.json").write_text(json.dumps(extensions), encoding="utf-8")
    r = c.post("/api/v1/agents", json={"type": "cursor", "name": "cu"})
    assert r.status_code == 201, r.text


def _register_hermes(c: TestClient, tmp_path: pathlib.Path) -> None:
    cfg_dir = tmp_path / ".hermes"
    cfg_dir.mkdir(parents=True, exist_ok=True)
    (cfg_dir / "config.yaml").write_text("mcp_servers: {}\n", encoding="utf-8")
    r = c.post("/api/v1/agents", json={"type": "hermes", "name": "he"})
    assert r.status_code == 201, r.text


@pytest.mark.acceptance(spec="004-agent-registry", scenario="list OpenCode plugins")
def test_list_opencode_plugins(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 59970)
    with _client(app) as c:
        _register_opencode(c, tmp_path, {"plugin": ["alpha", "beta"]})
        r = c.get("/api/v1/agents/oc/plugins")
        assert r.status_code == 200, r.text
        body = r.json()
        assert body["parse_errors"] == []
        assert {p["id"] for p in body["items"]} == {"alpha", "beta"}


@pytest.mark.acceptance(spec="004-agent-registry", scenario="toggle an OpenCode plugin")
def test_toggle_opencode_plugin(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 59980)
    with _client(app) as c:
        _register_opencode(c, tmp_path, {"plugin": ["alpha", "beta"], "other": 1})
        r = c.patch("/api/v1/agents/oc/plugins/alpha", json={"enabled": False})
        assert r.status_code == 204, r.text
        data = json.loads((tmp_path / ".config" / "opencode" / "opencode.json").read_text())
        assert "alpha" not in data["plugin"]
        assert data["other"] == 1  # siblings preserved


@pytest.mark.acceptance(spec="004-agent-registry", scenario="uninstall an OpenCode plugin")
def test_uninstall_opencode_plugin(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 59990)
    with _client(app) as c:
        _register_opencode(c, tmp_path, {"plugin": ["alpha", "beta"]})
        r = c.delete("/api/v1/agents/oc/plugins/beta")
        assert r.status_code == 204, r.text
        data = json.loads((tmp_path / ".config" / "opencode" / "opencode.json").read_text())
        assert "beta" not in data["plugin"]


@pytest.mark.acceptance(spec="004-agent-registry", scenario="list + toggle OpenClaw plugins")
def test_openclaw_plugins_list_and_toggle(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 58010)
    with _client(app) as c:
        _register_openclaw(
            c,
            tmp_path,
        ...
        r = c.get("/api/v1/agents/ow/plugins")
        assert r.status_code == 200, r.text
        by_id = {p["id"]: p["enabled"] for p in r.json()["items"]}
        assert by_id == {"alpha": True, "beta": False}

        r = c.patch("/api/v1/agents/ow/plugins/beta", json={"enabled": True})
        assert r.status_code == 204, r.text
        r = c.get("/api/v1/agents/ow/plugins")
        assert {p["id"]: p["enabled"] for p in r.json()["items"]}["beta"] is True


@pytest.mark.acceptance(spec="004-agent-registry", scenario="list Cursor extensions read-only")
def test_cursor_plugins_list_only(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 58020)
    with _client(app) as c:
        _register_cursor(
            c,
            tmp_path,
        ...
        r = c.get("/api/v1/agents/cu/plugins")
        assert r.status_code == 200, r.text
        assert {p["id"] for p in r.json()["items"]} == {"ms-python.python"}

        # Toggle and uninstall are rejected for the read-only model.
        ext = tmp_path / ".cursor" / "extensions" / "extensions.json"
        before = ext.read_bytes()
        r = c.patch("/api/v1/agents/cu/plugins/ms-python.python", json={"enabled": False})
        assert r.status_code == 422, r.text
        assert r.json()["error"]["code"] == "PLUGIN_TOGGLE_UNSUPPORTED"
        r = c.delete("/api/v1/agents/cu/plugins/ms-python.python")
        assert r.status_code == 422, r.text
        assert r.json()["error"]["code"] == "PLUGIN_UNINSTALL_UNSUPPORTED"
        assert ext.read_bytes() == before  # never written


@pytest.mark.acceptance(spec="004-agent-registry", scenario="Hermes has no plugin facet")
def test_hermes_plugins_empty_and_unsupported(tmp_path, monkeypatch):
    app = _app(tmp_path, monkeypatch, 58030)
    with _client(app) as c:
        _register_hermes(c, tmp_path)
        r = c.get("/api/v1/agents/he/plugins")
        assert r.status_code == 200, r.text
        assert r.json()["items"] == []

        r = c.patch("/api/v1/agents/he/plugins/anything", json={"enabled": True})
        assert r.status_code == 422, r.text
        assert r.json()["error"]["code"] == "PLUGIN_TOGGLE_UNSUPPORTED"
        r = c.delete("/api/v1/agents/he/plugins/anything")
        assert r.status_code == 422, r.text
        assert r.json()["error"]["code"] == "PLUGIN_UNINSTALL_UNSUPPORTED"
