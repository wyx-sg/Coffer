"""Integration tests for /api/v1/settings — master key storage.

The daemon's port is deliberately not served here: it has to be changeable when
the daemon cannot start, which is precisely when no route of its own can
answer. `coffer daemon port` owns it, and test_daemon_cmd.py covers it.
"""
