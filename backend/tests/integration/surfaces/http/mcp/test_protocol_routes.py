...
async def test_get_with_session_id_accepted(
    ...
    """Verify the GET /mcp endpoint accepts the request (session is created).

    httpx's ASGITransport collects the full response body before returning,
    so we cannot truly stream-test the SSE body (it would block forever).
    Instead we verify two things:
      1. The session is registered after a valid POST initialize.
      2. An invalid GET (no session id) is rejected with 400.
    The SSE content-type verification is done at the routing unit level; the
    EventSourceResponse behaviour is covered by sse-starlette's own tests.
    """
    ...
    # GET without session id → 400 (already covered by test_get_without_session_id_400
    # but including here for completeness of the GET contract)
    r = await http_client.get("/mcp")
    assert r.status_code == 400
...
async def test_downstream_response_routes_to_pending_server_request(
    ...
    handle_response_from_downstream and return 200 immediately (not an error).
