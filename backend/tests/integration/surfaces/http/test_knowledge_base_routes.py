...
class _SearchMetricsFakeKbService:
    ...
    layer is exercised without booting LlamaIndex or writing real docs."""

    def __init__(self) -> None:
        self.searches: list[tuple[str, str, int]] = []
        self.metrics_calls: list[str] = []

    async def search(self, *, kb_name: str, query: str, top_k: int):
        from coffer.domain.knowledge_base.store import Passage
...
def test_metrics_route_returns_kb_metrics(tmp_path, monkeypatch):
    """TEST22-016: GET /metrics → 200 with document_count + disk_bytes."""
    from coffer.surfaces.http.dependencies import get_kb_service
    ...
    with TestClient(app) as c:
        ...
        assert r.status_code == 200, r.text
        body = r.json()
        assert body == {"document_count": 7, "disk_bytes": 12345}
