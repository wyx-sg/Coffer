...
def test_document_lock_endpoint_blocks_mutations(tmp_path, monkeypatch):
    """PATCH .../documents/{id} {locked} flips the lock; while locked, edit and
    delete are refused with a DOCUMENT_LOCKED (409) envelope (ADR-028 / FR-021).
    DocumentOut carries the new locked + project_id fields."""
    ...
    with TestClient(app) as c:
        ...
        assert doc["project_id"] == "00000000000000000000000000"
        ...
        assert lk.json()["locked"] is True

        # edit + delete are refused while locked
        ed = c.put(
            f"/api/v1/knowledge_bases/kb/documents/{doc_id}",
            json={"markdown": "# A\n\nhacked\n"},
            headers=_HEADERS,
        )
        assert ed.status_code == 409, ed.text
        _assert_envelope(ed.json(), "DOCUMENT_LOCKED")
        de = c.delete(f"/api/v1/knowledge_bases/kb/documents/{doc_id}", headers=_HEADERS)
        assert de.status_code == 409, de.text
        _assert_envelope(de.json(), "DOCUMENT_LOCKED")

        # unlock → mutations allowed again
        un = c.patch(
            f"/api/v1/knowledge_bases/kb/documents/{doc_id}",
            json={"locked": False},
            headers=_HEADERS,
        )
        assert un.status_code == 200 and un.json()["locked"] is False
        assert (
