...
Spec 005 §User Story 7 — every CLI verb mirrors the REST surface.
"""
...
def skill_cli_daemon(tmp_path, monkeypatch):
    ...
    + agent skill_dirs underneath it.
    """
...
def _register_agent(home: pathlib.Path, name: str) -> pathlib.Path:
    """Register an agent via the CLI; return the path of its skill_dir."""
    skill_dir = home / f"{name}-skills"
    skill_dir.mkdir()
    r = _runner.invoke(
        cli_app,
        ["agent", "add", "cursor", "--name", name, "--skill-dir", str(skill_dir)],
    )
...
def test_skill_enable_then_disable(skill_cli_daemon):
    ...
    skill_dir = _register_agent(skill_cli_daemon, "cur")
    src = skill_cli_daemon / "src"
