"""Integration: `coffer knowledge merge-scan` / `coffer knowledge merge` (FR-056/057).

Mirrors ``test_knowledge_cmd.py``: full app via ``create_app``, CLI client
routed to a Starlette ``TestClient``. The test env has no internal engine, so
the scan degrades to ``no_model`` (the engine tier itself is unit-tested)."""
...
_runner = CliRunner()
_TOKEN = "test-token-knowledge-merge-cli"
_SRC = "project-" + "B" * 26
_DST = "project-" + "A" * 26
...
def _add_entry(c: TestClient, scope: str, text: str) -> None:
    r = c.post(f"/knowledge/{scope}/entries", json={"title": text, "text": text})
    assert r.status_code == 201, r.text


def test_merge_scan_json_reports_no_model(merge_cli_daemon):
    result = _runner.invoke(cli_app, [KIND_KNOWLEDGE, "merge-scan", "--json"])
...
def test_merge_scan_table_reports_no_candidates(merge_cli_daemon):
    result = _runner.invoke(cli_app, [KIND_KNOWLEDGE, "merge-scan"])
    assert result.exit_code == 0, result.output
...
def test_merge_happy_path_human_summary(merge_cli_daemon):
    _add_entry(merge_cli_daemon, _SRC, "src entry")
    _add_entry(merge_cli_daemon, _DST, "dst entry")
    result = _runner.invoke(cli_app, [KIND_KNOWLEDGE, "merge", _SRC, _DST, "--no-organize"])
    assert result.exit_code == 0, result.output
    assert f"merged {_SRC} into {_DST}" in result.output
    assert "organize: skipped" in result.output

    listing = _runner.invoke(cli_app, [KIND_KNOWLEDGE, "list", "--json"])
    names = [s["name"] for s in json.loads(_extract_json(listing.output))["scopes"]]
...
def test_merge_json_output_carries_aliases(merge_cli_daemon):
    ...
    result = _runner.invoke(
        cli_app, [KIND_KNOWLEDGE, "merge", _SRC, _DST, "--no-organize", "--json"]
    )
    assert result.exit_code == 0, result.output
    data = json.loads(_extract_json(result.output))
...
def test_merge_invalid_pair_exits_nonzero(merge_cli_daemon):
    result = _runner.invoke(cli_app, [KIND_KNOWLEDGE, "merge", "global", _DST])
