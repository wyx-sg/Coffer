...
# ---------------------------------------------------------------------------
# Chat CLI fixture (spec 008)
# ---------------------------------------------------------------------------


def _build_chat_app() -> FastAPI:
    """Build a minimal FastAPI app with chat + model routes for CLI tests."""
    from coffer.surfaces.http import errors as err_handlers
    ...
    fapp.include_router(model_router)
    fapp.include_router(turn_router)
...
_CHAT_TOKEN = "test-token-chat"
_CHAT_PORT = 8001


@pytest.fixture
def in_proc_chat_daemon(tmp_path, monkeypatch):  # type: ignore[no-untyped-def]
    """In-process daemon with chat routes wired; monkeypatches client_or_exit."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setenv("HOME", str(home))

    fapp = _build_chat_app()
    set_active_token(_CHAT_TOKEN)
    ...
    fake_client = TestClient(
        fapp,
        base_url="http://localhost/api/v1",
        headers={"X-Coffer-Token": _CHAT_TOKEN},
        raise_server_exceptions=False,
    )
