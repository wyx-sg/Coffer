...
def test_audit_list_filter_kind(in_proc_daemon):
    ...
    entries = json.loads(result.output)["audit_events"]
    # Must return at least one entry — otherwise the per-entry check below
    # passes vacuously and a broken --kind filter (returning nothing) would
    # look green.
    assert len(entries) >= 1, "expected at least one fake_kind audit entry"
    # And every returned entry must be for fake_kind.
    for e in entries:
