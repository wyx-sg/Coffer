...
path as every other resource, gets back ``workflow:<name>``, and leaves a
...
async def test_a_registered_template_is_a_workflow_resource_with_an_audit_entry(
    ...
    # FR-001: the ref is `workflow:<name>`, and the document is stored as written.
    assert str(created.ref) == "workflow:ship-a-feature"
    assert created.config == TEMPLATE
    # The framework's own lifecycle, nothing kind-specific: enabled on creation,
    # readable back by ref.
    assert created.enabled is True
    assert (await service.get(ResourceRef(KIND_WORKFLOW, "ship-a-feature"))).config == TEMPLATE
    assert [r.name for r in await service.list(kind=KIND_WORKFLOW)] == ["ship-a-feature"]

    # FR-040: the write left a trail.
    entries = await audit.query(kind=KIND_WORKFLOW, name="ship-a-feature")
