"""Fixtures for the ingestion half of the knowledge application layer.
...
class KBHarness:
    ...
    async def create_kb(self, name: str, *, config: dict | None = None) -> None:
        """Create a NAMED collection. Named scopes never auto-provision, so
        every ingestion test starts by creating one deliberately."""
...
async def kb(tmp_path: pathlib.Path, monkeypatch):
    ...
    scope = ScopeResolver(
        resources=resources,
        git_root=lambda _cwd: None,
        project_ulid=lambda _root: "0" * 26,
...
def vector_config() -> dict:
    return {
        "retrieval_modes": ["keyword", "grep", "vector"],
