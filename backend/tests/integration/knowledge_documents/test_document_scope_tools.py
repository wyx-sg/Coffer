"""Integration: the six built-in MCP tools, driven against a NAMED collection.

The sibling ``knowledge_scope/test_entry_scope_tools.py`` drives the same six
...
def _registry(kb) -> BuiltinToolRegistry:
    reg = BuiltinToolRegistry()
    register_knowledge_builtin_tools(reg, knowledge_service=kb.service)
...
async def test_the_six_tools_are_registered(kb) -> None:
    reg = _registry(kb)
    # Twelve tools over two kinds became six over one. Documents stay
    # co-managed (spec knowledge FR-062): read AND write are both here. Exact
    # equality, so a tool that silently disappears fails here.
