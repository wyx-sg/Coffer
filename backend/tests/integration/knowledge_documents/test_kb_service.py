...
async def _ingest(kb, name: str, filename: str, data: bytes, **kw):
    return await kb.service.ingest_bytes(
        scope_name=name, filename=filename, raw_bytes=data, actor="user", **kw
    )
...
async def test_create_kb_makes_dirs(kb) -> None:
    ...
    listed = await kb.resources.list(kind=KIND_KNOWLEDGE)
...
async def test_ingest_converts_csv_to_markdown(kb) -> None:
    ...
    assert await kb.documents.count_chunks(KIND_KNOWLEDGE, "kb1") >= 1
...
async def test_reupload_updated_file_updates_in_place(kb) -> None:
    ...
    assert (await kb.service.search(scope_name="kb1", query="apple", top_k=5)).passages == ()
...
async def test_list_documents_paginated(kb) -> None:
    ...
    docs, total = await kb.service.list_documents(scope_name="kb1", limit=1, offset=0)
    assert total == 2
    assert len(docs) == 1
...
async def test_keyword_search_ranks(kb) -> None:
    ...
    result = await kb.service.search(scope_name="kb1", query="fox", top_k=5)
...
async def test_keyword_search_matches_cjk(kb) -> None:
    ...
    result = await kb.service.search(scope_name="kb1", query="向量检索", top_k=5)
    assert result.mode == "keyword"
    ...
    short = await kb.service.search(scope_name="kb1", query="向量", top_k=5)
...
async def test_grep_returns_hits(kb) -> None:
    ...
    result = await kb.service.grep(scope_name="kb1", pattern="make release")
...
async def test_vector_search_with_embedding(kb, vector_config) -> None:
    ...
    result = await kb.service.search(scope_name="kb1", query="deploys", top_k=5, mode="vector")
...
async def test_hybrid_search_fuses_keyword_and_vector(kb, vector_config) -> None:
    ...
    result = await kb.service.search(scope_name="kb1", query="deploys", top_k=5, mode="hybrid")
...
async def test_hybrid_is_default_mode_when_vector_enabled(kb, vector_config) -> None:
    ...
    config = await kb.service.get_config("kb1")
    assert config.default_mode == "hybrid"
    await _ingest(kb, "kb1", "a.md", b"# Alpha\n\nthe alpha passage about deploys")
    result = await kb.service.search(scope_name="kb1", query="deploys", top_k=5)
    assert result.mode == "hybrid"
    assert result.fallback is None
    assert len(result.passages) >= 1
...
async def test_edit_sets_edited_mode_and_reflects_in_search(kb) -> None:
    ...
    assert (await kb.service.search(scope_name="kb1", query="banana", top_k=5)).passages == ()
...
async def test_out_of_band_edit_visible_on_reindex_on_read(kb) -> None:
    ...
    assert len((await kb.service.search(scope_name="kb1", query="narwhal", top_k=5)).passages) == 1
    ...
    assert (await kb.service.search(scope_name="kb1", query="narwhal", top_k=5)).passages == ()
    hits = (await kb.service.search(scope_name="kb1", query="platypus", top_k=5)).passages
    assert len(hits) == 1
...
async def test_out_of_band_file_removal_pruned_on_read(kb) -> None:
    ...
    assert len((await kb.service.search(scope_name="kb1", query="vanish", top_k=5)).passages) == 1
    paths.doc_path("kb1", doc.id).unlink()
    # Reconcile-on-read prunes the now-orphaned row.
    docs, total = await kb.service.list_documents(scope_name="kb1", limit=50, offset=0)
    assert total == 0
    assert docs == []
...
async def test_reconversion_blocked_after_edit(kb) -> None:
    ...
    await kb.service.edit_document(
        scope_name="kb1", document_id=doc.id, new_markdown="# T\n\nhand edited", actor="user"
    )
...
async def test_changing_chunk_params_reindexes(kb) -> None:
    ...
    after = await kb.documents.count_chunks(KIND_KNOWLEDGE, "kb1")
    assert after > before


# The spec still calls this scenario "changing embedding model re-embeds", from
# when embedding was per-scope config. It no longer is — embedding resolves
...
async def test_changing_retrieval_modes_reindexes(kb, vector_config) -> None:
    ...
    await kb.create_kb("kb1", config={"retrieval_modes": ["keyword", "grep"]})
    await _ingest(kb, "kb1", "a.md", b"# Alpha\n\nthe alpha passage about deploys")
    # Keyword-only to start: a vector search degrades, flagged.
    before = await kb.service.search(scope_name="kb1", query="deploys", top_k=5, mode="vector")
    assert before.fallback == "keyword"
    # Enabling vector re-embeds the corpus through the on_update_config hook.
    await kb.resources.update_config(
        ref=ResourceRef(KIND_KNOWLEDGE, "kb1"), new_config=vector_config, actor="user"
    )
    after = await kb.service.search(scope_name="kb1", query="deploys", top_k=5, mode="vector")
...
async def test_delete_document_removes_files_and_rows(kb) -> None:
    ...
    await kb.service.delete_document(scope_name="kb1", document_id=doc.id, actor="user")
    assert not md.exists()
    assert not raw.exists()
    assert await kb.documents.count_documents(KIND_KNOWLEDGE, "kb1") == 0
    assert (await kb.service.search(scope_name="kb1", query="grape", top_k=5)).passages == ()
...
async def test_same_file_in_two_kbs_keeps_both_searchable(kb) -> None:
    ...
    assert (await kb.service.search(scope_name="kb1", query="mango", top_k=5)).passages
    assert (await kb.service.search(scope_name="kb2", query="mango", top_k=5)).passages

    await kb.service.delete_document(scope_name="kb2", document_id=doc2.id, actor="user")
    assert (await kb.service.search(scope_name="kb1", query="mango", top_k=5)).passages
    assert (await kb.service.search(scope_name="kb2", query="mango", top_k=5)).passages == ()
    assert await kb.documents.count_chunks(KIND_KNOWLEDGE, "kb1") >= 1
...
async def test_delete_kb_cleans_up(kb) -> None:
    ...
    kb_dir = paths.scope_dir("kb1")
    assert kb_dir.exists()
    from coffer.domain.resource import ResourceRef

    await kb.resources.delete(ResourceRef(KIND_KNOWLEDGE, "kb1"), actor="user")
...
async def test_delete_kb_purges_index_rows_and_vec_store(kb, vector_config) -> None:
    ...
    assert await kb.documents.count_documents(KIND_KNOWLEDGE, "kb1") == 0
    assert await kb.documents.count_chunks(KIND_KNOWLEDGE, "kb1") == 0
    ...
    assert fts_rows == 0
    vec = kb.vec_stores.get((KIND_KNOWLEDGE, "kb1"))
...
async def test_metrics(kb) -> None:
    await kb.create_kb("kb1")
    await _ingest(kb, "kb1", "a.md", b"# A\n\nalpha content here")
    m = await kb.service.metrics(scope_name="kb1")
...
async def test_document_count_uses_indexed_count_without_du_walk(kb) -> None:
    ...
    from coffer.application.knowledge import queries as kb_service_mod
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        assert await kb.service.document_count(scope_name="kb1") == 2
...
async def test_degraded_embed_surfaces_and_retries_without_rechunk(kb, vector_config) -> None:
    ...
    stored = await kb.documents.get_document(KIND_KNOWLEDGE, "kb1", doc.id)
    ...
    stats = await kb.service.reindex_documents(scope_name="kb1", actor="user")
    # Content unchanged → not counted as reindexed; the pending flag cleared.
    assert stats["degraded"] == 0

    cleared = await kb.documents.get_document(KIND_KNOWLEDGE, "kb1", doc.id)
    assert cleared is not None
    assert cleared.embed_pending is False
    assert cleared.content_sha256 == real_sha  # sha unchanged across the retry
    assert (await kb.service.metrics(scope_name="kb1"))["documents_degraded"] == 0
    ...
    res = await kb.service.search(scope_name="kb1", query="degraded", top_k=5, mode="vector")
...
async def test_reindex_rebuilds_from_files_after_index_drop(kb) -> None:
    ...
    assert (await kb.service.search(scope_name="kb1", query="phoenix", top_k=5)).passages == ()
    # Force a full rebuild (content sha unchanged, so use force path via config reindex).
    config = await kb.service.get_config("kb1")
    await kb.service.reindex_documents(scope_name="kb1", config=config, actor="user", force=True)
    assert len((await kb.service.search(scope_name="kb1", query="phoenix", top_k=5)).passages) == 1
...
async def test_concurrent_ingests_do_not_corrupt_index(kb) -> None:
    ...
    assert await kb.documents.count_documents(KIND_KNOWLEDGE, "kb1") == 8
    res = await kb.service.search(scope_name="kb1", query="shared", top_k=20)
...
async def test_reindex_reconstructs_documents_rows_after_db_loss(kb) -> None:
    ...
    restored = await kb.documents.get_document(KIND_KNOWLEDGE, "kb1", doc.id)
    ...
    res = await kb.service.search(scope_name="kb1", query="banana", top_k=5)
...
async def test_search_default_grep_mode_serves_keyword(kb) -> None:
    ...
    await _ingest(kb, "kb1", "a.md", b"# A\n\nsearchable text")
    result = await kb.service.search(scope_name="kb1", query="searchable")
    assert result.mode == "keyword"
    assert len(result.passages) == 1
...
async def test_list_documents_offset_past_total(kb) -> None:
    await kb.create_kb("kb1")
    await _ingest(kb, "kb1", "a.md", b"# A\n\nalpha")
    docs, total = await kb.service.list_documents(scope_name="kb1", limit=50, offset=10)
    assert docs == []
    assert total == 1
...
async def test_check_sources_classifies_changed_unchanged_missing(kb, tmp_path) -> None:
    ...
    report = await kb.service.check_sources(scope_name="kb1", actor="user")
...
async def test_update_from_source_updates_in_place(kb, tmp_path) -> None:
    ...
    assert len((await kb.service.search(scope_name="kb1", query="apple", top_k=5)).passages) == 1

    # The external original changes; refresh in place from it.
    src.write_text("# Report\n\nchanged banana body")
    updated = await kb.service.update_from_source(
        scope_name="kb1", document_id=doc.id, actor="user"
    )
...
async def test_update_from_source_refuses_edited(kb, tmp_path) -> None:
    ...
    doc = await _ingest_from_file(kb, "kb1", src)
    await kb.service.edit_document(
        scope_name="kb1", document_id=doc.id, new_markdown="# Doc\n\nhand edited", actor="user"
    )
    # update_from_source must refuse an edited document.
    src.write_text("# Doc\n\nbody two changed")
    with pytest.raises(ReconversionBlocked):
        await kb.service.update_from_source(scope_name="kb1", document_id=doc.id, actor="user")
...
async def test_reindex_prunes_rows_whose_file_was_removed(kb) -> None:
    ...
    assert await kb.documents.get_document(KIND_KNOWLEDGE, "kb1", gone.id) is None
    assert await kb.documents.get_document(KIND_KNOWLEDGE, "kb1", keep.id) is not None
    assert (await kb.service.search(scope_name="kb1", query="vanishing", top_k=5)).passages == ()
...
async def test_unchanged_corpus_skips_full_scan(kb) -> None:
    ...
    await kb.service.list_documents(scope_name="kb1", limit=50, offset=0)
    assert calls[0] == 1
    ...
    await kb.service.list_documents(scope_name="kb1", limit=50, offset=0)
    await kb.service.search(scope_name="kb1", query="unchanging", top_k=5)
    await kb.service.grep(scope_name="kb1", pattern="content")
    await kb.service.get_document(
        scope_name="kb1",
        document_id=(await kb.service.list_documents(scope_name="kb1", limit=1, offset=0))[0][0].id,
    )
...
async def test_out_of_band_edit_breaks_fingerprint_and_rescans(kb) -> None:
    ...
    assert len((await kb.service.search(scope_name="kb1", query="walrus", top_k=5)).passages) == 1
    ...
    assert (await kb.service.search(scope_name="kb1", query="walrus", top_k=5)).passages == ()
    assert calls[0] >= 1
    hits = (await kb.service.search(scope_name="kb1", query="dolphin", top_k=5)).passages
    assert len(hits) == 1
...
async def test_write_during_scan_is_not_masked(kb) -> None:
    ...
    await kb.service.list_documents(scope_name="kb1", limit=50, offset=0)  # warm cache
    ...
    await kb.service.search(scope_name="kb1", query="dolphin", top_k=5)
    ...
    hits = (await kb.service.search(scope_name="kb1", query="narwhal", top_k=5)).passages
    assert len(hits) == 1
