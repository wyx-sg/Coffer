...
async def test_grep_finds_cjk_substrings(kb) -> None:
    ...
    result = await kb.service.grep(scope_name="kb1", pattern="蓝绿发布")
    assert len(result.hits) == 1
    assert "蓝绿发布" in result.hits[0].line

    result2 = await kb.service.grep(scope_name="kb1", pattern="流量切换")
...
async def test_keyword_matches_isolated_cjk_phrase(kb) -> None:
    ...
    hit = await kb.service.search(scope_name="kb1", query="蓝绿发布", top_k=5)
...
async def test_cjk_fact_roundtrip_via_grep_recall(kb) -> None:
    ...
    _, markdown = await kb.service.get_document_text(scope_name="kb1", document_id=doc.id)
    assert "超时时间是 30 秒" in markdown

    result = await kb.service.grep(scope_name="kb1", pattern="超时时间")
    assert len(result.hits) == 1
...
async def test_keyword_matches_cjk_substring_inside_a_run(kb) -> None:
    ...
    result = await kb.service.search(scope_name="kb1", query="蓝绿发布", top_k=5)
