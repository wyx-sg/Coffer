...
| Chat / channels / export-import | Cross-cutting daemon features                   | In-process LangGraph chat agent plus Claude Code/Codex CLI agents (spec channels); messaging channels relaying chat from IM apps (ADR channel-adapter-framework); one-shot vault export to a directory and import of one back (ADR vault-export-import). Not kinds — they span the kinds.                                                                               |
