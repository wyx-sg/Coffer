...
Coffer is a local-first developer tool: the user's accumulated AI assets — registered MCP servers, capability preferences, audit history — must never depend on a cloud service to be readable or writable. That constraint demands a persistence layer that is self-contained, zero-configuration, and trivially backed up.

The answer is SQLite. A single file at `~/.coffer/coffer.db` is the system of record for all control-plane state. There is no separate database server to install, no connection pool to tune, no network hop between the daemon and its storage. The user's data is their file.
...
Schema evolution is managed by Alembic, configured in `backend/alembic.ini` with a single migration history under `backend/coffer/infrastructure/persistence/migrations/`. Four revisions ship with spec 001 (three table-creating revisions plus an audit index):
...
On first daemon startup, `alembic upgrade head` runs before the HTTP server accepts connections. Because Alembic migrations are bundled as data files inside the PyInstaller daemon binary, end-user installs also get correct schema creation on first launch — no separate migration step.

## Table map

The six tables that exist after applying all four revisions, grouped by purpose:
...
Either can be changed by the user via `PATCH /api/v1/retention/{table_name}` or the equivalent CLI command; the change itself is audited.
