...
| `~/.coffer/daemon-config.json` | Configuration read before the database opens: the fixed port, the idle-shutdown window, this machine's name and cached id (mode `0600`). Written by `coffer daemon port`, `coffer daemon idle`, the Settings residency panel and `coffer sync machine rename`. |
