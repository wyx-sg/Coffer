...
1. **Loopback-only binding.** The HTTP API binds exclusively to `127.0.0.1`, and it is the only socket Coffer listens on. Nothing Coffer runs is reachable from the network: channels receive by polling or over an outbound connection the daemon opens (see [Channels without a public surface](#channels-without-a-public-surface)).
...
No channel needs Coffer to be reachable from the internet. Telegram is long-polled from inside the daemon. SeaTalk arrives over **one outbound websocket connection per channel**: a worker thread inside the daemon dials out, the register handshake authenticates the connection once from the channel's app id and app secret, and the platform pushes events down the connection the daemon opened (ADR seatalk-websocket-inbound). There is no public URL, no listening port, no tunnel and no signature to verify, so the daemon's loopback socket stays the only one the vault listens on. The SeaTalk client library is operator-supplied — loaded lazily from `~/.coffer/vendor` or `$COFFER_SEATALK_SDK_DIR` — and never vendored or declared as a dependency.
...
- **Secrets stay references.** A channel's configuration carries credential refs only — a Telegram bot token ref, a SeaTalk app secret ref — resolved from the credential store when the adapter starts.
...
- **Refs without a key are reported locked.** A round or a key import names the credential refs this machine holds ciphertext for but cannot decrypt (`locked_refs`), rather than failing decryption silently (spec vault-sync "Report refs without a key as locked").
...
The check covers every surface on the daemon's port, which is every surface Coffer has: there is no second listening process to carve out.
...
| **The internal engine's LLM calls** (the curation and distil passes) | `infrastructure/llm/langchain_models.py` | No — same. |
