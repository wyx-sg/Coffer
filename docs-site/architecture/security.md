...
- **The tunnel reaches only the listener.** A tunnel points at the listener's loopback port — the managed `cloudflared` the daemon spawns for a channel that records a connector token (the token reaches it through a `0600` temp file, never argv), or one the owner runs (cloudflared/ngrok). Only that one signed callback path is reachable from the internet; the management API and MCP endpoint are not on the tunnel at all.
