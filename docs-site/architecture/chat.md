...
Attachment bytes are not chat's storage. The channel that downloaded a file keeps it under `~/.coffer/channel-media`, pruned by file age after 30 days. Chat persists only a reference (`AttachmentBlock`: `path`, `mime`, `filename`) inside the user message.
...
3. **Everything else:** Claude Code receives inline content blocks (images go to vision). Codex, which is path-native over the app-server protocol, receives a note naming the saved path so it can open the file with its own tools.

The web composer sends text only (`SendMessageRequest` has a single `text` field), so attachments arrive through channels.
...
- Decisions: [Chat Is a Single-Owner Live Mirror](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/chat-single-owner-live-mirror.md), [Channel Adapter Framework](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/channel-adapter-framework.md), [Persisted Attachment Reference](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/persisted-attachment-reference.md), [Channel Media](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/channel-media.md), [SeaTalk Inbound Over WebSocket](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/seatalk-websocket-inbound.md), [Built-in Agent Is Internal](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/builtin-agent-is-internal-capability.md), [Remove Tool Approval](https://github.com/wyx-sg/Coffer/blob/main/docs/decisions/remove-tool-approval.md)
