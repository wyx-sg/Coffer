...
不过，MCP 网关只是更广义仓库的一项能力。Coffer 是一个本地优先的 AI agent 仓库：同一个守护进程、同一个与 kind 无关的 Resource 框架，还管理着已注册的编码 agent、主技能包、共享的 knowledge 存储，以及消息渠道——共计五种资源 kind——外加横切的聊天、渠道与多机同步。下文的原则约束的是整个仓库，其中网关是奠基性的 kind，而非系统的全部。
