...
**agent 和人，问的是同一个问题。** 这些记录曾经有三个读者、三种形态：一个没人
打开的审计日志页面、一个每行都属于某个 agent 的 Invocations tab，以及一份只能在
磁盘上找出来再 grep 的守护进程日志。页面都删掉了；回来的是它们底下那个需求——
东西不对劲的时候，没有人会问「改了什么」「调了什么」或「哪里报错了」，
人问的是**发生了什么**。

因此这些记录都按这一个问题来塑形，并有两条抵达路径：

- **`coffer__diagnose`** 是 agent 的入口。一次调用同时返回审计日志（改了什么、
  谁改的）与守护进程日志（发生了什么，包括失败），放在同一条从新到旧的时间线上——
  因为撞上失败的 agent 并不知道自己需要哪一份。
- **`/activity`** 是人的入口——一个页面，一份记录一个 tab、一张表，各自带着那份记录
  真正拥有的列，任意一行都能展开成它的原始形态。跨这三份去读是工具的活儿，不是页面的：
  一张表只能展示三者的共同部分。
- **事件类型两种形态都在。** 页面把它们渲染成口语化活动行（「Enabled demo-fs」），
  两个 locale 都有译文，并由守错误码的那同一条 CI 检查守住；工具返回的则是线上原值
  （`resource_enabled`），那正是 agent 要的。
- **每个被审计的事件同时也是一行日志**，因此两份记录共用同一套词汇，
  agent grep 其中一份就能找到另一份。

`GET /api/v1/audit` 与 `coffer audit` 保持不变，供脚本使用。页面另外两条泳道读的是
`GET /api/v1/mcp/invocations`（跨服务器）与 `GET /api/v1/daemon/logs`。
...
| `agent_mcp_entry_removed`                                  | MCP 条目从 agent 自有配置文件里被删除时              |
...
- [MCP Gateway 规范参考](/zh/reference/specs/mcp-gateway/spec) — 调用日志不变量、token 鉴权和 `X-Coffer-Actor` header 语义
