...
| 聊天 / 渠道 / 导出导入          | 守护进程的横切特性                        | 进程内 LangGraph 聊天 agent 以及 Claude Code/Codex CLI agent（spec channels）；从 IM 应用中转聊天的消息渠道（ADR channel-adapter-framework）；一次性地把仓库导出到一个目录、并把这样一个目录导入回来（ADR vault-sync）。它们不是 kind——而是横跨各 kind。 |
