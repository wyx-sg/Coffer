...
| 聊天 / 渠道 / 同步          | 守护进程的横切特性                        | 运行 Claude Code/Codex agent 的回合平台（spec channels）；从 IM 应用中转聊天的消息渠道（ADR channel-adapter-framework）；仓库与用户自有 git 远端的双向收敛（ADR vault-sync）。它们不是 kind——而是横跨各 kind。 |
