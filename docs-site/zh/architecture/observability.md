...
一个开发者运行着一个繁忙的 vault：几十个 MCP 服务器、面向多个 coding agent 运行的 turn、一个已连接的消息通道——同时在进行。没有运维可观测性，诊断线上问题就会变得不透明：「为什么这个服务器突然开始返回错误？」「那个失败的请求或 agent turn 期间究竟发生了什么？」结构化日志和链路追踪关联能回答这些问题，而无需用户运行独立的监控栈。
