...
hero:
  ...
  tagline: 为你机器上的每个 AI agent 提供一个安全、共享的统一接口。工具、技能、知识只配置一次,每个 agent 看到的都是同一个保险库。一切都不离开你的机器。
  actions:
    - theme: brand
      text: 快速开始
...
features:
  ...
  - title: 技能 · 知识
    details: 维护一份主技能库并投递给每个 agent;维护唯一一处知识存储 —— 摄取的文档与 agent 写下的条目 —— 供每个 agent 检索并贡献。
  - title: 与任意 agent 对话
...
flowchart LR
  ...
  UI --> D
  IM --> D
  D["coffer-daemon<br/>保险库:MCP 网关 · 技能 · 知识 · agents · 对话"]
  D -->|带命名空间的工具| U["上游 MCP 服务器"]
  D -->|对话| L["LLM 提供方"]
  D -->|导出 / 导入| G["你自己搬运的目录"]
```

Coffer 是一个常驻的本地守护进程,持有你的保险库。MCP 网关只是其中一部分:每个 AI agent 通过一个自动发现的端点接入,看到同一批工具、技能与知识 —— 同时密钥保持加密,一切都不离开你的机器。搬到你自己的另一台机器上是一个明确的动作:把保险库导出到一个目录,搬过去,再导入。
...
# 整理一个知识集合和一个共享技能 —— 你的 agent 会自动获取。
