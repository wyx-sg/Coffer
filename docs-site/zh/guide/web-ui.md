...
- **Overview** —— 该 agent 的类型与配置目录，以及它的 LLM 连接。在内置登录下，面板会列出
  该 agent 自己的 CLI 报告的模型，每个带一个复选框。勾选你的账号实际能跑的那些，之后模型
  选择器和聊天里的 `/model` 卡片就只提供这些；一个都不勾，就全部提供——这也是全新安装时的
  行为。Coffer 无法替你判断：一个账号能跑哪些模型是服务端事实，本机没有副本，而 CLI 的清单
  会列出已安装版本曾经知道的每一个模型。CLI 自己标记为已退役的模型，在你看到之前就已被剔除。
