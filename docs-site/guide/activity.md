...
Two things about this file are worth knowing before you read it. First, it is **not one format**: Coffer's own structlog writes JSON, but alembic, uvicorn, every stdio upstream that writes to the daemon's stderr, and the cloudflared child a tunnel spawns all write their own shapes into the same file. `application/log_reader.py` normalises all of them onto `timestamp` / `level` / `logger` / `event`, strips ANSI escapes, and keeps a line that fits no format whole rather than dropping it. Second, a traceback is folded into the record that raised it instead of becoming rows of its own, and a line that carries no time of its own borrows the timestamp of the record beside it — or renders a dash, rather than claiming a time it does not have.
