...
The remote is probed before it is accepted, so a typo or a token that cannot push fails here rather than an hour later. Re-running `remote set` changes what it names and keeps whether sync is switched on: a paused remote stays paused. `coffer sync remote show` prints what is configured; `coffer sync remote clear` forgets it and leaves the vault exactly as it is.
