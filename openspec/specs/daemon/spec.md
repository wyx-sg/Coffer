...
start time, the build's version and the executable answering. It MAY carry a count of registered,
...
`rotate-token`, and `port show|set|clear`. `start` MUST key off the liveness probe rather than the
...
`start`, and is how a setting read before the bind takes effect.
