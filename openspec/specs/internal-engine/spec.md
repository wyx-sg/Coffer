...
switch and timer of every pass Coffer runs when nobody asked it to, the bound on
one call to that model, and the speech-to-text model. It owns one global
...
speech-to-text model, the bound on one model call, and each unattended pass's
switch and optional interval, plus the timestamp of the last write — in one
global row whose primary key is fixed, with a database constraint making a
second row unrepresentable whatever writes it. `NULL` means "the built-in
default" for an interval and for the bound, and "unchosen" for a model.
...
bound, the speech-to-text model and every pass's switch and interval — Coffer's
passes behave the same everywhere only when they run on the same model, and
