...
Two disclosed Bot API parsing caveats are accepted and documented at their call
sites rather than specified away: mention entity offsets are matched against
plain code-point indices even though Telegram's own offsets are UTF-16 code
units, which drifts only when a surrogate-pair character precedes the mention in
the same message; and only `entities` on a plain text message is parsed for a
mention, so an @mention inside a media caption is not recognized.
...
title element of its own. Every callback value Coffer emits fits the **64-byte
callback budget** the Bot API allows; a choice whose value would not fit is left
off the card.
