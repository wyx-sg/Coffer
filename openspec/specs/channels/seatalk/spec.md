...
chunked at **4096 bytes**. Headings become bold and links become `label (url)`,
...
`send_media` MUST be wired to SeaTalk's **file-upload API** and the adapter
declares `supports_media` true, so an agent's `MEDIA:` sentinel returns a file
into the same chat **and thread** the turn came from ([channels](../spec.md) "Return outbound media into the originating thread"). An
