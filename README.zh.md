...
- **知识 (Knowledge)** —— 一个统一的知识层，按项目分作用域（也可以是 global，或你自己命名的集合）。agent 经 MCP 往里写笔记；你则放入任意格式的文档（自动转换为 markdown，原件保留作出处）。一趟周期性的整理流程会把重复的笔记合并成连贯的主题文档，并归档它替换掉的每一个版本。一次检索横跨全部内容 —— grep / 关键词 / 向量由引擎自行决定，不需要调用方回答这个问题。
