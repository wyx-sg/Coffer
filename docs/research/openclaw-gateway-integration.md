...
>
> **The conclusion is therefore reversed: openclaw IS a viable managed leaf agent.** It SHIPPED as one — [ADR-043](../decisions/ADR-043-openclaw-managed-agent.md) records the delivered slice, with every capability cell probe-verified against a real install (openclaw 2026.6.11). Two things this document got right and that still matter: openclaw drives Claude Code / Codex / OpenCode as sub-workers, so Coffer's writes to `~/.claude/` are inherited by openclaw-spawned workers; and its headless CLI (`openclaw agent --message … --json --local`) returns one JSON blob with **no streaming**, so it would be Coffer's only non-streaming chat provider.
>
