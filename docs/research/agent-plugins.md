...
- **The Codex "toggle + uninstall" capability is real and primary-sourced.** Backed by Coffer's own descriptor (`PluginModel.CODEX`, `can_toggle=True`, `can_uninstall=True` — `repo:backend/coffer/domain/agent/descriptor.py:259-264`) and FR-026 (`repo:specs/agent-registry/spec.md:191`, `:629`), and externally by OpenAI's docs: disable via `enabled = false` in `~/.codex/config.toml`, uninstall via the plugin browser / `codex plugin` CLI. (The Coffer half of that backing no longer exists — the descriptor's `can_toggle` / `can_uninstall` fields were deleted on 2026-09-10 along with the toggle and uninstall writes; the OpenAI-docs half still holds, so Codex's own capability is unaffected.) https://developers.openai.com/codex/plugins
