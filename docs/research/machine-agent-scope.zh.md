...
> （[Vault Sync](../../specs/vault-sync/spec.zh.md)）；机器轴也曾随它短暂回归，
> 形态是两条相互独立的允许列表，而不是本笔记设计的那个矩阵。它在
> **2026-09-14 再次被撤回**，而这一次，本笔记当初要探究的那个问题是被了结了、
> 而不是被搁置了：一个资源的**生效范围 (reach)**——它的 `enabled` 标志与它的
> `scope` 合在一起——被定为**本机专属**，在它生效的那台机器上设定，绝不同步
> （[Per-Agent Resource Scope](../decisions/per-agent-resource-scope.zh.md)）。
> 「只在台式机上」如今是由*这个设置住在哪里*来回答的，而不是由一条轴来回答，
> 因此 `scope` 只点名 agent，别的什么都不点。当前形态请以那份 ADR 为准；下文
> 保留的是机器轴（连同矩阵）当初是怎么得出来的记录。
