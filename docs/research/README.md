...
| --- | ------------------------------------------------------------------------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | [MCP ecosystem](./mcp-ecosystem.md)                                             | MCP gateway (001)        | Coffer is the only gateway with **no per-client scoping** — every rival curates per-client.                                                                           |
| 2   | [Agent config & rules mgmt](./agent-config-management.md)             | Agent registry (004)     | Coffer is structurally ahead (bi-directional, safe-edit) but **doesn't distribute instructions** — the category's core feature.                                       |
| 3   | [Agent skills](./agent-skills.md)                                                | Skill manager (005)      | Cross-agent delivery + SSRF-hardened ingest lead; **no skill scanner / standard-alignment** is the gap.                                                               |
| 4   | [Agent plugins](./agent-plugins.md)                                             | Plugin facet (004)       | "Visibility + safe toggle, no install" is **validated**; the **plugin→hub bridge** is the novel move.                                                                 |
| 5   | [Local-first control plane](./local-first-control-plane.md)         | Whole-product            | "Vault for **all** assets" is unique (rivals are MCP-only); borrow **upstream sandboxing** + **profiles**.                                                            |
| 6   | [Knowledge base / RAG](./knowledge-base-rag.md)                            | KB (006)                 | Files-as-truth + reindex beats locked-embedding rivals; borrow **RRF fusion + reranking**.                                                                            |
| 7   | [Messaging channels](./messaging-channels.md)                              | Channels (009)           | Channel-as-resource + stealth pairing is distinctive (Anthropic's TG plugin is the only analog); borrow **per-conversation binding**.                                 |
| 8   | [Multi-machine sync](./multi-machine-sync.md)                              | Sync (010)               | **Reconcile-not-overwrite** is unique; borrow chezmoi's **per-machine templating**.                                                                                   |
| 9   | [Credentials & secrets](./credentials-secrets.md)                         | Credentials (015)        | Coffer already implements 1Password's "**access without exposure**" ideal; borrow **external-provider refs** (`op://`).                                               |
| 10  | [Observability, audit & MCP security](./observability-governance.md) | Audit/retention/security | Governance/audit is **ahead** (free vs Langfuse's paywall); borrow **MCP tool-pinning** (mcp-scan).                                                                   |
| 11  | [Agent evaluation](./agent-evaluation.md)                                    | Eval (flywheel / harness) | **Correction: the flywheel shipped** (the eval-flywheel ADR is accepted, `evals/` exists); borrow **LLM-as-judge**.                                                   |
| 12  | [Memory systems](./memory-systems-landscape.md)                      | Memory (007)             | _Parallel session._ Field splits file-truth vs vector-truth; Coffer is a deliberate hybrid.                                                                           |
| 13  | [Platform-level competitors](./platform-competitors.md)                  | Whole-product (re-sweep) | **Blind-spot fix (2026-06-20):** the prior 12 reports missed cc-switch (~105k) + the whole Chinese ecosystem; no incumbent covers Coffer's full 10-capability matrix. |
