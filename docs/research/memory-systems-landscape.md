...
> A fresh fact-check pass against the live sources and Coffer's own code/spec. The web claims hold up; mem0's Neo4j removal is more complete than the report stated, and three Camp-A competitors are now characterized. **The big local change:** Coffer's native-projection layer — the precise mechanism the report names as its part-3 novelty — was **removed** (PR #110 / [Aggregate Agent Memory](../decisions/aggregate-agent-memory-never-write-it.md), 2026-06-18), so the projection bullets below were rewritten to current reality and the novelty assessment revised: the fan-out is no longer something Coffer itself does.
>
> **2026-09-12 update.** The bullets below describe the design in force between 2026-06-18 and 2026-09-12 and are kept as that record. Coffer no longer keeps a canonical store agents write into: it now aggregates each agent's own native memory read-only and pushes a budgeted digest back at session start ([Aggregate Agent Memory](../decisions/aggregate-agent-memory-never-write-it.md), spec [memory](../../openspec/specs/memory/spec.md)). The prohibition on writing native memory files is unchanged; the tool list named below is not — it is now a single `coffer__recall`.

### ✅ Confirmed

- **Coffer's projection consistency semantic (Open Question 3) is now N/A — there is no projection.** As of [Aggregate Agent Memory](../decisions/aggregate-agent-memory-never-write-it.md) (2026-06-18, supersedes [One Shared Knowledge Store](../decisions/agent-native-shared-memory.md)), Coffer **no longer projects memory into any agent's native files**. The whole projection layer — the `AgentMemoryAdapter` protocol with `projection_mode ∈ {SYMLINK, RENDER, NONE}`, the projection engine, per-agent adapters, the FS adapter, the binding persistence/table (dropped by migration `0024`), the projection REST routes, and the native-memory take-over/disable path — was deleted (`projection_service.py` and `new_agent_adapters.py` no longer exist). Coffer keeps its own per-fact-markdown canonical store and **never writes, symlinks into, or disables an agent's native memory**. Agents read and write Coffer memory **only through the MCP gateway tools** (`coffer__recall` / `remember` / `update_memory` / `forget` / `list_memory`); scope (global + per-project, git-root) is resolved from the MCP shim's cwd. So the "one-way render vs two-way sync, do local edits flow back" question dissolves: there is no native copy to flow back from. (An ambient session-start _injection_ hook — index into context, never a file write — is a deferred Memory via MCP follow-up, **not yet built**; until it lands, MCP `recall` is the sole access path.) `repo:docs/decisions/aggregate-agent-memory-never-write-it.md`, `repo:openspec/specs/knowledge/spec.md`, `repo:backend/coffer/application/memory/recall.py`, `repo:backend/coffer/application/memory/builtin_tools.py`
- **Conflict resolution at projection — no longer applicable.** With native files untouched there is no second writer and nothing to disable; the canonical store is trivially the sole store. Coffer's per-fact markdown is still **direct-write, no LLM at write, no automatic dedup/merge — human-curated** (Camp-B), reached over MCP. The old One Shared Knowledge Store take-over-and-merge-then-symlink behaviour described in the previous draft is gone with the projection layer. `repo:backend/coffer/application/memory/writes.py`, `repo:openspec/specs/knowledge/spec.md` (US-4 curation)
- **rohitg00/agentmemory's only native-file-writing bridge is single-target (Claude-only)** — "Bi-directional sync with MEMORY.md" is documented only for Claude Code (gated by `CLAUDE_MEMORY_BRIDGE`); other agents get MCP/skill access, not native-file writes. The report's single-target framing stands. https://github.com/rohitg00/agentmemory/blob/main/README.md
- **No surveyed system does native multi-target fan-out projection** — the _field_ finding survives this pass (though, per ✏️ Corrected below, Coffer no longer does it either, so the design point is now unoccupied by anyone including Coffer). Letta Context Repositories `/init` learns one-way from existing Claude Code AND Codex histories, but memory then lives in Letta's own git-backed MemFS repo (internal, per-agent via `/memfs`); no continuous write-back to external agents' native files. https://www.letta.com/blog/context-repositories/
- **Zep/Graphiti remains a central temporal-knowledge-graph store over MCP/API** — architecturally the opposite of native fan-out. https://help.getzep.com/graphiti/getting-started/mcp-server

### ✏️ Corrected

- **The projection model this report calls Coffer's novelty was superseded and dropped (PR #110 / [Aggregate Agent Memory](../decisions/aggregate-agent-memory-never-write-it.md), 2026-06-18).** The earlier draft of this update — and the report body above it — described Coffer's multi-target native fan-out (symlink where the format matches, managed block elsewhere, the agent's own memory disabled to prevent divergence) against One Shared Knowledge Store. **Memory via MCP supersedes One Shared Knowledge Store and removes that layer entirely:** Coffer now keeps its own per-fact-markdown store and reaches every agent through MCP gateway tools, never touching native memory files (`repo:docs/decisions/aggregate-agent-memory-never-write-it.md`, `repo:openspec/specs/knowledge/spec.md`). The ADR cites _this very research's_ yellow flag — that the whole field deliberately avoids writing native files (Letta's `claude-subconscious` refuses to touch `CLAUDE.md`) — as a reason for the reversal. **Honest bottom line for the report's thesis:** the part-3 "multi-target native fan-out is novel / nobody does it" finding _about the field_ still holds — no surveyed system does it. But it is **no longer something Coffer does**, so the novelty is now moot _for Coffer_: Coffer has joined the industry-default central-store-plus-tool-access (MCP) camp it was previously distinguished from. The fan-out remains an unoccupied design point; Coffer simply chose, on legibility/intrusiveness grounds, not to occupy it.
- **mem0 Neo4j removal understated.** Old: "OSS v2→v3 deprecated standalone Neo4j graph storage." Corrected: external graph stores (Neo4j et al.) were **removed entirely in v3 (~4000 LOC deleted, PR #4805, ~Apr 2026)**, replaced by built-in entity-linking graph memory with no external graph DB. v3 retrieval is multi-signal hybrid: semantic (sole candidate generator) + BM25 (boost only) + entity matching, fused into one score; `agent_id` scoping now lives inside a `filters` dict aligning OSS with the Platform API. https://docs.mem0.ai/migration/oss-v2-to-v3
- **rohitg00/agentmemory background note understated its reach.** The report frames it as a small central server; it now also fans out **SKILL shims** (not memory content) into 50+ agents' native skill dirs (Claude Code, Cursor, Codex, Gemini CLI, OpenCode, Windsurf, etc.). This is skill delivery, not memory native-file projection — so it does not affect the fan-out novelty claim. https://github.com/rohitg00/agentmemory/blob/main/README.md

### ➕ Coverage added

- **Cognee** — memory-first knowledge-graph engine: builds a self-hosted KG (subject-relation-object triplets) from data via embeddings + graph extraction + ontology generation; vector search hints into graph triplets, then traverses the graph for structured context. Custom Graph Model = domain schema for extraction. Multi-backend (Neo4j/FalkorDB/KuzuDB/NetworkX graphs; Redis/Qdrant/Weaviate vectors; SQLite/Postgres metadata). **Camp A** — graph-as-truth, LLM/ontology at write, central store, no native-file projection. https://github.com/topoteretes/cognee
- **LangMem (LangChain SDK)** — gives LangGraph agents long-term memory in three human-cognition types: semantic (collections + profiles), episodic (few-shot examples / conversation summaries), procedural (rules/behaviors). Backend-agnostic store (vector DB/KV/Postgres). Automatic consolidation merges related facts and resolves contradictions (LLM at write time). **Camp A** — no native-file projection. https://www.langchain.com/blog/langmem-sdk-launch
- **LlamaIndex memory module** — a `Memory` class = short-term (last X messages within a token limit) + long-term as Memory Block objects flushed from short-term: StaticMemoryBlock, FactExtractionMemoryBlock (LLM extraction), VectorMemoryBlock (vector-DB store/retrieve of message batches); priority-based truncation. **Camp A** — session/agent-scoped conversation buffer + vector store, no cross-agent sharing, no native-file projection. https://developers.llamaindex.ai/python/framework/module_guides/deploying/agents/memory/

All three reinforce the report's "Camp A = central store, LLM/extraction at write, vector/graph retrieval, no native projection" thesis, so adding them does not weaken Coffer's part-3 novelty claim.
...
- **This is a placeholder, not a verdict.** The knowledge and memory design is
  ...
  not standing on it at the moment. Re-check `docs/architecture.md`
