...
1. **无 MCP 工具定义完整性 / Tool Pinning（头号安全借鉴）。** mcp-scan 对工具定义做哈希以检测
   ...
   实时查询其能力（ADR capability-state-model）**——它正好处在能在网关哈希 + pin 工具定义并对漂移告警的位置。这是最
   契合使命、最该加的安全特性。
2. **无 opt-in 的参数/结果捕获用于调试。** 这个不变量作为*默认*是对的，但每个调试流程都需要逃生口。
   借鉴 OTel 模型：opt-in、默认关闭、仅本地的参数/结果捕获。
3. **无 LLM 层可观测。** Coffer 不追踪其内部模型（ADR builtin-agent-is-internal-capability）调用——token/成本/延迟。若要加追踪，
...
2. **加入 MCP 工具定义 pinning（mcp-scan 风格）** —— 在网关哈希 + pin 上游工具定义，对 rug-pull/
   静默改定义告警，扫描工具描述的注入。你的网关 + 能力追踪（ADR capability-state-model）让这成为自然、高价值的补充；威胁真实
