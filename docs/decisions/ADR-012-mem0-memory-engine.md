# ADR-012: Memory Engine — mem0 Behind an Application Port

**Status**: Accepted
**Date**: 2026-05-29
**Deciders**: Yuxing Wu
**Related**: spec `007-memory` (FR-011, FR-012, FR-014, SC-006, research.md §1 §2), [ADR-002](ADR-002-code-layout-layer-first.md), [ADR-008](ADR-008-everything-is-a-resource-kind.md), [ADR-010](ADR-010-llamaindex-rag-engine.md), [ADR-011](ADR-011-observability-tracer-port.md)
...
- Memory becomes the second consumer of the `Tracer` port introduced
  by spec 006 (ADR-011), satisfying the constitution's "extract
  cross-cutting after the second feature" rule and validating the
  port's shape.
