# 跨平台 skill 投递 —— Symlink / Junction / Copy-Fallback

> English: [cross-platform-skill-delivery.md](./cross-platform-skill-delivery.md)

**Status**: Accepted
**Date**: 2026-05-29
**Deciders**: Yuxing Wu
**Related**: spec `skill-manager`（FR-009、FR-012、SC-007），spec `agent-registry`（`AgentConfig.skill_dir`），[Everything Is a Resource Kind](everything-is-a-resource-kind.md)

## Context

Spec `skill-manager` 在 `~/.coffer/skills/<name>/` 维护一份规范主副本，并要求每个已注册的 agent 在它"自身的"skill 目录下能看到这些被管理的 skill —— 也就是 `~/.claude/skills/<name>/`、`~/.codex/skills/<name>/` 等等。这份主副本必须是**唯一可编辑的事实来源**（FR-003），同时每个 agent 仍按其既有约定从自己的路径下读取。
...
模式按 binding 而非按 OS 决定 —— 同一台用户机器上完全可能混用文件系统（如 NTFS C: 盘加上一个 SMB 挂载的 skill 目录）。`SyncEngine.classify_target` 在 `verify` 时重新读取磁盘形态，是 drift 分类的依据，详见 `specs/skill-manager/data-model.md` 中的 `DriftKind` 表。
