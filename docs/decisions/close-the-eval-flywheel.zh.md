# 闭合 Eval 飞轮（Loop Engineering）

**状态**：Accepted
**日期**：2026-06-14
**决策者**：Yuxing Wu
**相关**：[Harness in Layers](industrial-grade-harness-in-layers.zh.md)（分层 harness）、[Tool Retrieval](tool-retrieval-for-overload.zh.md)（工具检索）、`evals/README.md`、`.specify/memory/roadmap.md`

## 背景

[Harness in Layers](industrial-grade-harness-in-layers.zh.md) 分层搭建了 harness，并把 **loop engineering** 锚定为它**之上**的一层：
...
那份 ADR 的 Layer D 交付了 `evals/`，但它只是个**回归仪表**：每个 suite ~8 条
...
产品 spec——豁免 FE+BE+真实使用的 spec-deliverable 规则，与 Harness in Layers 一致）。
按**有序、可独立交付的切片**推进：
...
- **附带（可并行）。** Harness in Layers 承诺过但没建的 verify-before-commit hook；
...
- **把 loop engineering 建模成编号产品 spec。** 否决：它和 Harness in Layers 一样是工程
