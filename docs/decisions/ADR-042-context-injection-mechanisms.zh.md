...
> **呈现方式于 2026-07-10 修订。**「显式说明、而非静默隐藏」适用于 agent **自己的**
> 详情页：每个缺席 facet（连接、插件、对话记录）渲染统一、中性的「不支持」状态——
> 绝不是一张看似可用的空表格或一行原始报错。跨资源的选择器（连接表单的兼容
> Agent 勾选框）则改为**直接省略**永远不具备该能力的 agent，不再以禁用态列出：
> 在无关资源的表单上枚举其他产品的局限只是噪音，原因已经写在用户查看该 agent
> 的地方。矩阵以 `AgentOut.capabilities` 机器可读（spec 004 FR-003a）。
