# ADR-008: 分发 —— 用 PyInstaller 打包 daemon、shim 与 CLI

> English: [ADR-008-distribution-pyinstaller.md](./ADR-008-distribution-pyinstaller.md)

**Status**: 已采纳 (Accepted)
**Date**: 2026-05-20（2026-09-09 修订，见「修订历史」）
**Deciders**: Yuxing Wu
**Related**: spec `001-mcp-gateway` (FR-022, SC-009), [ADR-005](ADR-005-session-subprocess-model.md), [ADR-006](ADR-006-daemon-detect-or-spawn.md)

## 背景

Coffer 有三个可运行入口：长生命周期的 `coffer-daemon`、按 MCP 客户端会话
拉起的短生命周期 `coffer-mcp-shim`，以及 `coffer` 管理 CLI。项目的目标
用户群包含没有系统 Python 安装的用户。规范 `001-mcp-gateway` 明确承诺了
这一点：
...
- daemon 同时以静态文件形式在自己的 loopback origin 上托管构建产物 Web UI
  （spec 001 FR-024），因此 Web 资源随 daemon 二进制一同分发，而不再依赖
...
- **2026-05-20** —— 初版决定（spec 001 时期）：PyInstaller 二进制、
...
- **2026-06-12** —— 二进制部署改为把 shim 与 daemon **两份**都发到用户
  ...
  ADR-006 同目录探测在重启后能找到可自启的 daemon。
- **2026-09-09** —— **桌面外壳被移除，本 ADR 收敛为纯 PyInstaller 分发
  ...
  绕行方式改为在上文内联说明。
  spec 003（MCP Gateway Desktop）退役；其中仅存的两条发布流水线需求并入
  spec 001。
