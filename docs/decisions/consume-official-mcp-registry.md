...
- **Date:** 2026-06-19
- **Deciders:** Yuxing Wu
- **Spec:** mcp-gateway (spec.md updated before implementation)
- **Related:** [Capability State Model](./capability-state-model.md) (list live-queried, not mirrored), [Envelope-Encrypted Credentials](./envelope-encrypted-credential-store.md) (secrets become credential refs)
...
4. **Secrets via credential refs, never inline.** An `environmentVariables[]`
   ...
   inline value, so the autofill path inherits the [Envelope-Encrypted Credentials](./envelope-encrypted-credential-store.md)
