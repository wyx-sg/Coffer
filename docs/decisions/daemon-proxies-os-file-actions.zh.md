# 本地 daemon 代理 OS 文件动作

> English: [daemon-proxies-os-file-actions.md](./daemon-proxies-os-file-actions.md)

- **状态:** Accepted
- **日期:** 2026-06-21
- **决策者:** Yuxing Wu
- **Spec:** [agent-registry](../../specs/agent-registry/spec.zh.md) 拥有 `/fs` 路由与共享 FileActions 操作栏(FR-038、新增 FR-039);并改动 [skill-manager](../../specs/skill-manager/spec.zh.md) FR-006 / 新增 FR-030，以及 [knowledge](../../specs/knowledge/spec.zh.md) FR-020 / FR-021（知识层 spec，现同时拥有文档与条目两个界面）中的 open/reveal 回退条款 —— 不新增 spec;`spec.md` 在实现前先更新。
- **取代:** agent-registry FR-009/FR-038、skill-manager FR-006、已退役的 Knowledge Base 规范的 FR-020、knowledge FR-021 中"web 端 open/reveal 回退到 copy-path"的立场。

## 背景

Coffer 的文件查看器是**只读**的(ui-shell、agent-registry、skill-manager、Knowledge Base 与知识层这几份 spec):用户在自己的编辑器里改,经由共享的 `FileActions` 操作栏把受管文件(或其所在文件夹)带到 OS —— **在外部编辑器打开**、**在文件管理器中显示**、**复制绝对路径**。
...
- `POST /api/v1/fs/open` `{ path, with? }` —— 在某个应用里打开 `path`。`with` 是首选编辑器偏好(ui-shell);缺省时使用 OS 默认应用。同时服务于"在编辑器打开文件"和"在编辑器打开文件夹"。
...
本 ADR 统一了 open/reveal 但没统一*选取*:当时 OS 原生目录对话框只在打包的原生外壳里可用,所以浏览器界面继续用内嵌的 daemon 文件夹浏览器。那个原生外壳已不复存在,而 daemon 现在自己会打开宿主的原生**目录**对话框(spec agent-registry FR-042)。它不代理选文件与存文件对话框:浏览器自带的 `<input type="file">` 与 `<a download>` 已经覆盖,且前者更好——它直接交出文件**内容**,而不是交一个还要 daemon 再去读的路径。文件夹是那个例外,因为浏览器刻意不给出绝对路径。
