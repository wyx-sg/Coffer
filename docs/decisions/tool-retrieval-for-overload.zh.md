...
**状态**：Accepted
**修订自**：[Built-in Agent Is Internal](builtin-agent-is-internal-capability.zh.md)（2026-06-14）—— `coffer__search_tools` 获得带 BM25 回退的语义（嵌入）排序路径。
**修订自**：[Budget-Driven Tool Tiering](budget-driven-tool-tiering.zh.md)（2026-09-09）—— 「服务端不隐藏任何东西 / tool deferral 是客户端的选择」条款由预算驱动的工具分层取代；`coffer__search_tools` 仍检索完整目录，未列出的工具仍可调用。
**日期**：2026-06-14
**决策者**：Yuxing Wu
**关联**：`.specify/memory/constitution.md`、[spec `mcp-gateway`](../../specs/mcp-gateway/spec.zh.md)、[Everything Is a Resource Kind](everything-is-a-resource-kind.zh.md)、[Files as Truth](files-as-truth-sqlite-retrieval.zh.md)

## 背景

Coffer 是一个 local-first 的 MCP 聚合器：它把许多上游 MCP 服务器的工具合并起来，作为一个带命名空间的统一工具面，重新暴露给一个 coding agent（Claude Code / Codex）。这正是 spec mcp-gateway 的全部要点——注册一次，处处可用。但这种聚合有一个随成功而增长的失败模式：一旦用户注册了许多服务器，合并后的目录动辄超过 **150 个工具**，而一个 coding agent 的工具选择准确率在目录大约超过 **30–50 个工具**之后就会急剧下降。
...
- **对实时聚合目录排序。** 它把当前聚合的上游工具按意图 `query` 打分并返回 top-k。agent 随后用返回工具已有的 `<server>__<tool>` 名称直接调用；路由与 spec mcp-gateway 完全一致。
...
这次调用像任何其他网关调用一样被记入 invocation 日志（谁 / 何时 / 耗时多久 / 结果如何，不含参数和返回内容——遵循 spec mcp-gateway）。
...
- **仅靠静态命名空间 / 手动 curation。** 否决为不够：前缀和按工具的启用/禁用（spec mcp-gateway）已存在且有帮助，但一个精挑过的集合仍可能超过 30–50 工具悬崖，且要求用户为每个任务手工修剪并不可扩展。检索是 curation 做不到的、按请求级的答案。
