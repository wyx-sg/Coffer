...
[Harness in Layers](industrial-grade-harness-in-layers.md) built the harness in
layers and anchored **loop engineering** as the project *above* it: the harness
decides what the agent CAN do (static — tools, context, guardrails); the loop decides what it DOES NEXT and WHEN IT STOPS
...
behaviour — tool-routing, retrieval, chat. That ADR's Layer D shipped `evals/` as
...
FE+BE+real-usage spec-deliverable rule, like Harness in Layers). Build it in
...
- **Model loop engineering as a numbered product spec.** Rejected: it is
  engineering tooling like Harness in Layers, not a user-facing increment; an ADR
  with ordered slices fits.
