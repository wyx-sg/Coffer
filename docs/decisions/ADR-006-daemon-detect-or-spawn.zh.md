...
- Tauri 桌面应用 —— 打开后期望管理 daemon 状态。
- `coffer-mcp-shim` —— 每次 MCP 客户端 (Claude Code、Cursor) 启动时由其拉起。
- `coffer …` CLI —— 由用户临时调用。

daemon 必须**比任一单一入口活得更久**：用户期望关闭桌面应用窗口后 MCP 客户端
仍能继续通过 shim 工作，也期望某个 MCP 客户端的 shim 不会因为另一个客户端的
shim 退出而死掉。
...
- Tauri 应用、shim 和 CLI 都使用同一个 `detect-or-spawn` 辅助函数：
...
- daemon 在 Tauri 窗口关闭后仍然存活。Close-to-tray 可行（窗口隐藏，daemon
  保留）；即使 GUI 没打开，基于 shim 的 MCP 客户端依然工作。
- 不需要任何特权安装。建立方式是「打开一次应用」或「执行任意一次 `coffer`
  命令」。
...
- 未来某个规约可能增加可选的 `coffer daemon install --system`，供希望用
  launchd / systemd / Windows service 监管的用户使用；这是本 ADR 的严格超集。

## 备选方案

**手动 daemon（用户在做任何事之前先 `coffer daemon start`）**。被否决。
体验糟糕：强制用户在每次与 MCP 客户端交互前记住一个准备步骤。违背了桌面
应用的承诺。

**Tauri 拥有 daemon（Tauri 退出则 daemon 死亡）**。被否决。这样 shim 会在
用户关闭 GUI 时损坏。同时支持 GUI 和 shim 的核心意义正在于：它们是同一份
长生存周期状态之上两个互相独立的入口。

**以系统服务安装作为主要拉起方式（launchd / systemd / Windows service）**。
当前否决。需要更高权限、按平台编写的安装脚本，以及更复杂的卸载流程。
作为可选项推迟到后续规约。当前模式与之前向兼容：系统服务支持落地后，
detect-or-spawn 辅助函数依旧适用，只是每次都会发现一个已在运行的 daemon。
