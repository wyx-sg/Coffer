...
- **取代：** [ADR-033](./ADR-033-daemon-proxies-os-file-actions.md) §4 的
  「不把选择器收敛到 daemon」表态。该表态在 ADR-033 之后
  发布的 `POST /fs/pick-folder` daemon 端点中其实已被推翻；本 ADR 把这个新方向
  写明，并把它从「文件夹」扩展到「文件」。

## 背景

ADR-033 把 OS 文件**动作**（打开 / 在文件管理器中显示）经环回 daemon 代理，让浏览器
承载的界面执行真正的 OS 动作；但 §4 刻意把**选择**留在 daemon 之外：界面使用应用内的
daemon 文件夹浏览器，因为当时 OS 原生目录对话框只能从打包的原生外壳里调起。其后的一次
改动（`POST /fs/pick-folder`、`FsPickService`）就文件夹而言已经打破了这一表态——daemon
现在会打开宿主的**原生**目录对话框（macOS 用 `osascript`，Linux 用 `zenity`/`kdialog`），
仅当宿主没有原生对话框工具时才退回应用内浏览器。

仍有一个缺口：**没有原生的「选文件」或「存文件」对话框**，只有「选文件夹」。因此
每个需要*文件*路径的地方，都只能强迫用户把绝对路径打进文本框。最显眼的是「带外」主
密钥卡片（spec 010）：它显示一个原始的 `/path/to/coffer-master.key` 输入框，导入/导出
按钮就对用户手打的内容操作。手打绝对路径，正是当初引入文件夹选择器要消除的摩擦。
...
> **2026-09-09：** 本 ADR 曾描述的另一条分支——打包桌面外壳内的 Tauri `open`/`save`
> 对话框——已随该外壳一同移除。daemon 端点是唯一机制。
...
`pickSaveFile(suggestedName, start?)`，各自返回 `{ path, unavailable }`，两者都由新的
daemon 端点支撑。仅当 daemon 报告无原生工具（或调用出错）时 `unavailable` 才为真，
调用方据此显示手输回退。

### 3. 选择全面原生优先；手输仅作最后回退

主密钥卡片默认不再显示路径框。导入打开原生「选文件」对话框，导出打开原生「存
文件」对话框。手输的 `keyPath` 框**仅在**某次选择返回 `unavailable` 后才出现——在
macOS 上永不出现。既有三处「文件夹 + 文本框」（技能导入、Agent `config_dir` ×2）
去掉并排的原始文本框，改为只读显示所选路径 + `FolderPicker` 按钮（其自身的回退是
应用内浏览器，从不要求打字）。

## 影响

- 界面选文件的方式与它已有的选文件夹方式一致。手输路径仅在没有原生对话框的宿主上
  作为降级回退保留。
- daemon 新增的 OS 表面很小、有界：对话框只*返回*用户选定的路径——不创建、不打开
  任何东西。信任级别与既有的 `pick-folder` / `browse` 路由相同（环回 + token +
  参数向量调用）。
- ADR-033 §4 的「刻意双路径」就此退役。选择器对文件夹与文件都已是 daemon 原生；
  应用内文件夹浏览器仅作为文件夹的「无原生工具」回退保留。
