# Memory via MCP, not native projection (supersedes One Shared Knowledge Store)

> 中文版: [memory-via-mcp-not-native-projection.zh.md](memory-via-mcp-not-native-projection.zh.md)

**Status**: Accepted
**Date**: 2026-06-18
**Deciders**: Yuxing Wu
**Related**: spec `knowledge`, supersedes [One Shared Knowledge Store](agent-native-shared-memory.md); builds on [Files as Truth](files-as-truth-sqlite-retrieval.md); informed by [`docs/research/memory-systems-landscape.md`](../research/memory-systems-landscape.md)

## Context

[One Shared Knowledge Store](agent-native-shared-memory.md) made Coffer's one canonical
...
- **Kept (unchanged):** files-as-truth per-fact markdown + regenerated
  `MEMORY.md` ([Files as Truth](files-as-truth-sqlite-retrieval.md)); the
...
- **Ambient loading (~~deferred follow-up~~ — SHIPPED, spec knowledge FR-055):** the
  ...
  (`ContextInjectionSpec`, spec agent-registry FR-043) for every agent that has one
...
- **No divergence-disabling.** We no longer flip an agent's config to turn off
  its native memory — a reversible-but-invasive step One Shared Knowledge Store itself flagged.
...
- **Loses ambient auto-load until the injection hook lands.** With MCP-only
  ...
  context for free at session start. This was that ADR's core reason for
...
**Keep native projection (ADR agent-native-shared-memory).** Rejected: the legibility and
...
- **2026-09-10** — Vocabulary only. `memory` and `knowledge_base` merged into a
  ...
  [Files as Truth](files-as-truth-sqlite-retrieval.md)'s revision history), so
