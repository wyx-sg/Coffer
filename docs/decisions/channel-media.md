...
**Date**: 2026-07-04
**Deciders**: Yuxing Wu
**Related**: [channels](../../openspec/specs/channels/spec.md) "Hand inbound photos and files to the agent"; builds on [Channel Adapter Framework](channel-adapter-framework.md) (channel adapter framework); superseded in part by [Persisted Attachment Reference](persisted-attachment-reference.md)
...
- The design is modality-agnostic: audio, zip, etc. are the same reference; a new
  ...
  the path), keyed on the agent's capability — this is the seam [channels](../../openspec/specs/channels/spec.md)
  "Transcribe inbound voice only when the user opted in" builds on.
