...
The retired Agent Chat spec gave the chat platform a per-tool **human-approval** capability: an
...
(that ADR's "approval seat") or, after the Channels spec, from an IM interactive
...
`CHANNEL_APPROVAL_RESOLVED` audit event. The Agent Chat spec's User Stories 6/11,
the Channels spec's User Story 5, and their FR/SC/scenarios are removed.
