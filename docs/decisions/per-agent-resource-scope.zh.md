...
后者：一个资源的生效范围是本机专属的、绝不外传，于是一台机器只要不在这里激活它，
...
合起来就是这个资源的**生效范围 (reach)**，而生效范围是本机专属的。
...
2. **生效范围是本机专属的。** `scope` 与 `enabled` 是一件事——这个资源的*触达
   ...
   另一种做法——让生效范围随身携带机器 id 一起同步——正是本 ADR 在 2026-09-14
   ...
   回答谁赢：两台机器同时编辑一个资源的生效范围，等于把一条权限交给文本合并，而
   哪一轮跑在后面，就由它悄悄决定另一台机器暴露什么。本机专属的生效范围两个问题
   都没有，因为根本没有什么需要合并。

   代价是真实的，也值得说出来：一个资源第一次抵达某台机器时，起点是那台机器上该
   kind 的默认生效范围，而不是它在别处的生效范围。这是一个用户在页面上看得见、
   一次点击就能改的状态，而且它是「问一声」而非「替你假定」的那个方向。

   各界面**必须**在设定生效范围的地方把这件事说出来，而不是写在文档里。一个正在
   施加限制的人，有权知道这条限制到本机为止。

   「同一个控件」就是同一个按钮：按钮上写着它此刻已经持有的那个答案——*所有
   agent*、*2 个 agent*、*已禁用*——点开是一个面板，那些状态在面板里是可选项：
   *已禁用*、*所有 agent*、*只限选中的*（后者展开 scope 的勾选列表），下方就是那句
   「本机专属」。这个形状是靠丢掉上一个形状换来的。此前是并排的三段（*已禁用* /
   *全部生效* / *限定范围…*），用三个控件表达两个状态——因为「全部生效」和「限定
   范围且每个 agent 都勾上」是同一个答案写了两遍——而读的人得把三段都比一遍，才知道
   哪一段是当下生效的那个。一个把生效范围写在脸上的按钮，读一眼就回答了这件事。
   *已禁用*仍然保留自己的那一项，因为它仍然**是**一个独立的选择：它有自己的端点、
   自己的审计事件，而且那次写入刻意不动 `scope`，所以重新启用会恢复用户原本挑好的
   那些 agent。面板把勾选列表暂存起来，只在关闭时写入一次——两个整值选项会自己把
   面板关掉——因此一个只被打开又被关掉的面板，什么都不写。不声明 scope 的 kind 拿到
   的是同一个按钮，只是面板变成*已禁用* / *启用*两选一；而多选没有「当下的触达
   范围」可写，它的按钮读作*设置生效范围…*，面板打开时什么都没选中。

3. **哪里都注册，被授予处才激活。** 资源仍然会收敛到每一台机器，并在每一份注册表
   中保持可见；在生效范围之外它只是不被激活——不暴露、不分发。注册表仍是「有什么
   存在」的唯一事实来源；生效范围则是「有什么在跑」的本机答案。

   `channel` 和别的 kind 一样收敛，而「哪台机器运行它」由它自己的一个字段回答——
   配置里的 `runs_on`（spec channels FR-080）。那不是 scope 的一条轴，这是有意的：
   生效范围是「这里跑什么」的**本机**答案，所以它从不外传；而运行一条渠道的那台机器，
   是几台机器共有的同一个答案。渠道曾经短暂地根本不收敛，正是因为当时没有地方说这句话。
...
- **在会同步的 scope 里放一条机器轴**——不论是它被试过的哪一种形状：2026-08 的
  ...
  正是这么读的。
- **同步生效范围，但让最后写入者获胜**——把 `enabled` 与 `scope` 留在 bundle 里，
...
- 每一道接缝都向同一个函数问同一个问题——`is_active(scope, agent)`。没有 evaluator
  ...
  另一条」、还得为此解释自己。
- 生效范围是逐机器回答的，这意味着两台机器可以合法地在同一个资源上给出不同答案，
...
- 有两个 kind 读 scope 的后果超出了「过滤」：休眠的渠道不运行，休眠的连接不投射到任何 agent。
  ...
  可导出、可编辑。但休眠终究不等于*已禁用*，任何界面都不得把它写成已禁用：空列表是被收窄到
  一个都不剩的 scope，已禁用则是那个被扳下的开关——所以生效范围控件把它报成*未选中 agent*。
...
- **2026-09-14，同日稍后** — machine 轴再次被撤回，而这一次的理由不是机器不存在了。
  一个资源的生效范围——`enabled` 与 `scope` 合在一起——被定为**本机专属**：它在它
  ...
  判断不了时取休眠，从而不放宽任何东西。`channel` 当时也随之彻底不再收敛（第 3 条），
  后来又改回收敛，靠的是它自己的机器绑定而不是 scope 的一条轴。机器
