...
本 ADR 两度带上过 **machine** 轴，也两度把它丢掉。第二次把问题了结了。收敛后的
vault 让每台机器都持有所有机器的资源，于是「不在这里」确实是资源必须能够表达的
东西——但表达它有两个地方，而只有其中一个能被说这句话的人核对。把机器 id 写进
一份会外传的 `scope` 文档里，等于人坐在哪里就从哪里去断言，断言的还是自己看不见
的那些机器。把答案留*在*每台机器上，则是在它生效的地方说这句话。Coffer 现在选
后者：一个资源的触达范围是本机专属的、绝不外传，于是一台机器只要不在这里激活它，
就已经表达了「不在这里」，而那条轴再没有什么可补充的了。

## 决策

在资源模型上增加一个由框架拥有的 `scope` 字段——一条 agent 的允许列表；每个 kind
声明自己是否支持 scope，并各自拥有自己的执行点。scope 连同资源的 `enabled` 标志，
合起来就是这个资源的**触达范围 (reach)**，而触达范围是本机专属的。
...
1. **框架级 `scope`。** 唯一的 `scope` 形状位于 `Resource` 实体上，而不在 kind
   ...
   scope == {agents: ["claude-code"]}         → 只对该 agent
   scope == {agents: []}                      → 休眠（空列表匹配不到任何东西）
   ```

   - `None` 不构成限制；给了列表就限制到该列表。
   - `is_active(scope, agent)` → 该 scope 准入这个 agent 时为 `True`。无身份的
     会话（`agent=None`——一个手工配置、不上报 `--agent` 的 shim）只匹配不受限的
     scope，因此它看到的严格更少，绝不会更多。
   - 出现未知的名字是合法的，只是永远匹配不上——可以在某个 agent 出现之前就把它
     划进 scope。
   ...
   - kind 还可以**对被提议的 scope 做前置校验**（`Kind.validate_scope_for`），它是
     ...
     目前只有 `channel` 提供它（见第 8 条）。

2. **触达范围是本机专属的。** `scope` 与 `enabled` 是一件事——这个资源的*触达
   范围*，由同一个控件写下——而它们都不会被序列化进同步 bundle，也不会被任何一轮
   收敛写动（spec [vault-sync](../../specs/vault-sync/spec.zh.md) 的
   `## 不同步什么`）。在机器之间流转的是资源本身：它*是什么*，以及它是怎么配置的。
   它在这里触达到哪里，就留在这里。

   另一种做法——让触达范围随身携带机器 id 一起同步——正是本 ADR 在 2026-09-14
   落定、又随即撤回的那个形状。它有两处站不住。它无法在被设定的地方得到核对：
   坐在笔记本前的用户是在一份注册表里挑 id，而那份注册表描述的恰恰是他并没有坐在
   跟前的那些机器；一个敲错或已退役的 id，会让某个资源在他看不到的地方休眠。它也没有一个诚实的答案来
   回答谁赢：两台机器同时编辑一个资源的触达范围，等于把一条权限交给文本合并，而
   哪一轮跑在后面，就由它悄悄决定另一台机器暴露什么。本机专属的触达范围两个问题
   都没有，因为根本没有什么需要合并。

   代价是真实的，也值得说出来：一个资源第一次抵达某台机器时，起点是那台机器上该
   kind 的默认触达范围，而不是它在别处的触达范围。这是一个用户在页面上看得见、
   一次点击就能改的状态，而且它是「问一声」而非「替你假定」的那个方向。

   各界面**必须**在设定触达范围的地方把这件事说出来，而不是写在文档里。一个正在
   施加限制的人，有权知道这条限制到本机为止。

3. **哪里都注册，被授予处才激活。** 资源仍然会收敛到每一台机器，并在每一份注册表
   中保持可见；在触达范围之外它只是不被激活——不暴露、不分发。注册表仍是「有什么
   存在」的唯一事实来源；触达范围则是「有什么在跑」的本机答案。

   `channel` 是唯一一个根本不收敛的 kind。渠道是一条绑定在单台机器上的入站面——
   它的端口、它的隧道、平台被告知要去回调的那个 webhook URL——所以一条渠道跑到
   别处，往好了说是死的，往坏了说是两台机器同时应答同一段对话。

4. **各 kind 的支持情况与执行接缝。** 每个 kind 在自己既有的咽喉点查询 scope，
   而不是新设一个中央门禁：

   | Kind | 执行接缝 |
   | --- | --- |
   | `mcp_server` | 网关按会话的 agent 过滤该 server 的工具，因此一个被 scope 排除在某 agent 之外的 server 对它不呈现任何工具。 |
   | `skill` | 分发时取 skill 自身 `enabled` 与 scope 的交集；已分发但被禁用或不在 scope 内的副本会被回收。 |
   | `knowledge` | 内置知识工具过滤：一个会话能 list、grep、读、写哪些 collection。 |
   | `memory` | recall 过滤它能读哪些 partition，因此聚合出的 partition 只触达被 scope 到的那些 agent（spec memory FR-014）。 |
   | `channel` | **反向。** 渠道不被任何 agent 消费，因此它的 scope 命名的是渠道可以**驱动**哪些 agent：`/agent` 只列出、只提供、只接受这些，一条什么都驱动不了的渠道根本不会启动自己的适配器。 |
   | `provider` | 投射接缝：切换、按 agent 的密钥查找、导入后收敛、启动自检，读 scope（∩ `enabled`）来决定一条连接被写进哪些 agent。 |
   | `agent` | 无。它**就是** agent，因此没有什么可供 scope 收窄。非 null 的 scope 在校验阶段被拒绝。 |

5. **shim 自报的 `--agent` 身份。** shim 安装时把
...
6. **skill 的分发就是 `enabled` ∩ scope，再无其他。** 本决策最初保留了 agent 侧
...
7. **知识 collection 是要 scope 的——2026-09-12 修订。** 本 ADR 最初写的是
...
8. **渠道是要 scope 的，且它的 scope 是反向的——2026-09-13 新增。** 本 ADR 最初写的是
...
9. **`provider` 是要 scope 的，它自己那个「投给哪些 agent」的字段被撤回——2026-09-13 新增。**
   `provider` kind 本来就有这个轴：config 里的 `compatible_agents`，决定一条连接投射到哪些
   agent。它是最后一个还在用自己那套回答框架这个问题的 kind——正是决策第 4 条要终结的分歧
...
- **在会同步的 scope 里放一条机器轴**——不论是它被试过的哪一种形状：2026-08 的
  machine × agent 矩阵，还是 2026-09-14 的两条按 `AND` 相交的允许列表。两者回答
  「这东西在哪里跑」的方式，都是把机器 id 写进一份会外传的文档里。否决，理由见
  决策第 2 条：设定它的人无法从他所在的地方核对它，而两台机器同时编辑它，就是把
  一条权限交给文本合并。本机专属的答案不需要 id、不需要编辑时去查注册表、也不
  需要合并——而它表达不了的那个问题，「在同一个资源上为每台机器指定不同的
  agent」，本来就是靠在每台机器上把那个资源设成不同的样子来回答的，对用户来说也
  正是这么读的。
- **同步触达范围，但让最后写入者获胜**——把 `enabled` 与 `scope` 留在 bundle 里，
  接受由收敛来裁决。否决：输的那台机器永远不会被告知。一条权限因为另一台机器跑了
  一轮而改变，正是这套设计在别处——包括在它自己的 migration 里——处处拒绝的那一类
  无声放宽。
...
- `scope` 是 `Resource` 上的一个可空对象，按 kind 校验，没有任何专用机制。它不
  外传：一轮收敛既不读它也不写它。
- 每一道接缝都向同一个函数问同一个问题——`is_active(scope, agent)`。没有 evaluator
  对象要构建，没有机器身份要往下传、也就没得忘，更没有哪道接缝「只读一条轴而不读
  另一条」、还得为此解释自己。
- 触达范围是逐机器回答的，这意味着两台机器可以合法地在同一个资源上给出不同答案，
  而且谁都没错。这正是本意，也正是各界面必须明说出来的那件事——因为一个以为不是
  这样的用户，假定的恰好是更危险的那一半。
...
- **2026-09-12** — `knowledge` 反转为声明 scope：知识层被削减为纯文件后，collection 是它唯一的
  边界（决策第 7 条）。
- **2026-09-13** — `channel` 与 `provider` 反转为声明 scope（决策第 8、9 条），并把 `memory`
...
- **2026-09-14** — machine 轴随双向同步回归（[Vault Sync](./vault-sync.zh.md)），
  形态是第二条按 `AND` 相交的允许列表，而不是 2026-08 时的那个矩阵。
- **2026-09-14，同日稍后** — machine 轴再次被撤回，而这一次的理由不是机器不存在了。
  一个资源的触达范围——`enabled` 与 `scope` 合在一起——被定为**本机专属**：它在它
  生效的那台机器上设定，绝不序列化、绝不收敛，每台机器各设各的（决策第 2 条）。
  这件事定下来之后，那条轴就没有什么可表达的了，于是 `scope` 重新只剩 agent 一条轴，
  由 migration `0076` 剥掉那个键——逐行拿 daemon 当时真正在用的那个机器 id 去求解，
  判断不了时取休眠，从而不放宽任何东西。`channel` 则彻底不再收敛（第 3 条）。机器
  **注册表**丝毫未动：它属于同步，不属于权限。
