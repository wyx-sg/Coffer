...
`machine_id` is a key: it names the machine's descriptor file, it is what a
resource's `scope.machines` references, and it must survive reinstalling
Coffer. A machine that comes back under a new identity becomes a ghost —
everything scoped to the old one silently stops working, with no error to
explain it. So the id is derived from the host (`IOPlatformUUID` on macOS,
...
identity is not a nicety for the scope pick-list; it is what makes joining
safe.

`machine_name` is the opposite: chosen by the user, changeable at any time at
no cost, and stored inside the machine's own descriptor. Renaming a machine
cannot break a scope, because no scope references a name.
...
- Resource `scope` regains a machine axis — two independent lists, `AND`-ed,
  each `null` meaning unrestricted. Existing rows migrate by addition, because
  `machines: null` reproduces today's behaviour exactly.
