...
**日期**：2026-06-12
**决策者**：Yuxing Wu
**相关**：`.specify/memory/constitution.md`（凭据不变量——已修订至 v0.2.0）、spec `mcp-gateway`（data-model：`credentials` 表、审计事件）、[PyInstaller Distribution](distribution-pyinstaller.md)（未签名的 macOS 分发）
...
（[PyInstaller Distribution](distribution-pyinstaller.md) 推迟了需要付费 Apple
