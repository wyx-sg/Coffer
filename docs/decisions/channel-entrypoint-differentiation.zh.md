...
> English: [channel-entrypoint-differentiation.md](./channel-entrypoint-differentiation.md)

**状态**：已接受
**Spec**：[channels](../../specs/channels/spec.zh.md)
...
Channels 规范让 Coffer 成为 channel 入口管理者：owner 把一个 IM 账号配对到
...
2. **per-channel agent 路由是粘性的,存在 peer 上。** `/agent <key>` 对 agent
   ...
   `codex` 及未来注册的 agent）,且不为任何 agent 写 channel 侧代码（spec channels
   SC-004 保持）。
...
- 命令集增 `/agent` 与 `/model`；二者都骑现有 slash 命令接缝、在 channel core
  ...
  任何未来 channel 都继承它（spec channels SC-003 保持）。（`/cwd` 已撤销，见上方
  修订。）
