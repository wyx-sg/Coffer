# 代码布局 —— 分层优先，按 kind 分子目录

> English: [code-layout-layer-first.md](./code-layout-layer-first.md)
...
[Resource Framework Upfront](resource-framework-upfront.md) 承诺在规约 `mcp-gateway` 中实现资源框架。在遵守章程
...
评估后推迟：见 [Resource Framework Upfront](resource-framework-upfront.md)。它本身不是布局决策；在此仅作为我们考虑过
但否决的「最重」参照物。
