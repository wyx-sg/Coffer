...
- **（C）「真的能跑」那两级缺失。** 每一层都跑在 fake 上。channels 与 sync 两份 spec、以及 CLI-agent chat，都是带着 fake-API 覆盖合并、**真实使用留给手动的、带外的验证** —— 一笔反复出现的债。密闭回路里绿 ≠ 对真 Telegram/SeaTalk/git remote 能跑通。
