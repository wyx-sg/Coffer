# 跨 agent 共享的单一知识 store

> English: [agent-native-shared-memory.md](./agent-native-shared-memory.md)

**Status**: 部分被 [Memory via MCP](memory-via-mcp-not-native-projection.md) 取代（2026-06-18）—— 下文决策中「原生投影」那一半已移除；Coffer 绝不写 agent 自己的原生记忆文件。「共享单一 store」那一半依然成立，也正是本 ADR 现在所记录的内容。2026-09-10 以知识层词汇重新表述（见修订历史）。
...
第一版记忆设计（跑 mem0 memory 引擎）把每个 store 当成一个通过 MCP 查询的私有
孤岛。但现实中，开发者会在同一个项目上跑不止一个编码 agent（Claude Code、Codex
…）。每个 agent 各有自己的原生记忆位置 —— Claude Code 的 auto-memory 目录、Codex 的 `memories`
...
- **一个资源 kind：`knowledge`。** 一个存储根 `~/.coffer/knowledge/<scope>/`，
  ...
  事实；SQLite 是其上一份可重建的索引（ADR：files-as-truth-sqlite-retrieval）。
...
- **不碰 agent 文件也能做环境式交付。** _2026-09-10 撤回。_ Context 里点出的那个
  ...
  agent 上，因此已删除（spec agent-registry FR-043…FR-048）。于是这个落差**重新敞开**：没有
...
托管块，并关闭 agent 自己的原生记忆使副本无法分叉。被 [Memory via MCP](memory-via-mcp-not-native-projection.zh.md) 取代：那样 Coffer
