...
**Date**: 2026-06-14
**Deciders**: Yuxing Wu
**Related**: [`docs/principles.md`](../principles.md), [spec `mcp-gateway`](../../openspec/specs/mcp-gateway/spec.md), [Everything Is a Resource Kind](everything-is-a-resource-kind.md), [Files as Truth](files-as-truth-sqlite-retrieval.md)
...
namespaced surface. That is the whole point of spec mcp-gateway — register once, use
...
- **Ranks the live aggregated catalogue.** It scores the currently-aggregated
  ...
  routing is unchanged from spec mcp-gateway.
...
(who / when / how-long / outcome, no arguments or results — per spec mcp-gateway).
...
- **Static namespacing / manual curation alone.** Rejected as insufficient:
  prefixing and per-tool enable/disable (spec mcp-gateway) already exist and help, but a
