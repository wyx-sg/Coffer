# 信息架构 — 一切皆 resource kind

> English: [everything-is-a-resource-kind.md](./everything-is-a-resource-kind.md)
...
Spec `mcp-gateway` 在一个 kind-agnostic 的 Resource 框架（见 [Resource Framework Upfront](resource-framework-upfront.md)）之上引入了 `mcp_server` 这一 resource kind。`mcp_server` 是当下已交付的 kind。

Spec `ui-shell` 需要决定 resource kind——以及一些并非领域意义上"资源"的横切用户可见概念（审计日志、settings）——如何呈现在导航中。最朴素的做法是在 IA 中再引入一个顶层概念：在"resource kind"轴之外再设一条"surface"轴，让侧栏呈现为两组毫无关联的入口 (Resources vs Features)。本次重设计的目标恰恰相反：**一种无需随产品成长而反复重新论证的统一导航模型。**
...
- 来自 [Resource Framework Upfront](resource-framework-upfront.md) 的 Resource 框架是 UI 唯一需要理解的抽象；不需要再维护一份独立的"surface"注册表。
...
- 读起来就是一个未完成的脚手架（这恰是 spec ui-shell 动机里明确反对的）。
...
Skill Manager 规范把 **skill** 以后端 resource kind 交付（身份、生命周期、审计、`on_delete` 级联全部走 kind-agnostic 框架），但前端是**专属页面**（`/skills`、`/skills/:name`），没有走前端 kind registry。这让 UI 处在一个未声明的半途状态：registry 承诺"每个 kind 一个通用浏览界面"，而 skill 悄悄绕开了它。

本次修订定案前端模式：

- **后端这条轴不变。** 每个资产都是 resource kind；unify-identity/lifecycle/audit 与 never-unify-invocation 规则（ADR：resource-framework-upfront）对所有 kind 依然成立。
