...
**聊天是属主同样从手机（IM）驱动的那些会话的网页界面。** 一个属主、一条会话时间
线、两块屏幕。一个会话背后是一个真实的 agent session（一个 `--resume` session、
一个工作目录），所以手机上发起的回合和在网页界面继续的回合命中**同一个** session。

在网页界面上属主可以实时**观测**任何会话（包括从手机发起的回合，逐 token），**中断**
...
`channel_name` 被设置）；当存在 binding 时网页界面显示一个小小的"also on
Telegram/SeaTalk"指示。
...
- **同一套接缝，不另起并行路径。** 网页界面只通过既有的 `ConversationPort` /
  `TurnPort` / orchestrator 驱动回合；一个 agent 分不清一个网页回合和一个手机
...
**拒绝。** 属主明确想从网页界面抢方向盘（停下一个失控回合、继续输入）。只观测会让手机
发起的会话从网页界面无法操控 —— 而这正是网页席位存在的意义。
