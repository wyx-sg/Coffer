...
### 在能签名之前，app 是 CLI 之上的可选层

`coffer-daemon` **不**打包进 `.app`。壳按顺序解析它：先看自己的包内，再看 `daemon.json` 里是否已有在跑的 daemon，然后 `~/.coffer/bin/`，然后 `$PATH`，都没有则提示需要先安装 Coffer CLI。第一步今天是空的，加上 `externalBin` 的那一刻就会生效——解析顺序按最终形态写死，所以将来打包是一次打包配置的改动，不是代码改动。

这不是行业惯例，而且这个偏离是有意的。同类产品——Ollama、Docker Desktop、Tailscale、OrbStack——都发一个自包含的 `.app`，再把 CLI 从里面链**出来**。Coffer 将来也会这样，等它能被签名之后。在未签名的状态下，顺序必须反过来：`curl | sh` 下载的文件不带 `com.apple.quarantine`，装完直接能跑；而浏览器下载的未签名 `.app` 双击会被拒绝并提示「Coffer 已损坏」，只能去系统设置里救回来。在拿到 Developer ID 账号之前，终端那条路是**更好**的初次体验，所以 CLI 保持为文档里的主路径，app 作为「已经装了的人」的附加项呈现。

直接的后果是 `make desktop` 是一次几分钟的 `cargo tauri build`，而不是一次 PyInstaller 运行——这也正是让这个壳便宜到值得存在的原因。
...
- **Rust 重新进入构建，而它的测试不在 `make verify` 里。** `cargo` 仅作为 `make desktop` 的前置条件回归，发布流水线不受影响。代价是 `tray.rs` 的关闭到托盘判定、`daemon.rs` 的限流与端口解析辅助函数——它们当初被写成纯函数正是为了可单测——由一组没有任何门禁会跑的 `cargo test` 覆盖。有工具链的人可以用 `make desktop-test` 跑到它们。接受这一点，是因为壳很小、很少改动，而且改坏了也伤不到 daemon 和 Web 宿主；如果它开始积累逻辑，就该重新评估。
...
- **现在就打包全部二进制，把 `.dmg` 作为主下载。** 目前否决，但不是原则上否决。这是它最终该去的地方；前提是先有一个签名并公证过的 app，在那之前，它会让陌生人与 Coffer 的初次接触严格差于终端那条路。
