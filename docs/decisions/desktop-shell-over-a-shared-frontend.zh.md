...
壳解析 daemon 仍然走五级链路：自己的包内、`daemon.json` 指出的在跑的 daemon、`~/.coffer/bin/`、`$PATH`、再不行就提示安装 CLI。打包让第一级从「死的」变成常规路径；其余几级留作包内 sidecar 缺失或跑不起来时的退路，而第二级是防止 app 在用户已有 daemon 旁边再起一个的那道闸。
