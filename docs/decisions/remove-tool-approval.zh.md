# 移除工具审批系统；owner 配对即门禁

> English: [remove-tool-approval.md](./remove-tool-approval.md)

- **Status:** Accepted
- **Date:** 2026-06-18
- **Deciders:** Yuxing Wu
- **Spec:** [channels](../../specs/channels/spec.zh.md)（移除——不开新 spec；该 `spec.md` 随本 ADR 一起更新）
- **Supersedes:** 那份已被移除、把 Agent Chat 重定位为金库控制台的 ADR 的**「审批席位」**职责；移除由已退役的 Agent Chat 规范引入、再由 Channels 规范桥接到 IM 的逐工具审批中继（[Channel Adapter Framework](./channel-adapter-framework.zh.md)、[Channel Entrypoint Differentiation](./channel-entrypoint-differentiation.zh.md)）

## Context

已退役的 Agent Chat 规范给聊天平台加了逐工具的**人工审批**能力：agent 想跑某个工具时发出
`ApprovalRequest`，turn 停在 `ApprovalGate` 上，由人来允许/拒绝——可从 web 的
`ApprovalCard`（那份 ADR 的「审批席位」），或在 Channels 规范之后从 IM 的交互式
...
`CHANNEL_APPROVAL_RESOLVED` 审计事件。Agent Chat 规范的 User Story 6/11、Channels 规范的
