# Channel 媒体——DB 只存引用，发送时按 agent 物化

> English: [channel-media.md](channel-media.md)

**状态**：Accepted（其中 v1「推迟持久化附件块」的决定被
[Persisted Attachment Reference](persisted-attachment-reference.zh.md) 取代——FR-033）
**日期**：2026-07-04
**决策者**：Yuxing Wu
**相关**：spec `channels`（FR-020）；构建于 [Channel Adapter Framework](channel-adapter-framework.zh.md)（channel adapter 框架）；部分被 [Persisted Attachment Reference](persisted-attachment-reference.zh.md) 取代
