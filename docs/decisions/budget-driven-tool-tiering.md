...
[Tool Retrieval](tool-retrieval-for-overload.md) shipped `coffer__search_tools`
as the answer to aggregation overload and made one explicit bet:
...
tool-search indirection — **including `coffer__search_tools` itself**. Tool
Retrieval's escape hatch ended up locked behind the same door it was built to
open. Leaving
...
called first. Tool Retrieval's contract was documented for humans and never
delivered to the agent that had to follow it.

Tool Retrieval's own research is the calibration: agent tool-selection accuracy
falls off sharply past **30–50 tools**. Coffer was serving two-to-four times that and
...
- `budget` (**default 50**, from Tool Retrieval's own 30–50 finding) counts
  **upstream tools only**; the always-listed built-ins sit on top of it.
...
`mcp_capability_preferences.enabled` ([Capability State Model](capability-state-model.md)),
...
**Escape hatch.** A `mode: auto | off` setting; `off` restores Tool Retrieval's
...
- **The default install stops overloading the client.** The listed upstream
  ...
  upstream slice sits inside Tool Retrieval's 30–50 accuracy band. Whether that
  clears any particular client's deferral threshold is not something Coffer can
  know —
...
- **Coffer gains strategy 1 without the cost of per-agent MCP scoping.** The
  subset is curated by observed behavior rather than by the user, which is
  precisely the manual step that made that retired decision not worth its
  complexity.
...
**Keep Tool Retrieval unchanged; rely on client deferral.** Rejected: this is the
option that was measured and failed. The client folded away the very tool Tool
Retrieval added, and it has no signal with which to choose better.

**Manual per-agent allowlists only (strategy 1 as the field does it, or
extending Per-Agent Resource Scope's agent axis to tool granularity).** Rejected
as the primary mechanism: it does nothing until configured, and the unconfigured default is
exactly where the failure occurs. The retired per-agent MCP scoping decision
already demonstrated that hand-curated scoping is not worth its complexity here.
Per-Agent Resource Scope's server-level axis remains available and composes with
tiering — scope decides which servers a session sees, tiering decides which of
their tools get listed.
