...
- **2026-09-12** —— 可选的固定端口。端口漂移破坏的恰恰是用户有权视为稳定的那一样
  ...
  顺带彻底修掉了两个漂移诱因。绑定用的 socket 现在设置 `SO_REUSEADDR` —— 少了它，
  `stop` 紧接着 `start` 可能因端口仍处于 `TIME_WAIT` 而失败并漂到下一个端口；在
  macOS/BSD 上该选项只放行 `TIME_WAIT`、绝不放行活着的 `LISTEN`，因此 CODE-041 的
  防窃取性质不受影响。固定端口的绑定另加了短暂重试，针对的是同一个重启场景。
