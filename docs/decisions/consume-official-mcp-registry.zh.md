# 消费官方 MCP Registry 做服务器发现

> English: [consume-official-mcp-registry.md](./consume-official-mcp-registry.md)

- **状态：** Reverted（2026-06-20）——见下方撤销说明
- **日期：** 2026-06-19
- **决策者：** Yuxing Wu
- **Spec：** mcp-gateway（实现前更新 spec.md）
- **相关：** [Capability State Model](./capability-state-model.md)（列表实时查询、不做镜像）、[Envelope-Encrypted Credentials](./envelope-encrypted-credential-store.md)（secret 转为凭据引用）
...
4. **secret 走凭据引用，绝不内联。** 被标记 `isSecret` 的 `environmentVariables[]` 条目在
   ...
   [Envelope-Encrypted Credentials](./envelope-encrypted-credential-store.md) 的保证：secret 明文绝不落入
   任何持久化配置。
