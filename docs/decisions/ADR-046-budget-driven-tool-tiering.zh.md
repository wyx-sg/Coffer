# ADR-046：网关侧的预算驱动工具分层

> English: [ADR-046-budget-driven-tool-tiering.md](ADR-046-budget-driven-tool-tiering.md)
...
ADR-018 交付了 `coffer__search_tools` 作为聚合过载的答案，并押了一个明确的赌注：
...
**客户端的 deferral 是无差别的。** 面对来自单一服务器的约 125 个工具，Claude Code 把整个 `mcp__coffer__*` 命名空间挪到了它自己的 tool-search 间接层后面——**其中包括 `coffer__search_tools` 自己**。ADR-018 的逃生舱，最终被锁在了它本要打开的那扇门后。把决定权交给客户端，前提是客户端会**有选择地**折叠；它不会，也做不到，因为它没有可供判断的使用信号。Coffer 有。

**没有任何东西告诉过 agent Coffer 的存在。** 网关的 `initialize` 应答不带 MCP `instructions` 字段，因此没有任何客户端在系统提示层面收到过「Coffer 是什么」或「该先调 `search_tools`」的说明。ADR-018 的契约只写给人看，从未投递给真正需要遵守它的那个 agent。
...
- `budget`（**默认 50**，取自 ADR-018 自身的 30–50 结论）**只计上游工具**；恒定列出的内置工具叠加在它之上。
...
未列出的工具仍完全可调。`tools/call` 校验的是 `mcp_capability_preferences.enabled`（[ADR-004](ADR-004-capability-state-model.zh.md)），从不校验是否在列表中；`search_tools` 检索的仍是**完整**目录。工具离开的是列表，不是网关。正是这一条使分层可以安全地默认开启，也正是它与 strategy 1 的手工白名单的分野。
...
**逃生开关。** 一个 `mode: auto | off` 设置；`off` 精确恢复 ADR-018 的语义。若使用统计查询失败，分层降级为全部列出——一个出故障的统计层，绝不允许让工具消失。

## 后果

**正面**

- **默认安装不再压垮客户端。** 列出的上游切片从约 125 降到至多 `budget`；加上恒定列出的 Coffer 内置工具，一个会话列出约 50 + 17 ≈ 67 个而非约 142 个，且上游切片落在 ADR-018 的 30–50 准确率区间内。这是否越过了某个特定客户端的 deferral 阈值，Coffer 无从得知——此处的主张仅仅是：它远低于今天所提供的量。
...
- **Coffer 拿到了 strategy 1，且没有 ADR-026 的代价。** 子集由观测到的行为策展，而非由用户策展——后者正是当初让 ADR-026 得不偿失的那一步。
...
**保持 ADR-018 不变，依赖客户端 deferral。** 否决：这正是被实测且失败的那个选项。客户端折叠掉了 ADR-018 新增的那个工具本身，且它没有任何信号能做出更好的选择。

**只做手工 per-agent 白名单（业界的 strategy 1，或把 ADR-045 的 agent 轴下沉到工具粒度）。** 作为主机制否决：不配置就不生效，而未配置的默认态恰恰是故障发生地。ADR-026 已经证明手工 scope 在此处不值其复杂度。ADR-045 的服务器级轴仍然保留并与分层组合——scope 决定会话看到哪些服务器，分层决定它们的哪些工具被列出。
