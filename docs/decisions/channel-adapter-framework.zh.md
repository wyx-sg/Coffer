...
> English: [channel-adapter-framework.md](./channel-adapter-framework.md)
...
通过它们与聊天平台（Agent Chat 规范）上的任何 agent 对话、接收
...
1. **channel 是一种 resource kind**（`channel:<name>`，[Everything Is a Resource Kind](everything-is-a-resource-kind.zh.md)），完整
