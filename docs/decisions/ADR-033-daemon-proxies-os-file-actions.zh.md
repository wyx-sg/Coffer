...
本 ADR 统一了 open/reveal 但没统一*选取*:当时 OS 原生目录对话框只在打包的原生外壳里可用,所以浏览器界面继续用内嵌的 daemon 文件夹浏览器。这条理由已不再成立 —— daemon 自己就会打开宿主的原生对话框([ADR-036](./ADR-036-daemon-native-file-and-save-dialogs.zh.md) 取代本节),而它当时所指的那个原生外壳也已不复存在。
