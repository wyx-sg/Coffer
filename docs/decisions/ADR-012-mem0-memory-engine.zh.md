# ADR-012：Memory 引擎 —— mem0 藏在 application 端口背后

> English: [ADR-012-mem0-memory-engine.md](./ADR-012-mem0-memory-engine.md)
...
- Memory 成了 spec 006 引入的 `Tracer` 端口（ADR-011）的第二个消费者，满足 constitution 的「第二个 feature 用到时再提取横切关注点」原则，并验证了端口形状。
