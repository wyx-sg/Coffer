# ADR-026 —— Skill 内容信任层（启发式扫描，警告而非拦截）

> English: [ADR-026-skill-content-trust-layer.md](./ADR-026-skill-content-trust-layer.md)
...
体积上限、中和仓库 hooks），却从未看过**内容**。对一个以保险库（vault）自我定位的工具，
内容信任天经地义。
...
2. **每次内容进入都扫描。** import、fetch、adopt、改动内容的 update、就地文件编辑都会重新
   扫描；verdict、findings 数、ruleset 版本、扫描时间缓存在 skill config 上（无需迁移——
   `config_json` 不透明）。每次扫描都审计。
...
- 更新检测仍以 `SKILL.md` 哈希为准（FR-013），所以只改脚本、不动 `SKILL.md` 的更新是
  no-op、不会重扫；按需重扫与就地编辑重扫覆盖其余路径。
