# 把 channel 附件持久化为引用块，从历史重新物化

> English: [persisted-attachment-reference.md](persisted-attachment-reference.md)

**状态**：已接受
**日期**：2026-07-09
**决策者**：Yuxing Wu
**相关**：spec `channels`（FR-033）；取代 [Channel Media](channel-media.zh.md)
中 v1「推迟持久化附件块」的决定

## 背景

[Channel Media](channel-media.zh.md) 把 channel 媒体的字节存在带外的
...
Channel Media 接受的后果：附件按 turn 物化但从不持久化，因此任何重读全历史（无 session
resume）的场景看到的是一条备注而非图片，且附件在会话里不留痕迹。FR-033 重新审视这个
...
3. turn 任务通过从**历史里最后一条用户消息**读回附件来**推导**交给 `adapter.run_turn`
   ...
   **保持不变**（它们仍收到 `attachments: Sequence[Attachment]`，并像 Channel Media 那样按模型
...
- **保持 Channel Media 原样（仅本 turn、带外参数）**——最简单，但附件在网页 UI 里不可见，且在
  turn 中途守护进程重启后丢失。否决：FR-033 存在的意义正是让附件持久且可见。
- **在持久化块里内联 base64**——Channel Media 否决的 DB 膨胀情形；仍然否决。引用块在不含 bytes
  的前提下拿到历史/可见性的好处。
...
- 附件留存在历史里、并在守护进程重启后存活；持久化引用是物化的单一
  事实来源。
- adapter 保持不变——移动的是接缝（历史读回而非参数），而非 adapter 契约。Channel Media 的模态无关
  性质仍成立。
- 旧的纯文本消息行反序列化不变（无 schema 的 JSON 内容，无迁移）；`block_from_dict` 多了一个
  `attachment` 分支。
- 媒体字节由 30 天清扫界定，而非永久累积（补上了 Channel Media 留下的保留缺口）。
- Channel Media 中被取代的条目：其「推迟持久化附件块」的替代方案与「不作为消息内容持久化」的后果被
  本 ADR 取代。
