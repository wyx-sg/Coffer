...
> mem0（写入时不再调 LLM；由 agent 自己写出干净事实），改为以逐条 markdown 为
> 事实源、由 SQLite FTS5 + sqlite-vec 建索引（ADR-012），并把 memory 做成一个跨
> agent 共享的单一规范 store，经 MCP + 原生投影分发（ADR-013）。以下内容仅作历史
> 背景保留。
