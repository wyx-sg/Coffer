...
- **D12 — 除此之外，账号权限是用户的答案，推导不出来。** 在当前安装上目录列出十九个模型，
  ...
  配置上。于是由用户来勾。**勾在哪里变了（2026-09-12）：** 最初的答案是 `AgentConfig.models`，
  而那个归属是错的——agent 没有受众，在它身上策展会同时收窄「手机上的一个聊天」和「打开 agent
  页面的那个人」。策展现在住在**有受众的那个面**上：channel 的 `default_model` + `models`
  （spec channels FR-071）。agent 上的字段、它的路由、以及迁移 0060 的回填都已移除，由迁移
  0062 剥除。把那九个写死的方案依然被否决：Claude Code 每隔几周就发新模型，而新模型一直不
  出现的用户将无从查起。
- **D13 — 一个 agent 只回答一个问题；收窄它是「面」的职责。** `GET …/models` 返回经过滤的
  完整清单，`offered()` / `suggest()` 返回同一份列表（或激活连接的策展集合），agent 上没有
  任何东西收窄它们——`…/models/selection` 路由已不存在。需要少提供一些的那个面，自己把范围叠
  在这份列表上，并拒绝落在范围外的值，channel 的 `/model` 卡片正是这么做的。没有任何地方拿
  清单去校验模型**名字**——CLI 接受清单之外的名字，所以在 channel 范围不说话的每个地方，模型
  名都仍是原样透传。
