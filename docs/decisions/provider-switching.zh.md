...
- **D12 — 除此之外，账号权限是用户的答案，推导不出来。** 在当前安装上目录列出十九个模型，
  ...
  配置上。于是 `AgentConfig.models` 记录用户在清单里勾选了哪些——**空表示尚未策展**，整份清单
  都提供，这是默认值，也是迁移 0060 给每个既有 agent 的取值。把那九个写死的方案被否决：
  Claude Code 每隔几周就发新模型，而新模型一直不出现的用户将无从查起。
- **D13 — 策展收窄的是「提供」，不是校验，也不是清单路由。** 每个**选择器**（网页选择器、
  经 `ModelSuggestionPort.suggest` 的 channel `/model` 卡片、每轮的说明）拿到的都是策展集合；
  `GET …/models` 仍返回经过滤的完整清单，因为策展界面渲染的就是这份清单，并把策展集合作为
  勾选叠在上面。没有任何地方拿它们去校验模型**名字**——CLI 接受清单之外的名字，所以
  `/model <名字>` 仍是原样透传。该集合按 agent **类型**寻址（`GET|PUT …/models/selection`），
  与清单、与 `/model` 卡片一致，并存放在提供 config dir 的同一个 agent resource 上。
