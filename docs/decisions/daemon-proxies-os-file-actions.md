...
That fallback rests on a premise stated in the retired Knowledge Base spec's FR-020 — *"the daemon cannot act on
...
- `POST /api/v1/fs/open` `{ path, with? }` — open `path` in an application. `with` is
  the preferred-editor preference (ui-shell); when absent the OS default
...
host's native **directory** dialog itself (spec agent-registry FR-042). It does not proxy an
