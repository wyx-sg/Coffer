...
- `application/` 不得 import `surfaces/`。
- 由 `scripts/check_*.py` 与 importlinter 契约强制执行。

跨层公共模块 (凭据、审计、保留策略，……) 放在 `application/` 或
`infrastructure/` 下，且只在第二个 feature 也需要它时才抽取。（例外：
Resource 框架本身属于核心 domain，**不**算跨层组件——见
...
| Desktop                        | 独立进程                | _(计划中——见未来的 UI 规范)_ 嵌入 Vite 前端；监管 daemon；系统托盘；关闭即收起到托盘；开机自启。 |
...
- **桌面端** — GUI 外壳；首次需要时拉起 daemon；绝不复刻业务逻辑。
- **Stdio shim** — 短生命周期；其生命周期绑定到单个 MCP 客户端进程。

三者通过 `~/.coffer/daemon.json` 互相发现 (PID + 端口 + token，权限位
`0600`)。见
...
终端用户只需安装一个安装包，即可获得一个可工作的 daemon、shim 与桌面
端，且无需依赖系统 Python。实现方式：PyInstaller 把 daemon 与 shim 打成
二进制，再作为 Tauri sidecar 一同打包。跨平台 CI 矩阵产出 macOS
universal、Windows x64、Linux x64+arm64 各版本。见
...
| 想知道……                      | 请读                                            |
| ----------------------------- | ----------------------------------------------- |
| 项目必须坚守的不变量          | `.specify/memory/constitution.md`               |
| 当前正在构建什么              | `.specify/memory/roadmap.md`                    |
| 某个 feature 对用户承诺了什么 | `specs/<NNN>-<name>/spec.md`                    |
| 某个架构选择背后的原因        | `docs/decisions/ADR-*.md`                       |
| 今天的代码长什么样            | 本文件                                          |
| 某个 feature 的细节如何工作   | `specs/<NNN>-<name>/plan.md` 与 `data-model.md` |
