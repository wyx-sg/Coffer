...
| 010 | **多机同步** ([spec](../../specs/010-sync/spec.md))                           | 已接受 — 通过用户自有 git 仓库同步仓库状态 ([ADR-016](../../docs/decisions/ADR-016-multi-machine-sync.md))；由章程 0.3.0 对原则 1 的修订解禁 · **作用域修订进行中：** 把 channel 的 `runs_on` 推广为框架级机器 × agent `scope` 矩阵（`mcp_server`/`skill`/`agent`/`channel`），外加顶级 Machines 舰队视图（[ADR-045](../../docs/decisions/ADR-045-machine-agent-resource-scope.zh.md)，修订 [ADR-043](../../docs/decisions/ADR-043-sync-machine-identity-near-real-time.zh.md)）                                                                                                                                                                                                                                                                                               |
