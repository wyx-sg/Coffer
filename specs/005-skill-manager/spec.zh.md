...
**Created**: 2026-05-22
**Status**: Draft
**Input**: 用户描述：「Coffer 用开放的 AgentSkills 标准（agentskills.io）管理可移植的 AI skill。一份规范副本放在 `~/.coffer/skills/` 下；每个 agent 的可见性通过指向其 skill 目录的目录 symlink/junction 实现。用户可以从本地路径导入 skill，或从公开 Git 仓库拉取，然后按 agent 启用或禁用每个 skill。v1 支持 Claude Code、Claude Desktop、Cursor 与 Codex CLI 作为同步目标（每个 agent 是 spec 004-agent-registry 中 kind 为 `agent` 的 Resource）。」
...
开发者希望某个 skill 在 Claude Code 中可用，但在 Cursor 中不要。他按 agent 启用后，Coffer 在该 agent 的 skill 目录下创建目录 symlink。

**为什么是这个优先级**：这是「统一管理」的核心价值。没有按 agent 的启停，Coffer 与手工拷贝文件没本质区别。

**独立可测**：注册一个 Claude Code agent（按 spec 004）；导入一个 skill；为该 agent 启用；验证 `<skill_dir>/<skill-name>` 下出现指向 `~/.coffer/skills/<skill-name>/` 的目录 symlink。
...
agent 的 skill 目录可能被外部篡改（删除、替换、编辑）。开发者需要看到当前哪里和 Coffer 不一致，然后自己决定怎么处理。

**为什么是这个优先级**：用户对同步引擎的信任来自「不一致时能讲清楚」。

**独立可测**：手动删除某 agent skill 目录下的一个 symlink；运行 `coffer skill verify`；观察 drift 报告把该 missing link 列出，并附建议处置方式。
...
用户打开 Coffer，看到 Skills 页，可以通过文件选择器导入或粘贴 Git URL 拉取，能浏览列表、按 agent 切换启停、内联查看 SKILL.md。

**为什么是这个优先级**：非 CLI 用户需要一个可视化日常管理面板。

**独立可测**：打开桌面 App → Skills → 用文件选择器导入一个文件夹 → 在列表中看到它 → 把某 agent 切到 enabled → 验证 symlink 已落盘。
...
- **用户在某 agent 的 skill_dir 内编辑 SKILL.md**：由于 agent 路径是指向 master 的 symlink，该编辑实际落在 master 上，其他 agent 下次读取时即可见；不会被识别为 drift。
- **用户从某 agent 的 skill_dir 内删除一个由 Coffer 管理的文件**：同样会作用到 master；下次 `verify` 会标记其他 agent 上对应 link 是否仍能一致解析。
- **移除一个还带 skill binding 的 agent（spec 004）**：spec 004 定义了 agent kind 的 `on_delete` 接缝；spec 005 在组装根处提供 `cleanup_bindings_for_agent` 回调，先清掉该 agent 的所有 binding 与 symlink，再删掉 agent 行本身。
- **agent 的 `skill_dir` 在外部被移走或删除**：下一次同步操作会暴露失败；`verify` 报告受影响的 binding；用户通过更新 agent 的 `skill_dir` 或移除该 agent 来处置。
...
- **Then** 在 `<agent.skill_dir>/my-skill` 处创建一个指向 `~/.coffer/skills/my-skill/` 的目录 symlink（Windows 上是 junction），同时写入一行 `skill_agent_bindings` 记录该 link。
...
- **FR-009**：启用一个 binding 必须在 `<agent.skill_dir>/<skill-name>` 创建一个指向 `~/.coffer/skills/<skill-name>/` 的目录 symlink（POSIX）或目录 junction（Windows）。
