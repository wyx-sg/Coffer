...
**Created**: 2026-05-22
**Status**: Draft
**Input**: User description: "Coffer manages portable AI skills using the open AgentSkills standard (agentskills.io). One canonical store lives at `~/.coffer/skills/`; per-agent visibility is a directory symlink/junction into each agent's skill directory. Users can import skills from local paths or fetch them from public Git repositories, then enable or disable each skill per registered agent. v1 supports Claude Code, Claude Desktop, Cursor, and Codex CLI as sync targets (each registered as a Resource of kind `agent` per spec 004-agent-registry)."
...
The developer wants this skill available in Claude Code but not in Cursor. They enable per agent, and Coffer creates a directory symlink into that agent's skill directory.
...
**Independent Test**: Register a Claude Code agent (per spec 004); import a skill; enable for that agent; verify a directory symlink appears at `<skill_dir>/<skill-name>` pointing to `~/.coffer/skills/<skill-name>/`.
...
Files in agent skill directories can be tampered with (deleted, replaced, edited). The developer needs to see what's out of sync and decide what to do.

**Why this priority**: Trust in the sync engine depends on transparency about disagreement.

**Independent Test**: Manually delete a symlink in an agent's skill directory; run `coffer skill verify`; observe the drift report identifies the missing link with a suggested remedy.
...
The user opens Coffer, sees the Skills page, can import via file picker or paste a Git URL, browse the list, toggle per-agent enable, and read SKILL.md inline.

**Why this priority**: Non-CLI users need a visual surface for daily management.

**Independent Test**: Open desktop app → Skills → import a folder via picker → see it listed → toggle enabled for one agent → confirm the symlink exists on disk.
...
- **User edits `SKILL.md` inside an agent's skill_dir**: Because the agent's path is a symlink to master, the edit lands in master and is visible to all other agents on next read; no drift is detected.
- **User deletes a Coffer-managed file from inside an agent's skill_dir**: Master is affected (same reason); next `verify` flags any other agents whose links no longer resolve consistently.
- **Removing an agent (per spec 004) while it has skill bindings**: Spec 004 defines the agent kind's `on_delete` seam; spec 005 supplies the `cleanup_bindings_for_agent` callback at the composition root, so removing an agent first cleans up that agent's bindings and any associated symlinks before the agent row is deleted.
- **Agent's `skill_dir` is moved or removed externally**: The next sync operation surfaces the failure; `verify` reports the affected bindings; user remediates by updating the agent's `skill_dir` or removing the agent.
...
- **Then** a directory symlink (or junction on Windows) is created at `<agent.skill_dir>/my-skill` pointing to `~/.coffer/skills/my-skill/`, and a `skill_agent_bindings` row records the link.
...
- **FR-009**: Enabling a binding MUST create a directory symlink (POSIX) or directory junction (Windows) at `<agent.skill_dir>/<skill-name>` pointing to `~/.coffer/skills/<skill-name>/`.
