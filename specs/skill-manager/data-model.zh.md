...
skill manager 的实体、字段、关系与 SQLite schema 增量。建立在 spec agent-registry 的 agent kind 与 spec mcp-gateway 的 kind-agnostic Resource 框架之上。
...
迁移 `20260526_0005_skill_tables.py`（revision `0005`，down_revision `0004`）新增 skill 绑定表。agent 本身存在共享的 `resources` 表里，因此 spec agent-registry 不需要专门的 agent-tables 迁移。
...
操作（FR-006）；UI 查看器从不编辑内容。另一个**写入**辅助函数
...
| `abs_path` | `str`             | 磁盘绝对路径（用于在外部编辑器中打开 / 显示，FR-006） |
| `type`     | `"file" \| "dir"` | 节点类型                                                         |
...
| `abs_path`        | `str`  | 文件的磁盘绝对路径（FR-006）                 |
| `folder_abs_path` | `str`  | 文件所在文件夹的磁盘绝对路径（FR-006）       |
