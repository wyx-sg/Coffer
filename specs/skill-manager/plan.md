...
- Add Alembic migration `20260526_0005_skill_tables.py` (revision `0005`, down_revision `0004`) creating `skill_agent_bindings`. Agents live in the shared `resources` tables, so spec agent-registry needs no agent-tables migration.
...
- Integration:
  ...
  - Remove agent (via spec agent-registry) → that agent's bindings + symlinks cleaned but master unchanged.
...
- Acceptance markers `@pytest.mark.acceptance(spec="skill-manager", scenario="…")` for each spec scenario.
