...
一个流式回合协议，以及用户中断 —— 全部针对一个 **agent-provider 注册表**表达，
因此第二个 agent 是一个注册表条目，而非一次重构。所交付的这一个 agent 是内置的
"Coffer Assistant"：一个进程内的 LangGraph ReAct 循环，运行在用户配置的 LLM 上，
使用 Coffer 的 MCP gateway 所聚合的每一个工具。
...
| **测试**        | 带 acceptance 标记的 4 层模型。LLM 通过 `FakeChatModel` 伪造；平台接缝以一个 `FakeAgentAdapter` / `FakeAgentProvider` 演练。                                                    |
| **约束**        | LangGraph/LangChain 限制在 `infrastructure/chat`。内置 agent 外部行为不变。DB 或日志中无凭据材料。文件大小 + importlinter 契约成立。                                            |

## 宪法检查

- **Local-first** —— 对话与消息从不离开设备；LLM provider 只是推理
  provider。
- **Spec-as-Truth** —— `spec.md` 先被修订；`contracts/api.openapi.yaml` 是由
  `make verify-contract` 把关的 wire 契约。
- **分层架构** —— `domain/chat` 保持纯净。`application/chat` 定义平台接缝
  （`AgentAdapter`、`AgentProvider` 端口；注册表的具体实现）。`infrastructure/chat`
  持有 LangGraph adapter 与 `BuiltinAgentProvider`。import 方向不变；Contract 9 不变。
...
- **`AgentAdapter.run_turn(*, history)`** —— 自包含。adapter 携带它
  自己的模型、工具与配置；orchestrator 只注入历史。
...
| `run_turn` 输入              | `history, tool_gateway, model, system_prompt` | `history`（其余在构建时烘焙进 adapter）                              |
...
- **Unit** —— `test_agent_registry.py`（新）；
  `test_turn_orchestrator_with_fake_adapter.py` 为注册表驱动的 API 与中断
  而重做 —— 用一个 fake adapter 驱动，这就是 spec 的 "fake test adapter"；
  `test_chat_service_with_fakes.py` 为
...
无契约变更。新的 `application/chat` `registry` 模块与新的
...
用户创建或用户编辑的 agent；一个管理注册表的 GUI；远程通道；超出 gateway 门控的
每 agent 能力作用域；对话摘要、搜索、导出。见 `spec.md` 的 Assumptions。
