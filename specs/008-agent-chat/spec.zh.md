...
上的**同一个属主** —— 所以聊天是**属主同样从 IM 驱动的那些会话的浏览器界面**：一个
属主、一条会话时间线、两块屏幕、一个底层 agent session。在浏览器上属主可以实时**观测**
...
- **流式客户端在回合中断连**（页面关闭、导航离开）：回合在服务器端完成且助手
  消息被持久化；下次加载显示已完成的消息。
- **SSE 连接在回合中途断开但页面仍打开**（代理/空闲超时、瞬时网络抖动）：客户端
  重新订阅；由于 attach 会重放进行中回合的事件（FR-019a），错过的事件——包括终止
  的 `turn_done`——会在新连接上抵达，于是实时气泡得以收束，而不会一直转圈「思考
  中」直到下次发送。重连有上限；一旦回合的回复落库，客户端便对账持久化消息并停止。
...
- **FR-018a**：当进行中回合结束时，System MUST 把待处理队列的队首出队、把它提交为
  ...
  后自动推进（包括一个 IM 驱动的回合），所以一条排在手机发起回合之后、在 Web UI 中输入的消息，会在那个
...
- **FR-019a**：System MUST 在 `GET /conversations/{id}/events`（SSE）暴露一个每对话
  ...
  回放当前回合的事件（让一个迟到的订阅者 —— 例如回合中途打开的 Web UI，或观看一个手机
...
- **FR-027**：System MUST 在 Web UI 中提供一个 Chat 页面：一个可折叠的对话历史
