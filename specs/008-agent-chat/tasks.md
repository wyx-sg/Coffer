...
- [x] T-003 Revise `contracts/api.openapi.yaml` (GET /agents, POST /interrupt, ConversationCreate)
- [x] T-004 Commit `docs(agent-chat): revise spec 008 to an agent-agnostic chat platform`

## C2 — Domain

- [x] T-010 `domain/chat/events.py` — extend the `AgentEvent` union
- [x] T-011 `domain/errors.py` — add `UnknownAgent`, `AgentConfigRejected`
- [x] T-012 Commit `feat(agent-chat): platform domain errors`

## C3 — Application platform seam

- [x] T-020 `application/chat/ports.py` — `AgentProvider`; new `AgentAdapter.run_turn(*, history)`
- ~~T-021 (removed — approval channel deleted)~~
...
- [x] T-024 `application/chat/turn_orchestrator.py` — `_ActiveTurn`, registry-driven `start_turn`, `interrupt_turn`
...
- [x] T-026 Commit `feat(agent-chat): agent-provider registry, interrupt`

## C4 — Built-in agent as a provider

- [x] T-030 `infrastructure/chat/langgraph_agent.py` — `run_turn(*, history)`; self-contained adapter
...
- [x] T-040 `surfaces/http/chat/schemas.py` — `ConversationCreate` (agent_key/agent_config), `ChatAgentOut`/`ChatAgentListOut`
- [x] T-041 `surfaces/http/chat/conversation_routes.py` — POST body; `GET /chat/agents`
- [x] T-042 `surfaces/http/chat/turn_routes.py` — `POST .../interrupt`
- [x] T-043 `surfaces/http/{dependencies,wiring,errors}.py` — registry wiring + new error-code mapping
- [x] T-044 Update `tests/integration/chat/*` + `tests/contract/test_chat_openapi.py`. Interrupt is proven by orchestrator-level acceptance tests with a fake adapter; the HTTP routes get 404/204 route-guard tests
- [x] T-045 Commit `feat(agent-chat): chat platform REST surface — agents, interrupt`

## C6 — Frontend

- [x] T-050 `lib/api/chat.ts`, `lib/chat/streamClient.ts` — agent list, interrupt
- [x] T-051 `lib/hooks/{useChatAgents,useConversations,useChatTurn}.ts`
- [x] T-052 `components/chat/NewConversationDialog.tsx`; `Composer`, `MessageThread`, `ChatPage` updates
- [x] T-053 i18n keys (`en.json`, `zh.json`); `NewConversationDialog` component tests
- [x] T-054 Commit `feat(agent-chat): chat platform UI — agent picker, stop`
