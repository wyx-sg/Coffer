...
Design decisions for the chat platform and the built-in agent. The first three
...
fake provider used in tests demonstrates.

## 2. A self-contained `AgentAdapter`

**Question**: What does a turn hand an agent?

**Decision**: `run_turn(*, history)` — nothing else. An agent's
...
## 5. Agent framework, multi-provider, in-process gateway (unchanged)
...
## 6. Conversation persistence & streaming (unchanged)
...
per-agent capability scoping beyond gateway gating;
