...
- **F1 — 连接投射到哪些 agent——2026-09-13 修订。** 连接携带一个「投给哪些 agent」的集合：
  ...
  *所有 agent*。因此：
  ...
  - **迁移 0071 把每一行都物化**——它算出旧字段解析出的有效集合，具体地写进 `scope`，再把已死的
    键从 config 里剥掉。生效范围逐字节不变；downgrade 会把值放回去。
...
- **F5 — 连接页移除每行「切换」。** 激活按 agent，落在 Agent 详情 → 概览页（按连接所触达的
  agent 过滤）。连接页是库：增 / 改 / 删 + 展示每条连接的生效范围。
...
- **K2 — 已撤回：agent 上的 `models: list[str]`。** 那个勾选集合存在 `AgentConfig` 上，并收窄
  ...
  **放置的位置**错了：agent 没有受众，在它身上策展会同时收窄「手机上的一个聊天」和「打开 agent
  ...
  不留 load-time 垫片（房规）。答案随后搬到了 **channel** 上，又在那边被撤回（修订 0068）：
...
- 详情页头部改用共享的 `ScopeControl`，取代只读的启用/停用徽章：此前列表能停用一条连接，
  而它自己的页面不能。`provider` 不声明按 agent 的 scope，因此该控件的面板渲染为
  停用 / 启用两选一的降级形态——并且现在它是这一状态**唯一**被展示和被修改的地方。
...
## 修订 2026-09-13 —— Claude Code 提供的是它的档位别名，不是它的目录

> 状态：Draft。**取代 H1 中「只列真实模型」这条对 Claude Code 的规定**，并**撤回 K1**
> （退役过滤器）——它唯一的职责是把已死的名字挡在一份如今已不存在的清单之外。

**为什么。** H1 从 Claude Code 二进制里读出带版本号的目录，是为了让选择器能区分 Opus 5 与
Opus 4.8。那份目录是累积的，而且对账号一无所知：它列出已安装版本听说过的每一个模型，包括
多数账号根本跑不了的内部系列，而本机没有任何东西说明某个账号能用其中哪些——D12 确认的正是
这件事，并把「这个账号跑不了的模型会在被选中时失败」当作代价接受了下来。让谁来策展剩下那些
的两次尝试（先在 agent 上，后在 channel 上）都已被移除。账最终以一份朴素的 bug 报告结清：
选择器里摆着十四个 Claude 模型，大多数用不了。CLI 自己从不要求用户给出带版本号的 id——它自己的
选择器提供四个档位别名，并在回合时拿账号去解析每一个，而那恰恰是 Coffer 没有的知识。提供它所
提供的，不需要 Coffer 拿出任何它其实拿不出的东西；版本也没有从屏幕上消失，因为别名当前指向
哪个模型是从同一张表里读出来的。

- **M1 —— Claude Code 的清单就是它的别名表。** id 是 CLI 自己接受的别名（`opus`、`sonnet`、
  `haiku`、`fable`——表里有什么就是什么），label 是该别名在 first-party 账号上解析到的那个
  模型的显示名（「Opus 5」）；于是一个把别名挪到新版本的发布，会自己给选择器换上新标签。
  两半来自同一段内嵌数据：紧跟在目录之后的 `aliases` 对象，以及它指向的那条目录条目。定位
  方式与从这个 bundle 里读出的一切一致——按结构定位；锚点一旦不再匹配，代价是这个来源整个
  失效，而不是退回到目录。
- **M2 —— 带版本号的目录与退役表都不再读取。** K1 随之撤回：别名不会退役，而那张按 provider
  记录退役日期的表，存在的唯一理由就是修剪 M1 所取代的那份清单。`firstParty` 日期解析与它注入
  的时钟一并删除。
- **M3 —— 其余一切不变。** Codex 依旧由它自己的 `model/list` 回答；各 CLI 的原生配置依旧贡献
  只有它知道的本地选择（Claude Code 的 `additionalModelOptionsCache`——账号自己的附加选项，
  例如 1M 上下文变体——以及 Codex 的 `config.toml`）；有激活连接时依旧由连接直接回答（K3）。
  模型**名字**在任何地方仍是原样透传：已经钉在 `claude-opus-4-8` 上的会话继续跑在它上面，
  任何 id 也仍然可以在任何能输入名字的地方输入。
- **接受的代价。** 不是所在系列当前头部的模型，无法再从 Web 选择器或频道 `/model` 卡片里
  **选**出来。它并非不可达——仍可输入，账号自己的附加选项也仍会从 `.claude.json` 到达选择器。

**取代：** H1 的「清单只列真实模型……CLI 的档位别名不提供」，就 Claude Code 而言。
**撤回：** K1。

## 修订 2026-09-13b —— Codex 的推理强度也是一个选择，Coffer 现在提供它

> 状态：Draft。**把 M1**——清单就是 agent 自己的选择器所提供的东西——**延伸**到那个选择器
> 为 Codex 携带的第二个字段。**细化**按会话的 `AgentConfig` 与 `…/models` 的返回形状。

**为什么。** M1 解决的是选择器里的 **id** 该是什么，却没有注意到：对 Codex 而言，id 并不是
选择的全部。`model/list` 会为每个模型报告一个 `supportedReasoningEfforts` 列表——`low` /
`medium` / `high` / `xhigh`，每一档还带自己的说明——以及一个 `defaultReasoningEffort`。这两样
Coffer 都没有读，于是它的选择器只提供一个模型、别无其他；而 Codex 自己的选择器提供的是模型
**和**强度，后者才是一个 Codex 用户真正要做的大部分选择。在这台机器上，`model/list` 目前只
返回**一个**模型，账算得就很直白：强度实际上是**仅剩**的那个选择，而 Coffer 一点都没提供。
这个差别也不是装饰性的——同一个提示词、同一个模型，`low` 下报告 53 个推理输出 token，`xhigh`
下报告 2569 个（约 48 倍），数字来自 Codex 自己的 token 用量通知。

- **N1 —— 强度走在 id 的旁边，绝不塞进 id 里。** `AgentModel` 新增
  `efforts: tuple[str, ...]`——agent 报告的档位，顺序与它报告的一致，不吃这套设置的 agent
  则为空——以及 `default_effort`，即未做选择时它自己会用的那一档；`…/models` 路由把两者都
  暴露出去。之所以是「旁边」，因为协议就是这么收的：强度是回合上自带的一个字段，把四个档位
  揉进名字里，会把一个模型变成四条其实是同一个模型的条目，而且用的是 Coffer 自己编的名字
  ——这正是 J3 所禁止的。只有当报告的默认值确实在提供的档位里时才保留它；一个选择器显示不出来
  的默认值，比没有默认值更糟。对报告不出任何档位的模型，不凭空造任何东西。
- **N2 —— 按会话存储，就放在模型旁边。** 它进的是模型已经待着的那个 provider 自有的
  `AgentConfig` blob——`cwd`、`session_id`、`model`，现在加上 `effort`——因为它是同一类事实：
  为**这一个会话**做出的、归 provider 所有的选择，既不属于 agent 资源，也不属于连接。
  `PATCH /api/v1/chat/conversations/{id}/agent-config` 与 `model` 并列接收 `effort`；请求体
  提到其中一个时，另一个原封不动，空值或 null 则清除该字段，让 agent 跑在它自己的默认档位上。
- **N3 —— 它落在回合上——`turn/start` `{threadId, input, effort}`，未设置时不发。**
  另外两个可能的落点是被**实验**否决的，不是靠读文档否决的。`thread/start` 完全忽略传给它的
  强度字段：响应里回显的依旧是配置里的默认值——于是一个线程级的控件会「看起来生效、实则什么
  都没改」。`thread/settings/update` 确实存在，但除非客户端在 `initialize` 里申请
  `experimentalApi` 能力，否则会被拒绝——为了一个本来就能老实设置的字段去背这个依赖，Coffer
  不干。`turn/start` 是靠测量而不是靠文档确认的（上面那组 53 对 2569）；对一个效果从不出现在
  「确认收到」的响应里的字段来说，这是唯一值得拿出来的证据。
- **N4 —— Coffer 不校验任何档位。** 与模型名一样，这个字符串原样交给 CLI；这个命名空间归
  Codex 所有，于是一次改名或新增第五档的发布，当天就能用，而这个账号跑不了的档位，会在其余
  一切不可用选择失败的同一个地方失败——在 CLI 里。报告不出任何档位的 agent（Claude Code）
  端到端完全不受影响：它的回合不带这个字段，界面上也不出现这个控件。

**延伸：** M1（清单就是 agent 自己的选择器所提供的东西）。**不变：** J3/M3——Coffer 不写死
任何模型**名字**，现在也不写死任何**档位**；两者都保持原样透传。

## 修订 2026-09-14 —— Claude Code 也会报告自己的推理档位

> 状态：草案。**修正** 2026-09-13b 修订的 **N4**，那条的收尾句——「报告不出任何档位的
> agent（Claude Code）端到端完全不受影响」——描述的其实是 Coffer 自己的遗漏，当时却被
> 读成了这个 agent 的属性。**细化 N1**（档位从哪里来），并**延伸**这个选择被提供的接口面。

**为什么。** N4 说 Claude Code 报告不出任何档位。它确实不是按 Codex 的方式报告的——它的
catalog 里没有逐模型的 `supportedReasoningEfforts`——但 Coffer 用来驱动它的那套运行时是
接受档位的：已安装的 Claude Agent SDK 声明了 `EffortLevel`，并把
`ClaudeAgentOptions.effort` 渲染成 CLI 的 `--effort` 标志。Coffer 在三处同时把它丢掉了
（发现源不报告档位、provider 不持久化、adapter 从不设置），于是「Claude Code 没有 effort」
是一句关于 Coffer 的话，而不是关于 Claude Code 的话——而且它是那种会自我印证的话：什么都
没报告，就没有任何界面会去问。

- **O1 —— 档位来自**运行时**，而不是来自模型。** Codex 是逐模型的，因为它的协议这么说；
  Claude Code 则每个条目都一样，一次性读自 SDK 自己的 `EffortLevel` 别名，而不是在这里
  写死。这恰好守住 M1 的纪律：Coffer 不持有任何列表，SDK 发布新增一档，不用改代码就能
  抵达各个 picker。一个没声明这个别名的 SDK 得到空元组，也就是本修订之前的世界——控件
  自行隐藏，turn 一切照旧。
- **O2 —— 不为它命名默认值。** `ClaudeAgentOptions.effort` 默认是 `None`，意思是「CLI
  自己决定」，而 CLI 并不说那是什么。于是 N1 的规则（「picker 显示不出来的默认值，比没有
  默认值更糟」）落到每个 Claude Code 条目的 `default_effort: None` 上。它的文字文档把某一
  档称为默认；但文字不是机器可读的事实，而一个报错默认值的 picker 比一个不报默认值的更糟。
- **O3 —— 激活中的连接不再抹掉档位。** 2026-09-11c 修订让激活连接的策展 id 成为 picker
  得到的全部答案，而那个答案是以裸 id 重建的——于是连接一旦激活，每个 effort 控件都会静默
  清空。id 该由连接回答；**档位**不该，因为无论指向哪个端点，turn 依然跑在 agent 自己的
  运行时上。所以 agent 也报告过的 id 保留它的档位，agent 从没听说过的 id 则没有档位。
  label 与 description 仍然是连接的事（依旧为空），因此 2026-09-11c 是被收窄而不是被推翻。
- **O4 —— 再有两个接口面提供这个选择，且对两个 agent 都成立。** 网页 Chat 页面的**草稿**
  栏获得已打开会话早已有的 effort picker（spec channels FR-078）——第一条 turn 最值得定调，
  而等会话存在时它已经在跑了——渠道则获得 `/effort`，在各方面都是 `/model` 的姊妹命令
  （spec channels FR-017）。N4 的「Coffer 不校验任何档位」在两者上都不变：档位原样透传，
  而 `xhigh` 落在做不到的模型上会在 Claude 自己的运行时里降级——那正是这个决定该待的地方。

**修正：** N4 的最后一句。**不变：** N2（与模型并列，逐会话存储）、N3（Codex 在 turn 上
施加它）、J3/M3（原样透传，不写死任何名字、也不写死任何档位）。
