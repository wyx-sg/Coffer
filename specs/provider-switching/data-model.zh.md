# 数据模型——Provider Switching

> English: [data-model.md](./data-model.md)

provider 注册表的实体、字段与复用锚点。
依赖 spec agent-registry 的 agent kind 和 spec mcp-gateway 的 kind-agnostic Resource 框架。
...
模型选择不在连接上——`models` 只是裁剪菜单，不做选择。投射给某 agent 的模型来自该 agent 的绑定（Agent 页双槽 →
