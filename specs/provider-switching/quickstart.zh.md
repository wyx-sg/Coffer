...
- 至少已注册一个 agent（自动检测或通过 `coffer agent add`——参见 spec agent-registry 快速上手）。
...
**激活 profile 报告 `skipped: ["codex"]`**——Coffer 中尚未注册 Codex agent。通过 `coffer agent add`（参见 spec agent-registry）或 Web Agents 页注册，profile 仍会标记为活跃。
