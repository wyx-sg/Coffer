...
These are pinned in [Capability State Model](../../docs/decisions/capability-state-model.md) and [Session Subprocess Model](../../docs/decisions/session-subprocess-model.md); concrete protocol semantics are listed here so the implementation has a single reference point.
