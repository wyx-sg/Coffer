...
| `psutil`              | `>=5.9`                              | 孤儿子进程清理所需的 PID 存活性 + 命令行验证（[Detect-or-Spawn](../../docs/decisions/daemon-detect-or-spawn.md)）。考虑过仅 POSIX 的 `os.kill(pid, 0)`——被否决，因为孤儿清扫需要确认该进程确实是 coffer 拉起来的 MCP 服务器，而非偶然复用了同一 PID 的无关进程。 |

## MCP protocol — concrete forwarding decisions

以下决策固化在 [Capability State Model](../../docs/decisions/capability-state-model.md) 和 [Session Subprocess Model](../../docs/decisions/session-subprocess-model.md)；具体协议语义集中在此，便于实现时统一参照。
...
- Coffer ADRs in `docs/decisions/`: `resource-framework-upfront`,
  `code-layout-layer-first`, `resource-identifier-format`,
  `capability-state-model`, `session-subprocess-model`, `daemon-detect-or-spawn`.
