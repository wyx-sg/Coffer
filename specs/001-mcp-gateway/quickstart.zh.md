...
markdown 树（`knowledge/`、`memory/`、`skills/`）是事实源；`coffer.db` 只是其上可重建的索引。Coffer 不再自带备份命令——全部事实源都经由 spec 010 的 vault 导出流转；若要逐字节拷贝整个目录，在 daemon 停止的前提下执行 `cp -r ~/.coffer/ <dest>` 即可。
