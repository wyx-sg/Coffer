...
构建 coffer MCP 网关 (gateway)：一个本地 daemon，聚合上游 MCP 服务器，并通过带命名空间的统一界面把它们的工具 (tool)、资源 (resource)、提示词 (prompt) 重新暴露给一个或多个 MCP 客户端（Claude Desktop / Cursor / Claude Code）。配套交付 CLI、桌面 UI、stdio shim 以及单一打包安装器。

本次实施同时把 kind-agnostic 的 Resource 框架铺好——首个具体 kind 是 `mcp_server`，后续 spec 再加更多。

面向用户的契约见 [./spec.md](./spec.md)；架构理由见七个 [ADR](../../docs/decisions/)。

## Technical Context

| Dimension                               | Value                                                                                                                                                                                                             |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Language / Version**                  | Python 3.12+, TypeScript 5.x, Rust stable（仅用于 Tauri 壳）                                                                                                                                                      |
| **Primary Dependencies** (本 spec 新增) | SQLAlchemy 2 + aiosqlite + Alembic, Typer + Rich, structlog, keyring, httpx, `mcp` SDK (官方), psutil。前端：TanStack Query, React Router 6, i18next, react-hook-form + zod, openapi-typescript + openapi-fetch。 |
| **Storage**                             | SQLite，位于 `~/.coffer/coffer.db`，WAL 模式，daemon 是唯一写入方                                                                                                                                                 |
| **Testing**                             | Coffer 的 4 层模型：unit / integration / contract / e2e。`pytest` + `vitest` + `Playwright`。Acceptance markers 把测试与 [spec.md](./spec.md) 中的 scenario 绑定。                                                |
| **Target Platforms**                    | macOS arm64+x64 (universal), Windows x64, Linux x64+arm64                                                                                                                                                         |
| **Project Type**                        | 桌面应用 + CLI + daemon（多进程 local-first）                                                                                                                                                                     |
| **Performance Goals**                   | 按 [spec.md](./spec.md) SC-003：每次工具调用网关开销 ≤ 50 ms（100 次采样的中位数）。daemon 冷启动 ≤ 5 s 进入 ready。                                                                                              |
| **Constraints**                         | Local-first（仅 127.0.0.1）；凭据仅存 OS keychain；分层架构（importlinter contracts 1–6）；单文件 ≤ 400 LOC 后端 / 250 前端组件。                                                                                 |
...
| **Languages**                             | ✅         | 仅 Python 3.12 + TS 5。所用 Rust 即 Tauri 壳（按 `agents/stack.md` 已符合宪法）。                                                         |
...
backend/
├── coffer-daemon.spec                    # PyInstaller daemon spec (见 ADR-007)
└── coffer-mcp-shim.spec                  # PyInstaller shim spec (见 ADR-007)

# 分发
Makefile  bundle-binaries                 # 调用 scripts/build_binaries.sh
```

### 未来 (由计划中的 UI 规范承担——将作为 `002-ui-shell` 在后续 PR 中起草)

下列内容**不**由本 PR 交付，仅出于前瞻性可见性列出：

```text
frontend/src/                              # React + Vite 桌面 UI（计划中）
...
desktop/                                   # Tauri 2 外壳（计划中）
├── src/{main,lib,daemon_supervisor,tray,window}.rs
├── tauri.conf.json                        # daemon + shim 的 sidecar 接线
└── icons/

scripts/
├── build_binaries.sh                      # 已交付 (bundle-binaries 驱动)
└── smoke_test_bundle.sh                   # 计划中 (post-build 矩阵 smoke test)
.github/workflows/
└── release.yml                            # 计划中 (跨平台构建矩阵)
```
...
**MVP gate:** Phase 3 之后，即使桌面 UI 还没有，也可通过 `pip install -e ./backend`
（以及 Phase 6 的 PyInstaller 二进制）把此功能交付早期用户。这是刻意的 MVP 切线。

### Phase 4 — US4：可观测性（audit + invocations + retention CLI）

audit 日志、invocation 日志、retention 框架本身在 Phase 2 作为 kind-agnostic
组件落地；Phase 4 完成它们的 CLI 接口（`coffer audit …`、`coffer mcp invocations …`、
...
### Phase 5 — US3：CLI 完备性（与 REST 对齐）

很多 CLI 子命令在 Phase 2（`resource`、`daemon`、`keychain`）和 Phase 3
（`mcp`）已经落地。Phase 5 补两阶段留下的缺口——通常是 `--json` 输出开关、
...
### Phase 6 — 分发（PyInstaller daemon + shim）

daemon + shim 的 PyInstaller spec（`backend/coffer-daemon.spec`、
`backend/coffer-mcp-shim.spec`），以及 `make bundle-binaries` 驱动
（`scripts/build_binaries.sh`）。单一桌面安装包（Tauri sidecar 接线、
GitHub Actions 发布矩阵、post-build smoke test）延后到计划中的 UI 规范
（将作为 `002-ui-shell` 在后续 PR 中起草），因为 Tauri sidecar 配置本身
就属于那份规范。

**Done when:** `make bundle-binaries` 在宿主平台上产出可工作的
`coffer-daemon` + `coffer-mcp-shim`，二者都能在干净的 `~/.coffer/`
上到达 `status: ready`。跨平台发布矩阵由计划中的 UI 规范承担。
...
_(系统托盘 / 开机启动 / 观测 UI / i18n 延后到计划中的 UI 规范——将作为
`002-ui-shell` 在后续 PR 中起草。)_
...
| 同时提供 stdio shim 与 HTTP `/mcp` 端点                                                                     | MCP 客户端生态分裂——主流客户端只支持 stdio 配置，新一批偏好 HTTP。                                          | 仅 stdio 会失去远端 MCP 服务器和 daemon HTTP 原生形态；仅 HTTP 会失去 Claude Desktop 等仅支持 stdio 的客户端。 |
...
| PyInstaller 打包（[ADR-007](../../docs/decisions/ADR-007-distribution-pyinstaller-tauri-sidecar.md)）       | spec 承诺 "装一个包就能用"；要求系统 Python 会把大部分目标用户挡在外面。                                    | "要求 Python 3.12" 违反 SC-009，并把多数非 Python 背景的开发者用户挡在门外。                                   |
