...
**Created**: 2026-05-20
**Status**: Accepted
**Scope note**: 本 spec 仅覆盖后端层——daemon、MCP 网关 (gateway)、REST API 以及 `coffer` CLI。所有面向用户的界面 (web UI、Tauri 桌面壳、视觉语言、信息架构、国际化) 延后到计划中的 UI 规范（将作为 `002-ui-shell` 在后续 PR 中起草）。
**Input**: 用户描述: "Coffer's first feature — an MCP server gateway. Like mcpjungle / metamcp: one MCP client (Claude Code / Claude Desktop / Cursor) connects to coffer; coffer aggregates many upstream MCP servers and re-exposes their tools, resources, and prompts as a single namespaced surface. Coffer will later manage other resource kinds (skills, memory, channels, agents) — design the first feature on top of a generic Resource framework so follow-on kinds plug in cleanly."

## User Scenarios & Testing

### User Story 1 — Aggregate multiple MCP servers in one client (Priority: P1)

某位开发者使用 Claude Code（或 Claude Desktop、Cursor），希望把若干 MCP 服务器（filesystem、GitHub、Postgres……）接入其中。与其每次列表变动都去编辑各客户端的 MCP 配置并重启，不如**只在 coffer 中注册一次**上游服务器 (upstream server)，然后让每个客户端都指向 coffer 提供的同一个 shim。所有上游的工具 (tool)、资源 (resource) 和提示词 (prompt) 都会在各个客户端中以带命名空间的方式出现，互不冲突。
...
- **Daemon crash**: 正在运行的 shim 会话会给它们的 MCP 客户端返回干净的错误而不是挂起；监管者 (supervisor) 可以检测崩溃并重启 daemon（桌面端的 supervision 延后到计划中的 UI 规范——将作为 `002-ui-shell` 在后续 PR 中起草）。
- **Concurrent clients**: 多个 MCP 客户端（例如 Claude Code 和 Cursor 同时）可同时连接而互不干扰；每个客户端都拿到独立的上游子进程集合。
- _(首次安装 / 启动器可发现性属于 UI 范畴，将在计划中的 UI 规范里跟踪。)_
...
_(FR-018–FR-020 (desktop app, system tray, i18n) 延后到计划中的 UI 规范（将作为 `002-ui-shell` 在后续 PR 中起草）。)_

_(桌面端、系统托盘、开机启动以及 UI 国际化由计划中的 UI 规范承载。)_

**Distribution**

- **FR-021**: 终端用户的安装路径 MUST 产出一个可工作的 coffer daemon 和 shim，无需用户自行安装 Python 或其他运行时。（桌面端的打包由计划中的 UI 规范——将作为 `002-ui-shell` 在后续 PR 中起草——承载。）
...
- **SC-004**: 两个 MCP 客户端（例如 Claude Code 与 Cursor）同时连接时，在 30 分钟的交互会话中都能成功 discover 并调用工具，互不干扰。
...
- **SC-009**: 一台干净的机器（macOS、Windows 或 Linux，没有装 Python）的用户可从单一发行包安装 coffer，并在仅点击安装器之外没有任何手工步骤的情况下到达 daemon 的 "ready" 状态。
- **SC-010**: 任何上游服务器凭据值都不应出现在任何数据库表、日志文件、audit 条目或 invocation 记录里——以一次代表性会话之后对这些产物的自动扫描验证。

## Assumptions

- 单一用户在自己机器上运行 coffer。不存在多租户或远程访问需求。
- MCP 客户端（Claude Code、Claude Desktop、Cursor 等）至少支持 `stdio` 或 `http`/`sse` 中的一种 MCP 服务器配置；coffer 两种入口都提供。
...
- 本 spec 不包含付费的 Apple notarization；macOS 用户首次启动时可能需要手动清除 quarantine。
