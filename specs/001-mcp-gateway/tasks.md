...
**File-size guard**: backend files ≤ 400 LOC, frontend pages ≤ 200 LOC, components ≤ 250 LOC, hooks ≤ 300 LOC. `scripts/check_file_sizes.py` enforces.
...
### T002 [P] Add frontend dependencies

**Files:**

- Modify: `frontend/package.json`

- [x] **Step 1: Add deps in `dependencies` and `devDependencies`**
...
git commit -m "build(mcp-gateway): add frontend dependencies"
```

---

### T003 Add importlinter contracts 5 (cross-kind isolation) and 6 (kind-agnostic core)
...
### T004 [P] Structured logging setup (structlog + trace_id contextvar)
...
### T005 SQLAlchemy declarative base + async engine factory
...
### T007 HTTP error envelope + global exception handler
...
### T008 X-Coffer-Token auth dependency + CORS allowlist
...
### T009 Daemon discovery (`~/.coffer/daemon.json`) + port allocator
...
### T010 FastAPI composition root + `/api/v1/daemon/status`
...
### T011 Typer CLI scaffold + `_client.py`
...
### T012 [P] `coffer daemon start/stop` (background daemon process)
...
### T013 [P] Acceptance audit script kept passing through Phase 1
...
### T014 Phase 1 checkpoint commit
...
### T015 [TDD] `ResourceRef` value object
...
### T016 [TDD] `Resource` entity + `Kind` descriptor + `KindModule`
...
### T017 [TDD] `AuditEntry` + `AuditEventType`
...
### T018 [TDD] `RetentionPolicy` entity
...
### T019 [TDD] Repository protocols (kind-agnostic)
...
### T020 [TDD] SQLAlchemy ORM models (kind-agnostic core tables)
...
### T021 Alembic migration `0001_initial`
...
### T022 [TDD] `SqlAlchemyResourceRepo`
...
(Code structure mirrors test cases; ≤ 250 LOC. Full listing kept verbose in this plan would dwarf surrounding tasks — implementer follows the Protocol shape from T019 and the ORM from T020.)

---

### T023 [TDD] `SqlAlchemyAuditRepo`

Same shape as T022 against `AuditLogModel`. Tests: insert + query by kind / name / event_type / since / limit.

---

### T024 [TDD] `SqlAlchemyRetentionRepo`

Same shape as T022 against `RetentionPolicyModel`. Tests: `upsert`, `update_retention` (None and positive int), `touch_pruned`, `delete_older_than` (SQL injection guard: table name not in `_ALLOWED_TABLES` raises `SecurityError`).
...
### T026 [TDD] `ResourceService.register` + audit integration
...
### T027 [TDD] `ResourceService.list/get/update_config/set_enabled/delete`
...
### T028 [TDD] `PrunableTable` + `PrunableRegistry`
...
### T029 [TDD] `RetentionService` (initialize_defaults, list, set, prune)

Same TDD shape. `prune` test must include the SQL-injection guard case from T024 ("table name not in allowlist raises").

---

### T030 [TDD] `RetentionWorker` background task
...
### T031 [TDD] Pydantic API schemas matching `contracts/api.openapi.yaml`
...
### T032 [TDD] `/api/v1/resources/*` routes — kind-agnostic CRUD
...
### T035 [TDD] `/api/v1/daemon/*` routes — backup / rotate-token / shutdown
...
### T036 [TDD] CLI subcommand groups (kind-agnostic)
...
### T037 Composition root: register `fake_kind` in test mode, real kinds in prod
...
the feature is shippable to early adopters via `pip install -e .` even though
the desktop UI does not yet exist.

**Checkpoint**: User Story 1 + User Story 2 acceptance scenarios pass in `e2e/mcp/`.

---

### T039 [TDD] [US1] `fake_mcp_server.py` fixture
...
### T040 [TDD] [US1] MCP `Namespace` functions (pure)
...
### T041 [TDD] [US1] `MCPServerConfig` (Pydantic discriminated union)
...
### T042 [TDD] [US1] `MCPTool` / `MCPResource` / `MCPPrompt` value objects
...
### T043 SQLAlchemy ORM models for the MCP kind tables
...
### T044 Alembic migration `0002_mcp_tables`
...
### T045 [TDD] [US1] `MCPCapabilityPreferenceRepo` + `MCPInvocationRepo`

Pattern follows T022–T024.

---

### T046 [TDD] [US1] `CredentialResolver` (keychain → env / header overlay)
...
### T047 [TDD] [US1] `StdioUpstreamConnection` (spawn + initialize)
...
Tests use `fake_mcp_server.py` fixture with various scenarios (`basic`, `slow` for timeout, `crash` for spawn-then-crash). Each scenario is one task — split into T047-a, T047-b if needed to stay 2-5 min apiece.

---

### T048 [TDD] [US1] `StdioUpstreamConnection.request / on_notification / close`
...
### T049 [TDD] [US1] `HttpUpstreamConnection`
...
### T050 [TDD] [US1] `SubprocessSupervisor` (lazy spawn + health state machine)
...
### T051 [TDD] [US1] Orphan PID file tracking
...
### T052 [TDD] [US1] `CapabilityDiscovery.list_tools` (live + cache + preferences reconciliation)
...
### T053 [TDD] [US1] `CapabilityDiscovery.list_resources` + `list_prompts`

Same pattern — split for granular review.

---

### T054 [TDD] [US1] `MCPGatewaySession.initialize` (negotiate downstream + upstream capabilities)
...
### T055 [TDD] [US1] `MCPGatewaySession.handle_request`: dispatch `tools/list`, `resources/list`, `prompts/list`
...
### T056 [TDD] [US1] `MCPGatewaySession.handle_request`: dispatch `tools/call`
...
### T057 [TDD] [US1] `MCPGatewaySession.handle_request`: dispatch `resources/read` + `prompts/get`

URI / name parsing per `Namespace.parse_prefixed_uri` and `parse_prefixed_prompt`. Returns upstream payload unchanged.

---

### T058 [TDD] [US1] Capability disabled rejection
...
### T059 [TDD] [US1] Upstream → downstream notification forwarding
...
### T060 [TDD] [US1] `progressToken` passthrough + idle timer reset
...
### T061 [TDD] [US1] `sampling/createMessage` pass-through with capability negotiation
...
### T064 [TDD] [US1] `MCP_KIND` composition wire-up

> **Ordering**: this task depends on T065–T071 (the router and CLI files it
...
### T065 [TDD] [US1] HTTP `/mcp` JSON-RPC endpoint (POST)
...
### T066 [TDD] [US1] `/mcp` SSE channel (GET)
...
### T067 [TDD] [US1] MCP CRUD routes — `/api/v1/resources/mcp_server/*`
...
### T068 [TDD] [US1] CLI: `coffer mcp add/remove/list/show/refresh/test`
...
### T069 [TDD] [US1+US2] CLI: `coffer mcp tool enable/disable/list`
...
### T070 [TDD] [US1] CLI: `coffer mcp resource enable/disable/list` + `coffer mcp prompt enable/disable/list`

Same shape as T069 with `capability_type=resource` and `capability_type=prompt`. Two tasks; commit each.

---

### T071 [TDD] [US1] CLI: `coffer mcp invocations`

Calls `/api/v1/resources/mcp_server/{name}/invocations`. Rich table output (default) and `--json` (script-friendly).

---

### T072 [TDD] [US1] `coffer-mcp-shim` binary — connect
...
### T073 [TDD] [US1] `coffer-mcp-shim` graceful shutdown
...
### T074 [TDD] [US1] Contract test: coffer's `/mcp` against the official MCP SDK
...
### T075 [TDD] [US1] e2e: shim → daemon → fake upstream round trip
...
### T076 [TDD] [US1] e2e: per-tool disable hides tool from clients
...
### T077 [TDD] [US1] e2e: list_changed roundtrip
...
### T078 [TDD] [US1] e2e: concurrent clients work without interference
...
### T079 Phase 3 checkpoint + MVP gate
...
## Phases 4 – 9 — Detailed task lists deferred until after MVP
...
### Phase 4 — User Story 3 (P2): Desktop UI

- T-100 Generate TypeScript types from `contracts/api.openapi.yaml`
- T-101 OpenAPI-fetch client wrapper + auth header injection
- T-102 TanStack Query setup + provider + DevTools
- T-103 React Router routes for `/resources`, `/resources/:kind/:name`, `/settings/*`
- T-104 `ResourceListView` kind-agnostic component + `kindRegistry`
- T-105 `MCP_KIND_UI` registration in `frontend/src/kinds.ts`
- T-106 `McpServerListPage`
- T-107 `McpServerAddPage` (react-hook-form + zod, stdio + http discriminated)
- T-108 `McpServerDetailPage` + tabs (tools / resources / prompts / invocations)
- T-109 `CapabilityList` component with enable/disable toggles
- T-110 `InvocationsTable` component
- T-111 `HealthBadge` component
- T-112 `AuditPage`
- T-113 Settings pages (retention, daemon, about)
- T-114 Acceptance tests in `e2e/web/specs/`

### Phase 5 — User Story 4 (P2): CLI completeness

- T-115 `--json` flag on every list/show command
- T-116 `--verbose` traceback rendering
- T-117 Exit code map per error class
- T-118 `coffer keychain set/get/list` (credential management)
- T-119 e2e tests covering "command line surfaces same errors as desktop"

### Phase 6 — User Story 5 (P3): Tray + close-to-tray + autolaunch

- T-120 Add `tauri-plugin-autostart` to `desktop/Cargo.toml`
- T-121 Tray icon + menu in `desktop/src/tray.rs`
- T-122 Close-to-tray window handler in `desktop/src/window.rs`
- T-123 Settings toggles in frontend that drive Tauri commands
- T-124 Cross-platform smoke tests (macOS / Windows / Linux)

### Phase 7 — User Story 6 (P3): Observability + i18n

- T-125 i18next bootstrap + `LanguageSwitcher` component
- T-126 `frontend/src/locales/en/*.json` + `zh/*.json` initial translations
- T-127 Error-code → translation-key mapping in `lib/api/client.ts`
- T-128 Audit + invocations + retention UI strings translated
- T-129 e2e tests for language switching

### Phase 8 — Distribution

- T-130 `backend/packaging/daemon.spec` + hidden-imports
- T-131 `backend/packaging/shim.spec`
- T-132 `desktop/tauri.conf.json`: enable `bundle.active`, declare `externalBin` sidecars
- T-133 First-launch shim deployment to per-user PATH (Tauri command)
- T-134 GitHub Actions matrix: macOS arm64+x64 universal, Windows x64, Linux x64+arm64
- T-135 Bundle smoke test job: launch binary, curl /status, kill
- T-136 README + quickstart updates for installer

### Phase 9 — Hardening

- T-137 Daemon graceful-shutdown signal handler
- T-138 Upstream health state-machine end-to-end test
- T-139 Bounded log rotation (`RotatingFileHandler` size + retention)
- T-140 Concurrent-client load smoke (3 simulated clients × 100 tool calls each)
- T-141 Backup-before-migrate verification test
- T-142 Gateway-overhead latency test (covers SC-003: 100 fast calls, assert median overhead ≤ 50 ms vs direct upstream)
- T-143 Credential-leak audit test (covers SC-010: grep every log/audit/invocation artifact after a credentialed session for keychain values; assert zero hits)
- T-144 Final acceptance audit: every Edge Case in [spec.md](./spec.md) has a covering test
...
- **Phase 3 → Phase 4**: Phase 3 finishes with a working but UI-less feature. Phase 4 builds the frontend against the now-stable contract.
- **Phase 3 → Phase 8**: PyInstaller bundling depends on the daemon + shim being fully wired (Phase 3 done) so the spec files have something to bundle.

### Parallelisation

Tasks marked `[P]` can be done concurrently by separate agents. The biggest
parallel band is in Phase 2: T020 (models), T031 (Pydantic schemas), and T036
...
uncovered by the end of Phase 9.
