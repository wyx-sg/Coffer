...
**This layer adds no table of its own** (FR-034, SC-002). Facts, digests and
...
Everything under `~/.coffer/memory/` is **derived** (FR-016). Delete it, run
...
└── coffer/                        # one partition per project, named from its root
    ├── README.md                  # names the absolute project root (FR-009)
    ├── summary.md
    └── facts/
        ├── python-lockfile.md
...
- `~/.coffer/memory/` is the root; `$COFFER_MEMORY_ROOT` overrides it for
  ...
  FR-036 asks for lives (`check_segment` refuses an empty, hidden, all-dots
...
and no other partitioning axis exists (FR-008).

| Field | Where it lives | Notes |
|---|---|---|
| `name` | the directory name, and the Resource's name | A readable slug from the project root's own directory name — `/home/dev/coffer` → `coffer`. A collision is resolved by prefixing a parent segment (`work-api` vs `personal-api`), never by an id (FR-009). |
...
| `scope` | the Resource's own `scope` column | On creation, the set of agents the partition was aggregated from, so memory flows back to its sources with no setup step. Narrowing or widening it afterwards is the developer's, and a later pass never overwrites it (FR-012). |
...
agent's working directory at read time (FR-011) — the `memory` Kind sets
...
| `type` | `user` \| `feedback` \| `project` | `user` and `feedback` are *personal*: they file into `global` whichever project they came from (FR-010). |
| `body` | string | **The source's own words, never a paraphrase** (FR-014). Summarising happens in the digest. |
| `partition` | string | `global` or a project slug. |
| `origins` | list of Origin | Every place the fact was seen. Two agents contributing the same fact produce one fact with two origins (FR-015). |
| `status` | `active` \| `superseded` | |
| `superseded_by` | fact key, or empty | Set by organise when a later fact contradicts an earlier one. |
| `conflicts_with` | list of fact keys | Organise's finding when it cannot order a disagreeing pair. Nothing settles it by hand (FR-020). |
...
own `config_dir`s (FR-001, FR-036), and is strictly read-only (FR-002). Two
readers, written as two readers (FR-037):
...
**What this layer may reach, and what it may not.** Two `forbidden`
import-linter contracts in `backend/pyproject.toml` separate this layer from
knowledge, and they are not mirror images. Knowledge's contract names all five
of this layer's modules, so nothing there reaches in here. This layer's
contract names only knowledge's **facade** — `application/knowledge`, its HTTP
routes and its CLI group — and deliberately leaves `domain/knowledge` and
`infrastructure/knowledge` importable, because path layout, frontmatter parsing
and the ripgrep wrapper are substrate every kind shares. This layer's contract
also carries two recorded `ignore_imports` waivers, for the agent vocabulary its
readers and delivery share with the agent registry; knowledge's carries none.
The waivers are tracked as something to retire by promoting that vocabulary to
kind-agnostic substrate, not as a reason to re-home a requirement.
...
and a surface mounting mid-pass can show the running pass (FR-033). A restart
...
vault-wide audit surface (FR-026, FR-030).

## Audit events

This layer carries no audit surface of its own (FR-031); its events are read
...
| `memory_delivery_fired` | an installed hook fires (FR-026) |
