...
- **FR-023**: `~/.coffer/memory/` 下的整棵树 MUST 是派生的：删掉它再重跑聚合 MUST 能重现它。但它仍然 MUST 被纳入导出和备份（spec [vault-export-import](../vault-export-import/spec.md)），因为它的源活在 vault 之外，扛不住这台机器的丢失。
