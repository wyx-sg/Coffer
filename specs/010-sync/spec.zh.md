...
- **Conflict** —— 一次 `git merge` 冲突。引擎以确定性的「最新者胜」策略**自动解决**（2026-07-10 修订）：逐冲突路径，最后触及该路径的提交较新的一侧获胜——与机器无关，因此每台机器选出同一个赢家；时间戳相同时保留执行合并那台机器的一侧；`manifest.json` 永远取本机一侧（schema 门已先行）。只有引擎无法处理的路径才会让运行停在 `conflicted`——此时 UI 指引用户到自己的仓库（如 GitHub）解决，应用内不提供合并界面；`coffer sync resolve` 保留为 CLI 兜底。落败一侧的内容不会丢失——它留在 vault 仓库的历史里。
...
### Scenario: 冲突的编辑自动解决为较新的一侧

- **Given** 机器 A 和 B 自上次共同同步以来都编辑了同一个资源/文件，且 A 已经推送
- **When** 机器 B 运行 `coffer sync`
- **Then** 该次运行把每个冲突路径自动解决为最后提交较新的一侧（相同则保留 B 侧），无需用户操作即完成，且两台机器在各自的下一次运行后收敛到同一个赢家
