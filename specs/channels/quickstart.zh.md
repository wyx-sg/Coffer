...
如果你的仓库与一个同步远端相互收敛，一条渠道会出现在每一台机器上——并且只在其中
一台上运行：**Runs on**（运行机器）那一列指名的那台。注册渠道时它就被绑定到了你
注册它的那台机器，所以只有想挪动它的时候才需要动这里。

```bash
coffer channel bind my-telegram              # 在这里跑
coffer channel bind my-telegram <machine-id> # 在那里跑（机器列表见 coffer sync machines）
```

也可以在 **Runs on** 那一列，或渠道自己的页面上直接选机器。

不需要重启。让出这条渠道的机器会在一两秒内停掉；接手的那台会在下一轮同步把变更带
过去之后启动。**从当前正在运行这条渠道的那台机器上**改绑，交接就没有重叠——反过来，
在另一台机器仍然持有它时把它绑到你正坐着的这台，两边都可能应答，直到那台机器完成
下一轮同步。

两件要知道的事：

- 绑定在一台已退役机器上的渠道哪里都不跑，而且会明说。把它绑到一台还活着的机器上。
- **Runs on** 不是 **Reach**。生效范围说的是这条渠道可以驱动哪些 agent，在每台机器上
  各自设置；绑定说的是哪台机器运行适配器，在所有机器上都是同一个答案。
