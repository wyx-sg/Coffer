...
- **FR-006**: `/new`、`/stop`、`/status`、`/help` 命令在任何已配对的聊天里
  ...
  待处理队列（FR-050——就是网页端显示的那条；channel 在 10 条等待之后拒收）
  并按序运行。这些是不需要会话本身的命令；配置聊天所绑定会话的那些——`/agent`、`/model`、`/effort`——规定在 FR-013 与 FR-017，`/save`
  规定在 spec knowledge FR-036。它们共用同一份命令清单（FR-065），因此从它派生出
  的那些列表没有一份会走样。
...
- **FR-011**: Channels 页面列出 channel、注册新 channel（secret 经凭据存储
  ...
  码、切换启用/停用，并把每条渠道绑定到运行它的那台机器（FR-080）。
  CLI 对等：`coffer channel list / register / bind / pair / status / notify`。
...
- **FR-017**: owner 从 chat 切换 model。`/model` 无参时报告当前 model；
  ...
  **`/effort` 是这个选择的另一半。** 对于 model 需要一个推理强度的 agent——Codex 放在
  `turn/start` 上，Claude Code 走 CLI 的 `--effort`——model id 并不是决定的全部，而强度
  也不是 model **名字**的一部分，所以它是一条自己的命令，而不是 `/model` 的一个参数
  （网页 Chat 页面在 model picker 旁边渲染第二个 picker、而不是把强度折进去，是同一个
  理由，FR-078）。它的行为与 `/model` 完全一致：无参时报告当前生效的强度，并在
  `supports_buttons` 的传输上把候选渲染成选择卡片；带参时应用到**同一个**会话的下条
  turn；点选走与文本相同的路径；Coffer 不做任何校验——强度原样递给 agent，因此一个它
  跑不了的强度会在 CLI 上失败，和其他任何不可用的选择失败在同一处。提供的强度是会话
  **当前所在**那个 model 的强度，读自 `/model` 提供的同一份 catalogue，因此强度菜单
  绝不会描述一个会话并没有在跑的 model。model 不带强度的 agent 没有任何可选项：
  `/effort` 用一行说清楚，而不是渲染一张空卡片。和 `/model` 一样，没有清除形式——
  一个意为「取消设置」的保留词会是 Coffer 在自己并不拥有的命名空间里发明出来的强度
  名——回到 agent 自身默认值的路是 `/new`，因为新开的会话根本不带任何覆盖项。
- **FR-018**: 在声明了 `supports_buttons` 能力的传输上，内核 MAY 把一个命令的
  ...
  路径**改写同一条消息**到下一个窗口。各张卡片共用同一条规则——`/agent` 的两个候选
  在上限之内，因此完全不带任何导航元素，`/effort` 的那几个强度同理。

  导航载荷有自己的 callback 命名空间（`page:<kind>:<index>`），与候选所用的
  `agent:` / `model:` / `effort:` 值互不相交，且长度固定、远在 64 字节预算之内。正是这份隔离
  保证了那条不变量：**翻页什么都不改。**那个命名空间接受的 kind 集合是封闭且显式
  的，因此新增一张会勾选当前项的卡片（`/effort` 的是第三张）也必须把它的 kind 加进去；
  一个命名了 Coffer 并不渲染的 kind 的 `page:` 值会被丢弃，而不是被应用。它只重读当前
  生效值并重新渲染；它永远不可能
...
- **Channel** — 资源 `channel:<name>`；config = 类型、凭据 ref、默认
  agent + 配置，以及 `runs_on`——唯一那台由其 daemon 运行这条渠道适配器的机器的
  `machine_id`（见「渠道在哪里运行」）。
...
> **修订 2026-09-14（渠道会走，并指名运行它的那台机器）。** 推翻此前「`channel`
> 不参与收敛」那条规则。那条规则把危险说对了，把对策说错了：它不让文档走，理由是
> 到达的副本好一点是完全不起作用、差一点就是多出一个抢事件的消费者——然后它就再也
> 没有任何地方能说清那个唯一活着的消费者到底是哪台机器。下面写的是换一条路达成的
> 同一份保证：文档会走，而且它指名自己的机器。spec vault-sync 有配套的修订。

渠道的平台身份——被轮询的 bot、webhook 端点、一条长连的 WebSocket——只容许**一个**
消费者。所以「哪台机器来应答这个 bot」必须只有一个答案，而这个答案是写下来的：
**一条渠道绑定到一台机器**，只有被绑定那台机器的 daemon 会为它启动适配器。

这个绑定就是 `runs_on`，一个 `machine_id`（spec vault-sync，「身份是派生的」），
放在渠道自己的配置里。它是配置，而不是资源行上的属性，这个选择是承重的：配置正是
一份 resource 文档在机器之间携带的东西，所以绑定随它所绑的渠道一起走。每台持有该
文档的机器读到的都是同一个名字，而除了一台之外的每台机器都会发现被指名的不是自己。

### 生效范围与机器绑定回答的是不同的问题

它们很容易混，而且绝不能合并。

| | 生效范围（`enabled` + `scope`） | 机器绑定（`runs_on`） |
| --- | --- | --- |
| 问题 | 这条渠道**可以驱动哪些 agent**，以及它在这里是不是活的 | **哪台机器**运行适配器 |
| 会外传吗？ | 不会——每台机器各设各的，就留在那台机器上 | **会**——它是整个 vault 共用的一个答案 |
| 放在哪 | 资源行上 | 渠道的配置里 |

渠道的 `scope`
（[Per-Agent Resource Scope](../../docs/decisions/per-agent-resource-scope.zh.md)）
仍然只说一件事，而且仍然与机器无关（FR-079）：它命名的是这条渠道**可以驱动哪些
agent**。把绑定安置进 `scope`，会让「这在哪里生效」背上两个不相干的答案；而生效范围
上的机器轴当初被撤掉，恰恰是因为每台机器靠持有自己的生效范围就已经说清了它要激活
什么。绑定是相反性质的事实：它是各机器共用的一个决定，所以它外传，而生效范围不。

### runtime 拿它做什么

runtime 只为绑定到它所在那台机器的渠道启动适配器。其余情况一律**失败即关**——什么
都不启动——三种情形的区别体现在各接口面怎么报，而不是 runtime 怎么做：

- **绑定到另一台机器。** 正常。这条渠道本来就不归本机跑，各接口面也这么说，于是
  一条在本机安静的渠道，不会看起来像一条在本机崩了的渠道。
- **绑定到注册表不认识的机器**——已退役的，或者从没见过的。报成一处故障，因为它
  确实是：这条渠道哪里都不跑，而且除了改绑没有任何东西能修好它。它**不会**在本机
  被启动。「既然没人认领那就我来跑」正是抢事件那个故障从后门溜回来：每一台解析不
  出这个 id 的机器都会推出同样的结论，然后它们会全都启动。
- **未绑定**（`runs_on` 缺失或为 null）。同样哪里都不跑，同样要报出来。一份谁都没
  指名的文档，在每一台持有它的机器上含义都一样，所以「跑它」只可能意味着「在所有
  机器上跑它」。经 Coffer 任何自有接口面注册的渠道，在创建时就被绑定到注册它的那台
  机器，所以「未绑定」是用户通过导入或手工改文件才会走到的状态，不是正常使用会掉
  进去的。

两个适配器当然仍有可能指向同一个 bot 身份，但只剩下它们一直以来就有的那条路——有人
手工把同一个 bot 用两个名字注册了两遍——而这一条，Coffer 里放任何字段都拦不住。

### 改绑

把 `runs_on` 从机器 A 改成机器 B 是一次普通的配置编辑，收敛方式也是普通的那套——
不用重启，也没有任何一条命令会伸到另一台机器上去。

- **失去**这条渠道的那台机器，在它看见这次变更之后的下一个 reconcile tick 停掉自己的
  适配器：在做出这次编辑的那台机器上大约两秒，在另一台机器上则是把变更带过去的那轮
  收敛之后的一个 tick。
- **得到**它的那台机器，在同一轮收敛把文档送达之后的下一个 tick 启动一个。

所以交接一条渠道，干净的做法是从当前持有它的那台机器上改绑：停是立刻的、本地的，
启动发生在之后、在别处。在另一台机器还持有这条渠道时，从自己正坐着的这台机器把它
改绑过来也是允许的，而且有时是唯一的选择——坏掉的可能恰恰就是那台老机器——但这会
开出一个窗口，长度是那台机器的同步间隔，期间两个适配器都是活的。提供改绑的那个
接口面会把这一点讲明白。

### 配对随渠道一起走

渠道的 peer 配对属于同步状态（spec vault-sync，`## What syncs`），因为一条渠道走了
却不带配对，会让属主每次它移动都从手机上重新配对一遍，而改绑本该只是一次点击。随之
外传的是平台身份——chat id、sender id、显示名、该聊天粘住的 agent。当前会话指针不
外传：会话是本机专属的，发布出去的指针指名的会是对面并不拥有的那段会话。
...
- **Given** 一个已启用、绑定到另一台机器 `machine_id` 的 channel
- **When** runtime 做 reconcile
- **Then** 本机不启动任何 adapter，而管理接口面把这条 channel 报成绑定在别处，
  而不是报成已停止

### Scenario: a channel bound to an unknown machine starts nowhere

- **Given** 一个已启用的 channel，其绑定指名了一台注册表并不持有的机器
- **When** 任意机器上的 runtime 做 reconcile
- **Then** 没有任何机器为它启动 adapter，这条 channel 被报成绑定到了一台已经不
  存在的机器

### Scenario: an unbound channel runs nowhere and says so

- **Given** 一个已启用、配置里不带任何绑定的 channel
- **When** runtime 做 reconcile
- **Then** 不启动任何 adapter，而该 channel 的状态带一条诊断，点明缺失的绑定以及
  该怎么修

### Scenario: rebinding hands the channel over without a restart

- **Given** 一个已启用、正在本机运行的 channel
- **When** 用户把它绑定到另一台机器
- **Then** 本机在下一个 reconcile 停掉自己的 adapter，无需重启 daemon，而这条
  channel 的绑定正是下一轮收敛要发布出去的东西
...
- **Given** 一个已配对的渠道，它的每个 turn 都以一段说明消息来源的上下文块开头，
- **When** 一个新会话的第一条消息从该渠道到达，
- **Then** 会话以这个人真正打出来的字命名，而不是以每个聊天的每个 turn 都相同的
  那段上下文块命名。
...
- **Given** 一条正在运行的渠道，其默认 agent 以自己的名字注册成了一个资源，
- **When** 属主把渠道的 scope 收窄到那个资源名——也是生效范围控件唯一提供的名字，
- **Then** 这次编辑被接受，渠道继续运行，而这条 scope 抵达 `/agent` 时已被翻译成那个
  面所说的 agent key。
...
- **FR-048**: 系统必须把会话及其消息持久化在 SQLite 里作为事实来源；它们不建模为
  ...
  这个名字必须取自那条消息里人真正写下的部分：渠道 turn 的文本开头是渠道折进去的
  上下文块（FR-042 来源块、FR-029 线程历史），它们在每个聊天的每个 turn 里都一模
  一样，所以拿整段文本命名会让所有渠道会话叫同一个名字。知道自己的块到哪里结束的是
  渠道这一层，因此由它把人写的那部分显式传下去；聊天平台不得自己去识别块的格式。
  当人一个字都没写（只发了一张图或一个文件）时，用附件文件名来命名会话——那是这类
  消息在列表里唯一能被人认出来的部分；若连可命名的东西都没有，会话保留占位标题，
  而不是拿样板文字来命名。
...
- **FR-064**: 不是答案的絮语在群里保持私有。命令输出和错误是说给某一个成员听的，不是
  ...
  每条命令都在 FR-065 用来注册菜单的那份清单上声明自己落在这条线的哪一侧：`/new` 与
  `/stop` 改的是整个房间共享的状态，保持可见；而回答提问者本人的那些——`/agent`、
  `/model`、`/effort`、`/status`、`/help`——在传输支持时私密投递。新增命令若不作声明
  则默认为私密，这是保险的那一侧，但它仍然是清单明确写出的决定，而不是新命令碰巧
  继承来的。
...
- **FR-065**: bot 要做自我介绍。它的命令菜单从 Coffer 自己的命令清单注册到平台上，
  ...
  命令——帮助文本提供而菜单遗漏的命令是漂移 bug，不是设计取舍。这一点靠结构而不是靠
  评审来保证：菜单、帮助文本以及每条命令的私密标志（FR-064）都由同一份清单渲染，
  因此新增一条命令（最近的一条是 `/effort`）就是一个条目加它的处理分支，没有第二份
  列表可供遗忘。owner 已经写好的文案
...
- **FR-078**: 页面必须让属主经 `GET|PATCH .../agent-config` 读取与设置会话的 agent 配置——
  它跑在哪个 agent 上、那个 agent 被放在哪个模型上、以及那个模型想得多用力——在持久化这
  两者的同时保住会话的工作目录与上游 session id，任一被清空则回落到 agent 自己的默认；
  只提到其中一个的请求体不动另一个。推理强度是 model picker 旁边的**第二个**控件，
  而不是它的一个变体：agent 把它当作自己的字段，而不是模型名字的一部分；它只在所选
  模型报告了强度时才渲染——没有可选项就是根本不出现控件，而不是出现一个禁用的或空的。
  这两个控件在**草稿**面上也必须提供，而不只是在已打开的会话里，因为第一条 turn 恰恰
  是用户最想定调的那一条，而等会话存在时那条 turn 已经在跑了。缺少 Coffer 的 LLM 连接**不得**挡住这个
...
- **FR-079**：渠道必须承载框架级的 `scope`
  （[Per-Agent Resource Scope](../../docs/decisions/per-agent-resource-scope.zh.md)），
  即一条 agent 允许列表，读法是**这条渠道可以驱动哪些 agent**。其他所有 kind 的 scope
  命名的是资源被*投递给*哪些 agent；而渠道不被任何 agent 消费——它是一个入站面——所以
  这条列表是被反转，而不是照搬。这里明确写出来，因为按惯常读法理解的读者会正好读反。
  这个反转后的读法就是渠道 scope 的全部：里面没有第二样东西，尤其没有关于**哪台机器**
  运行这条渠道的任何说法。那个问题有它自己的字段 `runs_on`（FR-080），理由是 scope 满足
  不了它：scope 是生效范围，生效范围是本机专属、永不外传的，而运行一条渠道的那台机器是
  各机器必须共用的同一个答案。

  - scope 不加限制时必须表示所有已注册的 agent。这就是 scope 之前的行为，也是现存每一
  ...
  - **一个 agent 的两个名字。** scope 命名的是 agent **资源**——属主给它起的任何名
    字——因为那是生效范围选择器唯一拿得出来的东西，也是资源表里存的东西。而
    `default_agent`、`/agent` 以及 turn 路由说的都是 turn 平台据以路由的 agent
    **key**。两者之间的每一次比较都必须在**同一套**词汇里进行，经 agent 注册表翻译。
    直接比较这两个字符串，会拒掉用户唯一能表达的那种收窄，转而只接受一个生效范围选择
    器随后不得不渲染成「未在此注册」的值——所以这条不是实现细节：它决定了生效范围控件
    是能用，还是只会提供它自己不肯接受的东西。同一类型的两个 agent 资源会归并成同一个
    key；而 scope 命名了这个 vault 并不持有的资源时，它容许不了任何 agent。
  - **不变量**：渠道的 `default_agent` 必须是这条渠道可以驱动的 agent——只要 scope 非
    空，它就必须落在 scope 之内。这条不变量必须在**两条写入路径上同时**强制，从而让不一致
    的状态根本无法落库：编辑配置时，若它命名了当前 scope 之外的 `default_agent`，该编
    辑被拒绝；编辑 scope 时，若被提议的非空 scope 排除了当前的 `default_agent`，该编辑
...
- **FR-080**：渠道必须指名运行它的那一台机器。它的配置携带 `runs_on`，即由其 daemon
  启动这条渠道适配器的那台机器的 `machine_id`（spec vault-sync，「身份是派生的」）。
  它是配置而不是资源行上的属性，因为它**必须**随渠道文档一起走；生效范围**不得**外传，
  也**不得**被拿来承载这件事（见「渠道在哪里运行」）。

  - 这个绑定必须是适配器启动的唯一权威。runtime 只能为 `runs_on` 等于本机 id 的渠道
    启动适配器，其余情况必须**失败即关**：不认识的机器、已退役的机器、以及根本没有
    绑定，三者都表示「不是这台机器」，其中任何一个都不得被读成启动的许可。凭猜测启动
    是收不回来的——平台那边已经被应答了两次。
  - **未绑定必须哪里都不跑**，而且必须被报出来，不得看起来像一个已停止的适配器。经
    Coffer 任何接口面注册的渠道，创建时**必须**被绑定到注册它的那台机器，所以「未绑定」
    只会经由导入或手工改动到达，不会经由正常使用到达。
  - 绑定指名了一台已不在注册表里的机器时，必须在该渠道上报成一处故障，并与「仅仅是
    绑定在别处」区分开。两者在本机都是哪里都不跑；但只有其中一个是有人搞错了。
  - **不可能是机器 id** 的 `runs_on` 不得被当作绑定。渠道配置是一个系统此前
    也往里写过别的东西的口袋——被下线的机器轴就把 ULID 写在这同一个键下——
    所以形状不对的值指向的不是任何存在过的机器，它是化石，不是决定。Coffer
    必须把这样的渠道绑到本机，也就是这个键不存在时它本来会给的答案。这是唯一
    一处会覆盖既有值的情形，也正是唯一一处「不覆盖就会在升级时悄悄停掉一个
    还在用的机器人」的情形。
  - 改绑必须无需重启、也无需任何伸到另一台机器上的命令即可收敛。失去这条渠道的机器
    必须在看见变更之后的一个 reconcile tick 内停掉自己的适配器；得到它的机器必须在把
    变更带过去的那轮收敛之后的一个 tick 内启动一个。提供改绑的接口面必须说明：在另一
    台机器还持有这条渠道时把它绑到用户正坐着的这台，会让两边都活着，直到那台机器的
    下一轮收敛为止。
  - 渠道的配置必须只携带凭据**引用**，绝不携带密文材料——和它从不外传的年代一模一样，
    规则没变，而外传恰恰让这条规则从「整洁」变成了「承重」。这些引用对应的密文只在用户
    为远端开启凭据时才随行；持有密文却没有主密钥的机器，必须把这些引用报成已锁定，而
    不是默默解密失败。
  - 绑定到另一台机器的渠道，不得被本机自己的前置校验拒收。它的 `default_agent` 指名的
    是运行它那台机器上的 agent；在本机校验这一点，等于为一件不在任何机器上的故障，把
    一份好文档挡在注册表外面。
