...
渠道的平台身份（被轮询的 bot、webhook 端点）只容许一个消费者，所以「哪台机器来
应答这个 bot」必须只有一个答案——而保证这一点的办法，不是把渠道收窄到某一台机器，
而是压根不让它到达第二台。**`channel` 不参与收敛。** 它是唯一一个被收敛在两个方向
上都忽略掉的 kind（spec vault-sync，`## What does not sync`）：渠道是一个焊死在注册
它的那台机器上的入站面——那台机器的端口、那台机器的隧道、告诉平台去回调的那个
webhook URL——所以一份副本到了别处，好一点是完全不起作用，差一点就是多出一个抢事件
的消费者。

于是渠道就在配置它的那台机器上运行，也不存在第二台机器持有一行能再起一个适配器的
记录。渠道在本机被启用时运行，在本机被禁用时停止。两个适配器当然仍有可能指向同一个
bot 身份，但只剩下它们一直以来就有的那条路——有人手工在两台机器上把同一个 bot 注册
了两遍——而这一条，Coffer 里放任何字段都拦不住。

因此渠道的 `scope`
（[Per-Agent Resource Scope](../../docs/decisions/per-agent-resource-scope.zh.md)）
只说一件事，而且与机器无关——见 FR-079。它命名的是这条渠道**可以驱动哪些 agent**。
它与 runtime 只在一处交汇，就是空集这个情形：一条什么 agent 都驱动不了的渠道压根
不会运行。
...
- **FR-079**：渠道必须承载框架级的 `scope`
  ...
  这个反转后的读法就是渠道 scope 的全部：里面没有第二样东西，尤其没有关于它在哪里运行
  的任何说法——因为渠道根本不会到达一台还需要被告知「别运行它」的机器（见「渠道在哪里
  运行」）。
