...
- **FR-017**: owner 从 chat 切换 model。`/model` 无参时报告当前 model；
  ...
  选项取自该 agent 的 model catalogue（与网页 picker 同一份列表），且**整份**
  catalogue 都可选，只是**一次一页**（FR-018）：`claude_code` 的 catalogue 有 29 个
  ...
  的 model（包括 catalogue 里没有的）最快的路，卡片正文也这么写着。没有建议时回退到
  文本报告。
...
### Scenario: mention 只随最终快照发出，绝不出现在中间快照里

- **Given** 一个群 turn 把回复流式写入 live surface，其中间快照以纯文本发出、
  最终快照以富文本发出，
- **When** turn 结束，
- **Then** 只有最终快照带上 mention 标记。
...
- **FR-070**: 群里的回答要点名它是给谁的。在群里，bot 的回复必须以 @mention 驱动该
  turn 的那位成员开头，使用平台自己的 mention 标记——于是回答会提醒正在等它的那个人，
  热闹的房间也能一眼看出它属于谁。在单聊里则必须不这么做：1:1 的会话没有需要消歧的
  对象，在那里 @ 只是在喊。三条约束框住它。mention 由**平台**用来寻址成员的那个 id
  构造，而它未必等于 owner gate 比对的那个 id（SeaTalk 用 `employee_code` 把关、用
  `seatalk_id` 做 mention，而当发送者不在 bot 所属组织内时 `employee_code` 会是空）
  ——所以传输层两个都要带。它只能出现在那条以平台富文本格式渲染的消息里，绝不能出现
  在以纯文本发出的快照中，否则这段标记会以自身的字面源码抵达读者眼前。并且它静默降级：
  没有 id，或传输层无法仅凭 id 做 mention，就发一条普通的、不带 mention 的回复——绝不
  发出一个破碎的标签。
