...
本规范如今自己拥有的 turn 平台上。channel 内核（配对、路由、命令、排队、渲染策略、
...
导航：`Channels` 加入 `nav.group.agents` 组（[Everything Is a Resource Kind](../../docs/decisions/everything-is-a-resource-kind.zh.md)
预留的位置）。
...
- **Acceptance**：spec.md 的每个 scenario 都带一个
  `@pytest.mark.acceptance(spec="channels", scenario=...)`（或前端
