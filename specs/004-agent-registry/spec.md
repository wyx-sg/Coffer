...
The **session context injection** column names the file the hook entry lands in. Both products expose a shell hook: the agent execs `coffer-hook` at session start and reads its stdout, which carries the FR-044 payload (`GET /agents/{name}/session-context`). The on-disk entry shape and the lifecycle events differ per product; `HookFlavor` carries that difference.
...
- **Context Injection Spec**: The manifest facet describing how Coffer's session context (rules + memory) reaches one agent's model. Carries the allowlisted config `key` + `format` it writes, the lifecycle `events` it installs, and a `HookFlavor` selecting both the on-disk entry shape and the stdout envelope `coffer-hook` prints. One mechanism exists: the agent execs `coffer-hook` at session start and reads its stdout (FR-043).
