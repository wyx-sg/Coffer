...
**Cursor's `sessionStart` headless firing is documented but not empirically confirmed** — a marker probe was blocked by `cursor-agent` requiring authentication. See ADR-041's Consequences.
...
- **Context Injection Spec**: The manifest facet describing how Coffer's session context (rules + memory) reaches one agent's model. Carries an `InjectionMode` (`SHELL_COMMAND` / `PLUGIN_DROP` / `INSTRUCTIONS_BLOCK`), the allowlisted config `key` + `format` it writes, the lifecycle `events` it installs, and a `HookFlavor` selecting both the on-disk entry shape and the stdout envelope `coffer-hook` prints. Only `SHELL_COMMAND` is implemented; the other modes are recognized extension points, mirroring `SkillDeliveryMode`'s reserved values. See [ADR-041](../../docs/decisions/ADR-041-context-injection-mechanisms.md).
