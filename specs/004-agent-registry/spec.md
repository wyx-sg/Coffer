...
- **Agent type not in the supported list**: Registration rejected with a clear message and the supported-type list (the manifest types — `claude_code`, `codex`, `opencode`, and later `hermes` / `cursor`).
...
- **Then** only Coffer's block (and the separating blank line install added) is removed — the document returns to its pre-install bytes — the uninstall audits `agent_hook_uninstalled`, and uninstalling again is a no-op success that writes and audits nothing.
...
| `opencode` | `~/.config/opencode/` | `opencode run --format json` (JSONL) | ✓ `mcp` typed-local JSON | `PLUGIN_DROP` **slice** — no shell hook, but its plugin API *can* inject (`chat.message` mutates `output.parts`; `experimental.chat.system.transform` pushes onto the system prompt) | ✓ `provider` block, `apiKey:"{env:COFFER_PROVIDER_KEY}"` | **N/A** — no cross-session native memory feature exists to disable | this slice |
| `hermes` | `~/.hermes/` | `hermes acp` (ACP JSON-RPC over stdio) | ✓ `mcp_servers` YAML | ✓ `INSTRUCTIONS_BLOCK` — a marker block in `SOUL.md`, the only instruction file hermes reads from its home dir (`prompt_builder.load_soul_md`; `AGENTS.md` is cwd-only — both probe-verified on v0.18.0) (FR-047: rendered at install, refreshed before each Coffer-driven turn), because `on_session_start` / `pre_llm_call` are documented but **never invoked** upstream ([hermes-agent#2817](https://github.com/NousResearch/hermes-agent/issues/2817), closed as not planned); only tool hooks fire | ✓ `providers.coffer` + `key_env: COFFER_PROVIDER_KEY` | ✓ `memory.memory_enabled:false` | this slice |
| `cursor` | `~/.cursor/` | `cursor-agent -p --output-format stream-json` (NDJSON) | ✓ `mcpServers` in `mcp.json` | ✓ `SHELL_COMMAND` — `hooks.json` `sessionStart` (own flavor: flat entries, camelCase keys, top-level `version`), output field `additional_context` | **N/A** — `cursor-agent --help` (v2026.02.27) exposes no base-URL flag and `cli-config.json` no endpoint key; custom base URL is IDE-only. Uses its own `cursor-agent login` auth | **N/A** — Memories is an IDE-only Settings→Rules toggle, no CLI flag or config key | this slice |

**Cursor's `sessionStart` headless firing is documented but not empirically confirmed** — a marker probe was blocked by `cursor-agent` requiring authentication. See ADR-042's Consequences.

The fourth removed type, `openclaw`, is a peer gateway (own agent runtime, multi-agent router, native memory, MCP host); it is NOT a managed leaf agent and is out of this matrix (separate design track, [ADR-040](../../docs/decisions/ADR-040-re-widen-agent-registry.md)).
