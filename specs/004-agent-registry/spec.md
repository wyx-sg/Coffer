...
| `cursor` | `~/.cursor/` | `cursor-agent -p --output-format stream-json` (NDJSON) | ✓ `mcpServers` in `mcp.json` | **deferred** — `hooks.json` headless `SessionStart` firing is undocumented upstream (a follow-up slice) | **N/A** — locked to Cursor's backend, no custom LLM base URL; uses its own `cursor-agent login` auth | **N/A** — IDE-only toggle, no CLI | this slice |
