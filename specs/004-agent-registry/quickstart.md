...
## Deliver shared instructions to every agent

Maintain one set of house instructions (coding standards, tone, "always do X")
once and fan it out to every agent, instead of pasting the same text into each
agent's instruction file by hand. Coffer keeps a single **master instructions**
document in its hub at `~/.coffer/instructions/AGENTS.md`.

Set (or replace) the master from a file:
...
The master is read/written with a content fingerprint; a write carrying an
outdated fingerprint is rejected with `409 Conflict` so you don't clobber a
concurrent edit — re-read, then write again.

Deliver the master into an agent's instruction file (`CLAUDE.md` / `AGENTS.md`
/ `SOUL.md`):
...
Delivery writes the master into a Coffer-managed block fenced by
`<!-- coffer:instructions:start (managed, do not edit) -->` /
`<!-- coffer:instructions:end -->`, preserving everything outside the block
byte-for-byte (a `.bak` of the prior file is kept). It is idempotent —
re-delivering after a master edit rewrites the block in place, never
duplicating it. The managed block uses its own markers, distinct from the
spec-007 memory-projection block, so both coexist in the same file untouched.

Check delivery status (derived live, never stored):
...
Status reports `delivered=false` when no block is present, `delivered=true,
in_sync=true` when the delivered block matches the master, and `in_sync=false`
when the block has drifted (edit the master, then re-deliver to resync).

Going the other way, **adopt** an agent's existing instructions into the master
— takes the body of the agent's managed block, or the whole instruction file
when no block is present, and writes it as the new master (the agent's own file
is left unchanged):
...
Remove a delivered block with `coffer agent instructions remove claude-code`
(`DELETE /api/v1/agents/{name}/instructions`) — only the `coffer:instructions`
block is stripped, other content kept. Delivery, status, removal, and adopt
require the agent's type to define an instructions file; a type without one
(e.g. `openclaw` today) is rejected with `422 Unprocessable Entity`.
