...
- **FR-048**：对 manifest 声明 `PLUGIN_DROP` 注入的 agent（`opencode`——没有 shell hook；进程内 JS 插件 API 即注入点，[ADR-042](../../docs/decisions/ADR-042-context-injection-mechanisms.zh.md)），FR-043 的 install/uninstall/status 操作 MUST 作用于 agent 自动加载插件目录里的**一个 Coffer 专属插件文件**（`coffer-session-context.js`；manifest 的 `config_key` 指向 allowlist 的 `plugin` DIRECTORY 条目）。安装写入一个自包含模块：spawn 同一个 `coffer-hook` 二进制，参数 `--dialect raw --event sessionStart`（argv 直接执行、无 shell——hook 路径与 agent 名以 JSON 字符串字面量嵌入，任意路径安全），把返回的 FR-044 bundle 经 `experimental.chat.system.transform` 推入系统提示词，按会话缓存且失败静默（daemon 挂了绝不拖垮 agent；spawn 有 5s 超时）。与 FR-047 不同，此 mode 是**动态的**——插件实时取 bundle，因此没有回合前刷新机制。安装 MUST 幂等（原位覆盖 Coffer 的命名文件）且 MUST NOT 触碰插件目录中的其他文件；卸载只删除 Coffer 的文件（保留 `.bak`），文件不存在时为 no-op 成功；`status` 报文件存在性并携带 spawn 命令。`raw` 方言输出无信封的 bundle 文本且从不读 stdin。审计同 FR-043。
...
- **Context Injection Spec**：描述 Coffer 的会话上下文（规则 + 记忆）如何抵达某个 agent 模型的 manifest facet。携带一个 `InjectionMode`（`SHELL_COMMAND` / `PLUGIN_DROP` / `INSTRUCTIONS_BLOCK`）、它写入的 allowlist 配置 `key` + `format`、它安装的生命周期 `events`（`INSTRUCTIONS_BLOCK` 为空——块不是事件驱动的），以及一个 `HookFlavor`（同时决定磁盘条目形状与 `coffer-hook` 打印的 stdout 信封；仅对 `SHELL_COMMAND` 有意义）。三种 mode 均已实现：`SHELL_COMMAND`（FR-043）、`INSTRUCTIONS_BLOCK`（FR-047）、`PLUGIN_DROP`（FR-048）。见 [ADR-042](../../docs/decisions/ADR-042-context-injection-mechanisms.zh.md)。
...
- 多种 agent 类型已在能力清单（`AGENT_DESCRIPTORS`）中接线——`claude_code`、`codex`、`opencode`、`hermes` 与 `cursor`——每种都是一个 `AgentType` 枚举值加一条记录（安装标记、配置文件 allowlist、MCP 注入形态，以及它支持的任何可选 facet）。再增加一个产品也是同样的一条记录变更，外加当其 wire 协议是新的时一个 chat-provider 适配器，并在某 facet 于上游不存在时把该 facet 留空（能力矩阵，FR-003a）。从早先的两类型状态重新加宽即 [ADR-040](../../docs/decisions/ADR-040-re-widen-agent-registry.zh.md)。
