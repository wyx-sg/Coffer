# Data Model —— Memory（跨 agent 共享记忆）
...
与 Knowledge Base 规范的形状差异是刻意的：memory 规范把 embedding 字段保持**扁平**，让 memory 表单保持轻薄；Knowledge Base 规范则把它们嵌套在一个 `EmbeddingConfig` 对象里。自全局 embedding 重设计起，扁平字段已是遗留字段——为兼容性在 wire 上继续接受但被忽略；索引与 recall 都解析**全局** embedding 配置。同理，memory 规范的 recall 响应里的 `fallback` 是**布尔值** —— recall 跨多个 store，单一的回退模式字符串没有良定义；而 Knowledge Base 规范的单 store 搜索报告一个可空的模式枚举（`fallback: "keyword" | null`）。
...
agent 只通过 MCP 网关工具（`coffer__recall`/`remember`/`list_memory`）读写记忆；事实编辑/删除是用户面（REST/CLI/外部编辑器），不是 MCP 工具。Coffer 从不改动 agent 的原生记忆文件（原生投影已移除 —— 见 [Memory via MCP](../../docs/decisions/memory-via-mcp-not-native-projection.zh.md)）。
...
下面的 schema 与 KB 重设计迁移创建的是**同一份统一 schema**（迁移归 Knowledge Base 规范所有；这里是它的 memory 视角）。重设计 revision **删除** `memory_records` 并创建这些表。
