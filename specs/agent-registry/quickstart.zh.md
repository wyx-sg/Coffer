...
存入其加密凭据存储 ([Envelope-Encrypted Credentials](../../docs/decisions/envelope-encrypted-credential-store.md))；
明文绝不进入数据库、日志或审计：
...
- 每个 agent 都作为 kind 为 `agent` 的 Resource 存进 Coffer 的 SQLite
  数据库，标识为 `agent:<name>`。kind-agnostic Resource 框架（在 spec mcp-gateway
  中引入）提供 CRUD、校验与 audit。
- 每一次 add / edit / remove 都会写入 audit 事件，可通过 `coffer audit list`
  查询。
- agent 的 `<config_dir>/skills` 会成为未来 skill 投递（the skill-manager spec）使用的目标目录。
