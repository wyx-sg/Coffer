...
agent registry 的实体、字段、关系与存储说明。建立在 spec mcp-gateway 引入的 kind-agnostic Resource 框架之上——agent 是通用 `resources` 表中的行，因此 spec agent-registry 不新增任何自有表。
...
呈现**——查看器渲染内容并提供「在外部编辑器中打开 / 显示」（HTTP 的
...
**无。** `agent` kind 不需要自有表——agent 是通用 `resources` 表（来自 spec mcp-gateway
...
| 方法                                                                                              | 用途                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list_files(name) -> list[ConfigFileInfo]`                                                        | 对该 agent 类型的每个 `ConfigFileSpec`，返回 key、显示名、路径、所在文件夹的 `folder_path`（支撑只读 UI 的 打开/显示，FR-038）、格式、`kind`、`exists`，存在时附带大小 + mtime。目录条目额外携带 `files`（递归 `.md` 列表，`DirEntryInfo` 行）。                                                                                             |
| `read_file(name, key) -> ConfigFileContent`                                                       | 解析 `spec_for(type, key)`；返回内容 + `path` + `folder_path` + 格式 + `exists` + `fingerprint` + `memory_block`（`path`/`folder_path` 这一对支撑「在外部编辑器中打开 / 显示」，FR-038）。文件不存在 → 空内容、`exists=False`、`fingerprint=""`、不创建文件。                                                                              |
...
读取返回它，写入带回它。`ConfigFileContent.memory_block` 在文本包含 spec knowledge
...
- 所有 HTTP 路由绑定 `127.0.0.1`，共用 `X-Coffer-Token` 鉴权（依 spec mcp-gateway）。
