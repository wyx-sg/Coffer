...
- **FR-017b**：桌面 UI MUST 通过统一的 **知识** 导航来呈现 memory，而非一个独立的 Memory 页面 —— 知识 = 记忆（notes）+ 文档（documents，spec 006）× 全局 / 项目（[ADR-030](../../docs/decisions/ADR-030-per-project-kb-scope-and-soft-delete.md)）。在某个 scope 内（全局，或一个按 FR-017a 的可读 `project_root` 身份呈现的项目），一个 store 的事实与该 scope 的 KB 文档被一起列出，各自按类型打标，并配一个**按形态添加**的输入（粘贴的链接 / 文本变成一条 note；拖入的文件变成一份文档）。这只是**展示**层的改动：memory store 模型、scope 以及 REST/CLI/MCP 写入面都不变，事实查看器保持只读（FR-021）。UI 流程由前端测试 + e2e（延后）覆盖，而非新增后端验收场景。
