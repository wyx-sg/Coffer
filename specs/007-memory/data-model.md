...
| Organize (explicit trigger; internal LLM)      | Per inbox item: retrieve ≤3 candidate topic docs → one-shot LLM merge/create → write `knowledge/<slug>.md` → delete the inbox item (only after the write) → append `consolidation-log.md`. Then regenerate `INDEX.md`, reconcile the index, audit `memory_organized`. Malformed LLM output skips the item (stays in inbox); no internal model → no-op. |
