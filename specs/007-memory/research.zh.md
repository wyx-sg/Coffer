...
Spec 006（knowledge_base）在 `application/knowledge_base/` 内引入了一个 tracer 端口。Spec 007（memory）需要同样的 trace 面，覆盖 `add` / `search` / `edit` / `delete`。按 constitution 的「第二个 feature 用到时再提取横切关注点」规则，本规范**把** tracer 端口从 `application/knowledge_base/tracer.py` **提升**到 `application/observability/tracer.py`。KB 既有的 import 同步改写。整个变更是机械的，一次提交解决。
