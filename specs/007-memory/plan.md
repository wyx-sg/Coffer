...
This spec lands atop `006-knowledge-base`, which delivered the `Tracer` port that memory promotes to a shared `application/observability/` module here.
...
Identical analysis to 006 (knowledge_base). Layer rules and import-linter contracts extend symmetrically for the new `memory` kind. The `Tracer` port introduced in 006 is **promoted** to `application/observability/` here — this spec is the second feature that needs it, satisfying the constitution's "extract cross-cutting after the second feature" rule.
...
│   └── observability/                   # NEW (promoted from KB-only) — Tracer port lives here
│       ├── __init__.py
│       └── tracer.py
├── infrastructure/
│   ├── memory/
...
│   │   └── paths.py
│   └── observability/                   # extracted from KB-only; KB now imports from here
│       ├── __init__.py
│       ├── noop_tracer.py
│       └── langfuse_tracer.py
└── surfaces/
...
| The `Tracer` port promotion timing means we touch 005's files      | The promotion is a small, mechanical move; the imports change from `application.knowledge_base.tracer` to `application.observability.tracer`. Done in one commit, tested. |
