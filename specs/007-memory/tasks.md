...
- [ ] T-0085 Read-only-UI pivot: replace `MemoryAddFactForm.tsx` (in-app add/edit) with a read-only `MemoryFactViewer.tsx` (renders fact content; no in-app content editing). Add open-in-editor / reveal affordances for the fact file and its containing folder (via the loopback daemon, spec 004 FR-039), driven by `FactOut.path`/`folder_path` and `MemoryStoreOut.store_dir`, using the global preferred-editor preference (002-ui-shell). Rename `FactEditor.test.tsx` → `FactViewer.test.tsx`. (FR-021/FR-022, US4.)
