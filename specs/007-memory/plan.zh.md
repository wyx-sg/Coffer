...
本规范在 `006-knowledge-base` 之上落地 —— 006 引入的 `Tracer` 端口在本规范作为第二个消费者把它提升为共享 `application/observability/` 模块。
...
跟 006（knowledge_base）的分析一致。新 kind `memory` 沿着同一套层规则与 import-linter 契约对称扩展。006 引入的 `Tracer` 端口在本规范**被提升**到 `application/observability/` —— 这是第二个用到它的 feature，正好满足 constitution 的「第二个 feature 用到时再提取横切关注点」原则。
...
│   └── observability/                   # NEW（从 KB-only 提升）—— Tracer 端口落这里
│       ├── __init__.py
│       └── tracer.py
├── infrastructure/
│   ├── memory/
...
│   │   └── paths.py
│   └── observability/                   # 从 KB-only 抽出来；KB 现在 import 这里
│       ├── __init__.py
│       ├── noop_tracer.py
│       └── langfuse_tracer.py
└── surfaces/
...
| `Tracer` 端口提升的时机意味着我们要碰 006 的文件 | 提升是一次小而机械的迁移；import 由 `application.knowledge_base.tracer` 改成 `application.observability.tracer`，一次提交解决，配测试。 |
