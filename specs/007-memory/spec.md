...
- **FR-041:** Journal files SHALL be included in the sync mirror as source-of-truth history (like `rules/` and `superseded/`). (Journal participation in `recall` is specified by the journal-indexing slice; this slice ships storage + service only, so journal is not yet searched.)
- **FR-042:** Coffer SHALL expose internal `JournalService.append(cwd, body, actor)` and `read_recent(cwd, limit)`. Appending outside a git project raises `ScopeUnresolved`; reading outside a git project returns an empty list. The `journal_append` audit entry records `char_size` only — never the body.
