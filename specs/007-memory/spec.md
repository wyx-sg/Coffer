...
- **FR-017b**: The desktop UI MUST present memory through the unified **知识** navigation rather than a standalone Memory page — 知识 = 记忆 (notes) + 文档 (documents, spec 006) × 全局 / 项目 ([ADR-030](../../docs/decisions/ADR-030-per-project-kb-scope-and-soft-delete.md)). Within a scope (global, or a project shown by its readable `project_root` identity per FR-017a), a store's facts and that scope's KB documents are listed together, each tagged by type, with one **add-by-shape** input (a pasted link / text becomes a note; a dropped file becomes a document). This is a **presentation** change only: the memory store model, scopes, and REST/CLI/MCP write surfaces are unchanged, and the fact viewer stays read-only (FR-021). UI flows are covered by frontend tests + e2e (deferred), not new backend acceptance scenarios.
