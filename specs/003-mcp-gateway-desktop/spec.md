...
A user picks the download that fits their environment. The release offers **two tiers**, both built from the same pair of PyInstaller binaries:

1. **CLI-only** — a single archive `coffer-cli-<triple>.tar.gz` (macOS / Linux) or `coffer-cli-<triple>.zip` (Windows) containing just `coffer-daemon` and `coffer-mcp-shim` plus a SHA-256. For headless / no-GUI installs: extract it, run `coffer-daemon`, put `coffer-mcp-shim` on `PATH`, and point MCP clients at the shim. No desktop app required.
2. **CLI+desktop** — the Tauri bundle (DMG / MSI / AppImage / deb) that embeds the same two binaries as sidecars plus the desktop shell and the 002 web UI. One installer, no system Python, no separate daemon install, no manual PATH editing — first launch deploys the shim to a stable user-writable PATH location and the daemon comes up on its own.
...
**Independent Test**: CLI-only — on a clean machine (no Python, no Coffer checkout), download `coffer-cli-<triple>`, extract, run `coffer-daemon`, and verify a fresh shell resolves `coffer-mcp-shim` after it is placed on `PATH`. CLI+desktop — download the platform installer from a draft release, install it, launch it once; verify the daemon reaches `status: ready` and `coffer-mcp-shim` resolves from a fresh shell.
...
- **Then** the release contains the desktop installers — macOS arm64 DMG, macOS x64 DMG, Linux x64 (AppImage/deb), Windows x64 (MSI/NSIS)
- **And** the release also contains the CLI-only archives — `coffer-cli-<triple>.tar.gz` per macOS / Linux target and `coffer-cli-<triple>.zip` per Windows target, each holding `coffer-daemon` + `coffer-mcp-shim`
- **And** each artifact ships with a SHA-256 checksum file alongside it
...
- **FR-D09**: The release pipeline MUST produce, per `v*` tag, two download tiers from the same PyInstaller binaries: (a) the **CLI+desktop** tier — macOS arm64 DMG, macOS x64 DMG, Linux x64 (AppImage / deb), Windows x64 (MSI / NSIS); and (b) the **CLI-only** tier — a `coffer-cli-<triple>` archive per target (`.tar.gz` on macOS / Linux, `.zip` on Windows) containing only `coffer-daemon` and `coffer-mcp-shim`.
- **FR-D10**: Each release artifact — every desktop installer and every CLI-only archive — MUST be accompanied by a SHA-256 checksum file generated in CI.
...
- **SC-D04**: The post-build smoke test (`scripts/smoke_test_bundle.sh`) runs against the macOS and Linux artifacts in the release matrix and returns success — bundled shim talks to bundled daemon over loopback HTTP and gets a JSON-RPC `initialize` reply. The script is Unix-only, so the Windows artifact is built and uploaded but not smoke-tested in CI.

## Distribution

The release offers two download tiers, both built from the same pair of PyInstaller binaries (`coffer-daemon` + `coffer-mcp-shim`):

- **CLI-only** — a `coffer-cli-<triple>` archive (`.tar.gz` on macOS / Linux, `.zip` on Windows) containing just the two binaries plus a SHA-256. For headless / no-GUI installs: run the daemon and point MCP clients at the shim, no desktop app.
- **CLI+desktop** — a Tauri 2 bundle (DMG / MSI / AppImage / deb) that embeds the same two binaries as sidecars plus the desktop shell and the 002 web UI.

See [`docs/decisions/ADR-008-distribution-pyinstaller-tauri-sidecar.md`](../../docs/decisions/ADR-008-distribution-pyinstaller-tauri-sidecar.md) for the architectural decision and [`docs/distribution/macos-notarization.md`](../../docs/distribution/macos-notarization.md) for the macOS notarization runbook.

The release pipeline (`.github/workflows/release.yml`) runs PyInstaller per target architecture, then both packages the two binaries into the `coffer-cli-<triple>` archive and drops them into `desktop/binaries/` to build the Tauri bundle, notarizes (macOS, once a paid Apple Developer ID is in place), and uploads every artifact with its SHA-256 checksum.
