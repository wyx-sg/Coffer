...
用户挑选适合自己环境的下载。release 提供**两个层级**，二者都基于同一对 PyInstaller 二进制构建：

1. **CLI-only** —— 单个归档 `coffer-cli-<triple>.tar.gz`（macOS / Linux）或 `coffer-cli-<triple>.zip`（Windows），仅含 `coffer-daemon` 与 `coffer-mcp-shim` 外加一份 SHA-256。用于 headless / 无 GUI 安装：解压、运行 `coffer-daemon`、把 `coffer-mcp-shim` 放上 `PATH`、让 MCP 客户端指向 shim，无需桌面应用。
2. **CLI+desktop** —— Tauri bundle（DMG / MSI / AppImage / deb），把同样这两份二进制作为 sidecar 内嵌，外加桌面外壳与 002 的 Web UI。一份安装包，无系统 Python、无须单独安装 daemon、无须手工改 PATH —— 首次启动把 shim 部署到稳定的用户可写 PATH 目录，daemon 自动就绪。
...
**独立可测**：CLI-only —— 在干净机器（无 Python、无 Coffer checkout）上下载 `coffer-cli-<triple>`，解压，运行 `coffer-daemon`，把 `coffer-mcp-shim` 放上 `PATH` 后验证新开终端可解析它。CLI+desktop —— 从 draft release 下载平台安装包，安装，启动一次；验证 daemon 到达 `status: ready` 且新开终端中 `coffer-mcp-shim` 可被解析。
...
- **Then** release 含有桌面安装包 —— macOS arm64 DMG、macOS x64 DMG、Linux x64（AppImage/deb）、Windows x64（MSI/NSIS）
- **And** release 还含有 CLI-only 归档 —— 每个 macOS / Linux 目标一份 `coffer-cli-<triple>.tar.gz`、每个 Windows 目标一份 `coffer-cli-<triple>.zip`，各自含 `coffer-daemon` + `coffer-mcp-shim`
- **And** 每份制品旁边都带有 SHA-256 校验文件
...
- **FR-D09**: release 流水线 MUST 在每个 `v*` tag 上、基于同一对 PyInstaller 二进制产出两个下载层级：(a) **CLI+desktop** 层级 —— macOS arm64 DMG、macOS x64 DMG、Linux x64（AppImage / deb）、Windows x64（MSI / NSIS）；(b) **CLI-only** 层级 —— 每个目标一份 `coffer-cli-<triple>` 归档（macOS / Linux 为 `.tar.gz`，Windows 为 `.zip`），仅含 `coffer-daemon` 与 `coffer-mcp-shim`。
- **FR-D10**: 每份 release 制品 —— 每份桌面安装包与每份 CLI-only 归档 —— MUST 在 CI 中生成一份 SHA-256 校验文件随之发布。
...
- **SC-D04**: post-build smoke test（`scripts/smoke_test_bundle.sh`）MUST 在 release 矩阵的 macOS 与 Linux 制品上运行成功 —— bundle 自带的 shim 透过 loopback HTTP 与 bundle 自带的 daemon 通话，得到一次 JSON-RPC `initialize` 回包。该脚本仅支持 Unix，因此 Windows 制品会被构建并上传，但不在 CI 中做 smoke test。

## Distribution

release 提供两个下载层级，二者都基于同一对 PyInstaller 二进制（`coffer-daemon` + `coffer-mcp-shim`）构建：

- **CLI-only** —— 一份 `coffer-cli-<triple>` 归档（macOS / Linux 为 `.tar.gz`，Windows 为 `.zip`），仅含这两份二进制外加一份 SHA-256。用于 headless / 无 GUI 安装：运行 daemon、让 MCP 客户端指向 shim，无需桌面应用。
- **CLI+desktop** —— 一份 Tauri 2 bundle（DMG / MSI / AppImage / deb），把同样这两份二进制作为 sidecar 内嵌，外加桌面外壳与 002 的 Web UI。
...
release 流水线（`.github/workflows/release.yml`）按目标架构运行 PyInstaller，随后既把这两份二进制打成 `coffer-cli-<triple>` 归档，也把它们放入 `desktop/binaries/` 构建 Tauri bundle，公证（macOS，等 Apple Developer ID 就位后启用），并为每份制品上传 SHA-256 校验文件。
