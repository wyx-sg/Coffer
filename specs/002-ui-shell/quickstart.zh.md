...
- `/settings` — tabs 侧栏分 **Data**（retention 策略、手动清理、备份）和 **About**（版本 / 许可证 / 源代码）。桌面端会再加一个 **App** tab（开机启动），在 spec 003 上线时由本 PR 已挂好的 `isTauri()` 守护组件接管——在浏览器里它保持不可见。
...
今天侧栏 **Resources** 组下只有一个入口：MCP servers。等下一个 kind 上线（spec 004 agent registry、spec 005 skill manager……），它通过自己的 `kindRegistry.ts` 自动落进同一个组（见 [`plan.md`](./plan.md) 的 "Extension point: the kind registry"）。布局完全不动——侧栏和 `ResourcesPage` 是 kind-agnostic 的。

这是 [ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md) 给出的 IA 承诺：新 kind 接入时不再重新讨论导航。
...
- Tauri 桌面打包 (DMG / MSI / AppImage)、托盘监管以及开机启动。Web PR 已经把 `AppSettings` 组件以 `isTauri()` 守护交付，spec 003 接入时不需要改布局；详见 spec.md 中的 "Deferred Acceptance Scenarios"。
