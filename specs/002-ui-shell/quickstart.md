...
- `/settings` — tabs sidebar with **Data** (retention policy, manual
  prune, backups) and **About** (version / license / source). The desktop
  app adds an **App** tab (launch-at-login) when it ships in spec 003 —
  the component is already in this PR behind an `isTauri()` guard, so it
  stays invisible in the browser.
...
## 6. Add a second kind (forward-look)

Today the sidebar has exactly one entry under **Resources**: MCP servers.
When the next kind ships (spec 004 agent registry, spec 005 skill manager,
…), it adds itself into the same group via a per-kind
`kindRegistry.ts` (see [`plan.md`](./plan.md) "Extension point: the kind
registry"). No layout work is needed — the sidebar and the
`ResourcesPage` are kind-agnostic.

That's the IA promise of
[ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md):
new kinds plug in without re-litigating navigation.
...
- The Tauri desktop bundle (DMG / MSI / AppImage), tray supervision, and
  launch-at-login. The web PR ships the `AppSettings` component behind an
  `isTauri()` guard so spec 003 can wire it up without touching the
  layout; see spec.md's "Deferred Acceptance Scenarios".
