...
[ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md) ("everything is a resource kind"),
...
[ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md) for the IA decision.
...
| **Target Platforms**     | Modern evergreen browsers (Chromium / Firefox / Safari current-2). Mobile responsive layout deferred.                                                                                                                                                                                |
...
looking up the registry — no per-kind branches in shared code. Future kinds
(skill, knowledge_base, memory, channel, agent) add their own
`kindRegistry.ts` and are wired in by importing them at the composition
root; no other shared file changes.
...
### Future (planned in 003-mcp-gateway-desktop)

This PR ships the desktop-only `AppSettings` React component behind an
`isTauri()` guard so spec 003 can wire it up without touching the layout.
The autostart toggle, tray supervision, and Tauri sidecar packaging all
land in [spec 003](../../specs/) when it is drafted; see spec.md's
...
_(Phase 7 — desktop shell US5 — is deferred to spec
`003-mcp-gateway-desktop`. The web PR ships the `AppSettings` component
behind `isTauri()` so 003 can plug straight in.)_
...
- IA decision: [ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md)
