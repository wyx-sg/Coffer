...
把 `001-mcp-gateway` 交付时那个仅"能跑"的功能骨架，升级为一个真正的产品壳：一套统一的视觉语言、基于 [ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md)（"一切皆 resource kind"）单一概念的信息架构，以及让首次来访者就能用上的端到端流程。
...
面向用户的契约见 [./spec.md](./spec.md)，终端用户走查见 [./quickstart.md](./quickstart.md)，IA 决策见 [ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md)。
...
| **Target Platforms**     | 主流常青浏览器（Chromium / Firefox / Safari 当前-2）。移动端响应式延后。                                                                                                                                                                                  |
...
`frontend/src/kinds/<kind>/kindRegistry.ts` 导出一个小对象，声明该 kind 的展示名、侧栏图标、路由子项以及列表/详情组件。kind-agnostic 的 `ResourcesPage` 通过查注册表来分派——共享代码里没有任何按 kind 分支的逻辑。未来的 kind (skill、knowledge_base、memory、channel、agent) 各自加一份 `kindRegistry.ts`，在 composition root 处 import 即可，其它共享文件不需改。
...
### Future (在 003-mcp-gateway-desktop 中规划)

本 PR 已经把仅桌面的 `AppSettings` React 组件以 `isTauri()` 守护交付，spec 003 接上即可，不需要再动布局。开机启动开关、托盘监管、Tauri sidecar 打包都留给 [spec 003](../../specs/)（待起草）；详见 spec.md 中的"Deferred Acceptance Scenarios (US5 Desktop Shell)"。
...
_(Phase 7 — 桌面壳 US5 — 延后到 spec `003-mcp-gateway-desktop`。Web PR 已经把 `AppSettings` 组件以 `isTauri()` 守护交付，003 可直接插入。)_
...
- IA 决策: [ADR-008](../../docs/decisions/ADR-008-everything-is-a-resource-kind.md)
