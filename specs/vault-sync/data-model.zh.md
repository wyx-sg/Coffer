...
- `enabled` 与 `scope` 被**排除**，理由比「减少无谓变更」更硬：它们是一件事——
  这个资源的生效范围——而生效范围是本机专属的（spec `## 不同步什么`）。由较旧版本
  写出的文档里仍然带着它们；它们会被解析后丢弃，而绝不被拒绝，这样一台还没升级的
  机器就不会拖住其余机器的收敛。
- `channel` 和其他所有 kind 一样，有一份普通的文档。它曾经根本没有文档；如今它
  会随同步走，是因为它的配置指名了唯一那台由其 daemon 运行它 adapter 的机器
  （`runs_on`，spec channels FR-080），于是文档可以移动而 adapter 不跟着移动。
...
- `channel-peers/<channel>/<chat>.yaml` —— 一条渠道配对：chat id、属主门禁要核对的
  sender id、显示名，以及该聊天粘住的 agent。全都是平台身份，这正是它外传的理由：
  渠道现在会在机器之间移动，而一条抵达时不带配对的渠道，会让属主每次改绑都从手机上
  重新配对一遍。**当前会话指针不在文档里**——会话仅属本机，因此一份抵达的配对会保留
  本机原本持有的那个指针。文件名是经过净化的 chat id，因此它只是一个地址；真正的 id
  由载荷携带。
