...
- **共享状态**——归 vault 而非归某一台机器所有的模块自管区域：MCP 能力偏好、
  内部引擎设置、agent 的插件清单，以及 channel 的 peer 配对。

  channel 的 peer 配对又回来了。当初移除它的理由是配对属于平台层面，因此把一条
  channel 改绑到另一台机器时无需重新配对——而 channel 当时已经根本不会到达另一台
  机器，也就没有什么改绑可省了。如今 channel 重新会随同步走，并指名运行它的那台
  机器（见 `## 不同步什么`），这个前提没了，那次移除也就没了：一条 channel 移动
  过去却不带配对，会让属主每次都从手机上重新配对一遍，而这恰恰是这个区域当初
  存在的意义。随之外传的是平台身份——chat id、sender id、显示名、该聊天粘住的
  agent。**当前会话指针不外传**：会话是本机专属的，发布出去的指针指名的会是对面
  并不拥有的那段会话。
...
这份清单里有一条是决策而非机制，它说的是：一台机器*拿 vault 做了什么*，属于那
台机器自己。

- **生效范围 (reach)**——一个 resource 的 `enabled` 标志和它的 `scope`。它们读
  起来像两个字段，其实是一件事、由同一个控件写下：这个 resource 在这里是不是
  活的，以及对哪些 agent 是活的。生效范围在它生效的那台机器上设定，每台机器各
  设各的。把它发布出去，就等于允许一台机器悄悄替另一台机器重新回答一个后者已
  经为自己回答过的问题——笔记本上那个被刻意熄灭的 server，会在台式机的下一轮
  之后重新亮起来，而历史里没有任何一条读起来像是有人做的决定。

> **修订 2026-09-14（channel 随同步走，且每条都指名运行它的那台机器）。** 撤销
> 本清单里的 `channel` 条目，以及「收敛轮次在两个方向上都忽略
> `resources/channel/**`」这条规则。
>
> **为什么推翻。** 原条目说 channel 是一个绑定在单台机器上的入站界面——它的
> 端口、它的隧道、平台被告知要去回调的那个 webhook URL——因此一个 channel 到了
> 第二台机器上「往好了说是死的，往坏了说是两台机器同时应答同一段对话」。这个
> 危险是真的，而且至今没变。原条目缺的是一个能说清**哪一台**机器才是那个唯一
> 活着的消费者的手段；既然没有这样一个字段，它就只能不让文档动。现在 channel
> 自带 `runs_on`，也就是唯一那台由其 daemon 启动它 adapter 的机器的 `machine_id`
> （spec channels FR-080），而且只有那台机器会启动。这是正面回答了那个反对意见，
> 而不是绕开它：文档外传，adapter 不外传。
>
> **这换来了什么。** 第二台机器手上有这条 channel 的配置、它的凭据引用和它的
> 配对，于是接管一个 bot 是一次改绑，而不是一次重新注册——一台死掉的机器也不再
> 是一台把 bot 一起带走的机器。一条绑在死机器上的 channel，离跑在一台活着的机器
> 上只差一次点击。
>
> **这付出了什么。** 两样，都摆在明面上而不是藏起来。第一，多了一个**必须填对**
> 的字段：一个指名了没有任何机器认领的绑定，会让这条 channel 哪里都不跑，所以
> 它被报成一处故障，而不是被当成任何机器都可以启动的许可（FR-080）。第二，一条
> **单向的升级次序**：一台仍在跑「扣着不发」那个版本的机器，会持续把 channel
> 文档从共享树里删掉，而一台跑着当前版本的机器会像对待任何删除一样遵从它。
> 期望 channel 能外传之前，两台机器都必须先升级；在那之前，挡在一个陈旧导出方和
> 某人的 channel 之间的，是发布侧的删除守卫。
>
> **考虑过的替代方案。** *让 channel 保持本机专属，再加一套「认领」协议*——否决：
> 它需要同一个字段外加一次协商，而两台机器未必同时在跑，这样的协商没有答案。
> *把绑定表达成 `scope` 上的一个机器轴*——否决，理由正是当初移除机器轴的那条：
> 生效范围是本机专属的，而绑定是各机器共享的同一个答案，把两者塞进一个字段，
> 就让「这在哪里生效」有了两种含义和两种彼此打架的方式。
>
> `## 同步什么` 里那段配对文字由同一次修订恢复，理由也相同：它当初被移除，靠的
> 就是这个条目。
...
这里没有机器轴，也没有任何东西留给它去说。生效范围是本机专属的（见
...
**channel 的机器绑定不是这个轴借尸还魂。** `runs_on`（spec channels FR-080）和
生效范围回答的是不同的问题，因而放在不同的地方，理由也正是同一条。生效范围是
「在这里，对哪些 agent」——一个每台机器自己给自己的本地答案，所以它待在 resource
行上，永不外传。机器绑定是「哪台机器跑这个 adapter」——各机器共享的同一个答案，
所以它待在 channel 的配置里，随 channel 一起走。被撤掉的那个机器轴两者都不是：
它用一个会外传的字段，去记录一个每台机器靠持有自己的生效范围就已经说清了的事实。
而绑定记录的是一个任何单台机器都说不出来的事实，因为它讲的是它们当中谁来动手。
...
scope 编辑器**必须**在用户设定生效范围的地方写明：生效范围只对本机生效，不会被
...
| `resources/<kind>/<name>.yaml` | 经 resource 服务 upsert，展开 `${HOME}` 并跑该 kind 的导入闸门；本地 resource 的生效范围**不**被触碰 | 删除该 resource |
...
`channel` 与其它每个 kind 走同一行 upsert，而它的抵达不会启动任何东西：它携带的是
一条机器绑定，被指名之外的机器只是持有这份文档而已。渠道这个 kind 自己的注册期校验
恰恰对这种情形放宽（spec channels FR-080）——一条绑定在别处的渠道，不应该在这里被拿
去和本机恰好装了哪些 agent 做比对，因为要驱动它们的并不是本机。
...
- **Given** 一个两台机器上都存在的 `mcp_server`，在其中一台上被停用、在另一台上
  被限制到单个 agent，
- **When** 两台机器收敛，
- **Then** 每台机器仍然持有它各自被赋予的生效范围——停用的那台依旧停用、受限的
  那台依旧受限——而此后在任意一台上对该 server 配置的修改都会抵达另一台，并不
  把它的生效范围一并带过去。

### Scenario: a channel travels and runs only on the machine it names

- **Given** 在其中一台机器上配置好、并绑定到这台机器的一个 `channel`，
- **When** 两台机器收敛，
- **Then** 这条 channel 带着完好的绑定被注册到另一台机器上，那台机器不为它启动
  任何 adapter，而它指名的那台机器照常在跑它。

### Scenario: a channel's pairings travel with it

- **Given** 其中一台机器上一条已配对的 `channel`，
- **When** 两台机器收敛，且这条 channel 被改绑到另一台，
- **Then** 属主的配对出现在如今运行它的那台机器上、无需重新配对，而每台机器持有
  的会话指针各是各的。

### Scenario: a synced channel carries a credential reference, never a secret

- **Given** 一条配置里为其 bot token 与签名密钥写了凭据引用的 `channel`，
- **When** 一轮收敛导出本 vault，
- **Then** 工作树里这条 channel 的文档只装引用、不装任何密文材料，而密钥本身只
  以 Fernet 密文出现，且仅在远端被配置为携带凭据时才出现。
