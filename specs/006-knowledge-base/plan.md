...
| **Primary Dependencies (added by this spec)** | `llama-index-core` (RAG framework, behind port); `llama-index-embeddings-huggingface` + `sentence-transformers` (default local embedding model: `BAAI/bge-small-en-v1.5`); `pypdf` (PDF extraction); `langfuse` (optional, gated by env). |
...
| **III. OSS-Readiness**                                    | ✅         | LlamaIndex (MIT), sentence-transformers (Apache 2.0), pypdf (BSD-3). LangFuse (MIT) optional.                                                                                    |
...
| **Network defaults**                                      | ✅         | Loopback-only. LangFuse, if enabled, talks to user-provided host.                                                                                                                |
| **"Extract cross-cutting after second feature needs it"** | ✅         | LangFuse tracer port is _kind-specific_ in this spec; if `memory` (007) also needs traces, we extract `infrastructure/observability/` then.                                      |
