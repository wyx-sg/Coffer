...
The user fixes a conversion artifact by opening the document's Markdown in their own external editor (or via the edit API), then the change is picked up. They re-upload an updated version of a file under the same name and Coffer updates the **same document in place** (stable ULID id — no duplicate). They change chunk parameters or the embedding model and Coffer re-indexes/re-embeds the corpus. The single global embedding model is configured from Settings → Embedding through an add/edit dialog (only one model, with fetch-models and test-connection inside the dialog); because changing the model re-embeds every store the UI asks for confirmation first, while the enable toggle sits next to the chunking defaults. The Coffer UI renders the Markdown read-only — it never offers an in-app text editor — and instead offers affordances to open the document (or its containing folder) in the user's external editor or reveal it in the file manager, performed by the local daemon. Once a document is edited (`source_mode = edited`), re-conversion from the raw original is blocked to avoid clobbering edits.
...
### User Story 5 — Manage from the web UI and CLI, and observe (Priority: P2)

The user manages KBs from the web UI under `Resources` and from `coffer kb …` subcommands, and inspects per-KB metrics (document count, chunk count, disk usage, indexed modes, and the count of documents whose vector embed is pending because the provider was unavailable).
...
> **Deferred to future test work** (frontend Playwright + full-CLI e2e): create/upload/search/delete a KB through the web UI; CLI covers every web-UI operation; CLI search/grep return machine-readable JSON. Listed for completeness; `make verify-acceptance` does not gate on them.
