...
- **FR-009**: Users MUST be able to search a knowledge base with a query and receive ranked passages, each carrying its source document id, filename, a text snippet, and a relevance score — via the KB's configured `search_mode` (FR-017).
...
- **FR-017**: Each KB MUST declare a `search_mode`, fixed at creation: `keyword` (the default) ranks passages by term frequency over the ingested document text and requires no embedding model or model download — it is usable the moment the KB is created, like searching files; `semantic` uses the per-KB embedding model for vector similarity (FR-015 applies only to this mode). Both modes satisfy FR-009.
...
- **FR-016**: Users MUST be able to perform every operation through (a) a REST API under `/api/v1/knowledge_bases/`, (b) `coffer kb …` subcommands, and (c) a desktop UI under the existing `Resources` navigation.
