...
Once Coffer is your client's MCP server, the KB tools appear. Documents are **co-managed** (ADR-028) — there are read AND write tools. Read tools:
...
- `coffer__delete_document(kb, doc_id)` — remove a document.
