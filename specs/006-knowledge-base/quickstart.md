...
If you set `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_HOST` (your self-hosted LangFuse instance), Coffer emits traces for ingest and search calls. No text content is sent — only sizes, counts, and durations.

Unset variables → no tracing, no outbound calls, no log noise.
