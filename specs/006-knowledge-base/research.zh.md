...
**决策**：**Markdown-aware 切块** —— 按标题切，再由 `chunk_size`（默认 512）和 `chunk_overlap`（默认 64）限定。两者都是 **per-KB 且可变**：改它们会重新切块 + 重建语料索引（便宜，文件即真相）。这消除了原规范里 chunk 参数创建即冻结的半不可变毛病。

语义 / 层次 / 源码感知切块器 MVP 范围外；它们带来与预期语料规模不成比例的模型调用成本，可在同一切块接口后再加。
