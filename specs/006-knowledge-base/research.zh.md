...
**决策**：在 `application/` 中定义进程内 `Tracer` 端口。默认 adapter 为 **noop**。**LangFuse** adapter 只在环境变量 `LANGFUSE_PUBLIC_KEY` 设置时挂上去。这样：

- 维持 local-first 不变量：默认零出网。
- LangFuse 依赖在 adapter 内部懒 import（CI 失败也无碍）。
- 在本 PR 里，tracer 端口被抽到 `application/observability/`（比严格「等第二个消费者」规则提前一步），因为预期 spec 007（memory）就是第二个消费者，提前抽避免 007 上线时立刻 refactor。KB 是本 PR 中唯一消费者。
...
片段文本与查询文本都 **不** 发给 LangFuse —— 只发尺寸与计数。
