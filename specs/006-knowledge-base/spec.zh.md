...
- **FR-009**: 用户必须能用自然语言 query 检索 KB，得到排名片段，每条含源 document id、文件名、文本片段、相关性分数。
...
- **FR-016**: 系统必须通过 `Tracer` 端口在摄入与检索周边发出 span。默认 tracer 为 no-op；设置环境变量 `LANGFUSE_PUBLIC_KEY`（及相关 secret/host 变量）必须能透明启用 LangFuse 后端。

**接口面**

- **FR-017**: 用户必须能通过 (a) `/api/v1/knowledge_bases/` 下的 REST API、(b) `coffer kb …` 子命令、(c) 既有 `Resources` 导航下的桌面 UI 完成每个操作。
...
- **可观测性端口位置**：在本 PR 里，与 kind 无关的 `Tracer` 端口被抽到 `application/observability/` 下，而非留在 `application/knowledge_base/`。理由是前瞻：预期 spec 007（memory）是第二个消费者，宪法的「等第二个 feature 也需要再抽」规则被提前一步执行，免去 007 上线时的立即 refactor。本 PR 中 KB 是唯一消费者；LangFuse adapter 与 noop 默认实现都落在这里。
