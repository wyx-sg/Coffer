...
一份 5 分钟走查：从一份干净的 checkout 起，在浏览器里看到重设计后的 UI，通过 GUI 注册一台 MCP 服务器，并看到首次 invocation 出现。这是面向 **GUI** 的走查；CLI + shim 路径见 [`specs/mcp-gateway/quickstart.zh.md`](../mcp-gateway/quickstart.zh.md)。
...
要看到 invocation 落库，按 [`specs/mcp-gateway/quickstart.zh.md`](../mcp-gateway/quickstart.zh.md) 中 "Wire Coffer into your MCP client" 一节把 MCP 客户端接到 coffer，触发一次工具调用（比如让 Claude Code 读个文件），然后刷新 **Invocations** tab。
...
- `/settings` — tabs 侧栏打开在 **General**（默认每页条数偏好），另有 **Data**（retention 策略、手动清理）与 **About**（版本 / 许可证 / 源代码）。
...
- daemon 状态：`~/.coffer/coffer.db` 与 `~/.coffer/daemon.json`（与 spec mcp-gateway 一致）。
