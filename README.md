...
- **Knowledge** — one knowledge layer, scoped per project (or global, or a collection you name). Agents write notes into it over MCP; you drop in any-format documents (auto-converted to markdown, originals kept for provenance). A periodic tidy pass merges duplicate notes into coherent topic documents, archiving every revision it replaces. One search spans everything — grep / keyword / vector is an engine decision, not a question the caller has to answer.
