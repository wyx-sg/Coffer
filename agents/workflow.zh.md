...
Coffer **只启用 Dependabot 安全更新**。仓库中没有 `.github/dependabot.yml`，
因此不会再自动开出例行的版本升级 PR。

原因：本仓库要求分支在合并前必须与 `main` 保持同步，且未开启 auto-merge，
于是每个机器人 PR 都要付出一次手动分支更新、一整轮 CI 和一次合并的代价——
而且合并其中任何一个都会让其余全部失效，只能严格串行处理。对一个单人维护、
本地优先的工具来说，这份反复出现的成本无法被 patch 与 minor 升级所回报。
真正重要的大版本升级（例如 mcp 2.x SDK）本来就是照着 changelog 手动做的。

安全更新不受影响：它由仓库设置（Settings → Code security）单独控制，并非
由被删除的配置文件驱动，因此真实的安全公告仍会开出 PR。

手动升级某个依赖，走普通的 `chore(deps)` PR 即可。
