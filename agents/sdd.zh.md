...
所有 spec 平铺在 `specs/<short-name>/` 下，以功能命名而非编号。spec 只有一种类型——不分 foundation / capability / quality。
...
- `specs/<short-name>/spec.md`（及其 `spec.zh.md` 对照）——用户可见的契约与验收场景。
- `specs/<short-name>/contracts/api.openapi.yaml`——线上契约；增删/重命名端点与 schema 以匹配代码。
- `specs/<short-name>/data-model.md`、`plan.md`、`quickstart.md`（及各自的 `.zh.md` 对照）——实体、计划与用法说明。
