...
describe("planChannelEdit", () => {
  ...
  test("the default model and allowed range ride the same config PATCH", () => {
    const plan = planChannelEdit({
      name: "tg",
      config: {
        channel_type: "telegram",
  ...
  test("clearing them drops the keys, because a PATCH replaces the whole config", () => {
    // Leaving the spread-in old values behind would make "unpin" and "no
    // restriction" impossible to express from the form.
    const plan = planChannelEdit({
      name: "tg",
      config: {
        channel_type: "telegram",
      ...
      values: { default_agent: "claude_code", default_model: "  ", models: [] },
    });
  ...
  test("a caller that says nothing about them leaves the stored values alone", () => {
    const plan = planChannelEdit({
      name: "tg",
      config: {
        channel_type: "telegram",
    ...
    expect(plan.config.default_model).toBe("claude-opus-5");
    expect(plan.config.models).toEqual(["claude-opus-5"]);
  });
});
