// frontend/src/kinds/knowledge/SourceCheckDialog.tsx
//
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{t("knowledge.detail.sourceCheck.title")}</DialogTitle>
          <DialogDescription>{t("knowledge.detail.sourceCheck.description")}</DialogDescription>
        </DialogHeader>

        {sources.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("knowledge.detail.sourceCheck.empty")}</p>
        ) : (
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  {s.status === "missing" ? (
                    <p className="text-xs text-destructive">
                      {t("knowledge.detail.sourceCheck.missingNote")}
                    </p>
                  ) : null}
                  {s.status === "edited" ? (
                    <p className="text-xs text-muted-foreground">
                      {t("knowledge.detail.sourceCheck.editedNote")}
                    </p>
                  ) : null}
                </div>
                <div className="flex shrink-0 flex-col items-end gap-1.5">
                  <Badge variant={STATUS_VARIANT[s.status]} className={STATUS_CLASS[s.status]}>
                    {t(`knowledge.detail.sourceCheck.status.${s.status}`)}
                  </Badge>
                  {s.status === "changed" ? (
                    <Button
                      size="sm"
                    ...
                    >
                      {t("knowledge.detail.sourceCheck.update")}
                    </Button>
                  ) : null}
                </div>
              </li>
