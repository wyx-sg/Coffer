// frontend/src/kinds/knowledge/KnowledgeMergeDialog.tsx
// AI-assisted same-project scope merge. Opening the dialog runs the merge scan
// (POST /knowledge/merge_scan); each proposal shows the pair, the confidence +
// judged-by badge, and the engine's reason, with per-proposal swap-direction and
// Merge actions. A merge invalidates ["knowledge-scopes"] so the list drops the
// retired scope.
...
import { mergeScopes, mergeScanScopes, type MergeProposalOut, type ScopeEvidenceOut } from "./api";
...
function evidenceName(e: ScopeEvidenceOut): string {
...
export function KnowledgeMergeDialog({ open, onOpenChange }: Props) {
  ...
  // Re-scan each time the dialog opens (scopes may have changed since).
  useEffect(() => {
    if (open) {
      setProposals([]);
      scan.mutate();
  ...
  const merge = useMutation({
    mutationFn: ({ source, target }: { source: string; target: string }) =>
      mergeScopes(source, target),
    onSuccess: (_data, vars) => {
      void qc.invalidateQueries({ queryKey: ["knowledge-scopes"] });
      // Drop every proposal touching either merged scope — their evidence is stale.
      setProposals((prev) =>
        prev.filter(
          (p) => ![p.source, p.target].some((s) => s === vars.source || s === vars.target),
        ),
      );
      setMergingKey(null);
    },
          ...
          <div className="space-y-3">
            {noModel ? (
              <p className="rounded-md border border-border/60 bg-secondary px-3 py-2 text-xs text-muted-foreground">
                {t("knowledge.merge.noModelHint")}
              </p>
            ) : null}

            {proposals.length === 0 ? (
              <p className="py-4 text-center text-sm text-muted-foreground">
                {t("knowledge.merge.noCandidates")}
              </p>
            ) : (
                    ...
                    <li key={pairKey} className="rounded-md border border-border/60 p-3">
                      <div className="flex min-w-0 items-center gap-2 text-sm">
                        ...
                        <span className="ml-auto inline-flex shrink-0 rounded-full border border-border/60 bg-secondary px-2 py-0.5 text-xs">
                          {t(`knowledge.merge.confidence.${p.confidence}`, p.confidence)}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {p.judged_by === "remote"
                          ? t("knowledge.merge.judgedByRemote")
                          : t("knowledge.merge.judgedByEngine")}
                        {" — "}
                        {p.reason}
                      </p>
                      <p className="mt-1 truncate font-mono text-xs text-muted-foreground">
                        {src.store} ({t("knowledge.merge.entryCount", { count: src.facts })}) →{" "}
                        {dst.store} ({t("knowledge.merge.entryCount", { count: dst.facts })})
                      </p>
                      <div className="mt-2 flex justify-end gap-2">
                        ...
                        >
                          ...
                          {pending ? t("knowledge.merge.merging") : t("knowledge.merge.merge")}
                        </Button>
                      </div>
                    </li>
                  );
            ...
            {scan.data?.truncated ? (
              <p className="text-xs text-muted-foreground">{t("knowledge.merge.truncated")}</p>
            ) : null}
          </div>
        )}
      </DialogContent>
