...
import { projectDirName, scopeDisplayName } from "./types";
...
describe("scopeDisplayName", () => {
  ...
  test("a user-set label wins over the derived basename (FR-017c)", () => {
    expect(
      scopeDisplayName({ ...project, label: "My notes", project_root: "/Users/me/code/coffer" }),
    ).toBe("My notes");
  });

  test("falls back to the project_root basename when there is no label (FR-017a)", () => {
    expect(
      scopeDisplayName({ ...project, label: null, project_root: "/Users/me/code/coffer" }),
    ).toBe("coffer");
    // An empty / whitespace-free label is treated as no label.
    expect(scopeDisplayName({ ...project, label: "", project_root: "/Users/me/code/coffer" })).toBe(
      "coffer",
    );
  });
  ...
  test("the global store reads as its own name", () => {
    expect(
      scopeDisplayName({ scope: "global", name: "global", label: null, project_root: null }),
    ).toBe("global");
  });

  test("an orphan project store (no label, no known root) returns null for a graceful placeholder", () => {
    expect(scopeDisplayName({ ...project, label: null, project_root: null })).toBeNull();
    expect(scopeDisplayName({ ...project, label: null })).toBeNull();
  });
});
