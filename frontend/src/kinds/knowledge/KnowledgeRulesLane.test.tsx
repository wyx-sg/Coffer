// frontend/src/kinds/knowledge/KnowledgeRulesLane.test.tsx
//
...
vi.mock("./api", () => ({ getKnowledgeRules: vi.fn(), deleteKnowledgeRules: vi.fn() }));
...
function renderLane() {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <KnowledgeRulesLane scope="global" />
    </QueryClientProvider>,
  );
}
...
describe("KnowledgeRulesLane", () => {
  test("renders the rules Markdown when a row is selected", async () => {
    vi.mocked(api.getKnowledgeRules).mockResolvedValue({ text: "always run lint" });
    renderLane();
  ...
  test("delete asks for confirmation then calls the rules DELETE helper", async () => {
    vi.mocked(api.getKnowledgeRules).mockResolvedValue({ text: "always run lint" });
    vi.mocked(api.deleteKnowledgeRules).mockResolvedValue();
    renderLane();
    fireEvent.click(await screen.findByRole("button", { name: /^Rules$/ }));
    ...
    await waitFor(() => expect(api.deleteKnowledgeRules).toHaveBeenCalledWith("global"));
  });
});
