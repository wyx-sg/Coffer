// frontend/src/kinds/knowledge/KnowledgeHandoffLane.test.tsx
//
...
vi.mock("./api", () => ({ getKnowledgeHandoff: vi.fn(), deleteHandoffBranch: vi.fn() }));
...
function renderLane() {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <KnowledgeHandoffLane scope="global" />
    </QueryClientProvider>,
  );
}
...
describe("KnowledgeHandoffLane", () => {
  ...
  test("preview shows the three-button toolbar (open / reveal / delete)", async () => {
    vi.mocked(api.getKnowledgeHandoff).mockResolvedValue({ scenes: [SCENE] });
    renderLane();
    fireEvent.click(await screen.findByText("feat/login"));
