// frontend/src/kinds/knowledge/KnowledgeMergeDialog.test.tsx
//
...
vi.mock("./api", () => ({ mergeScanScopes: vi.fn(), mergeScopes: vi.fn() }));
...
function renderDialog() {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <KnowledgeMergeDialog open onOpenChange={() => {}} />
    </QueryClientProvider>,
  );
}
...
describe("KnowledgeMergeDialog", () => {
  test("scans on open and renders the proposal with reason and confidence", async () => {
    vi.mocked(api.mergeScanScopes).mockResolvedValue({
      engine: "ok",
      truncated: false,
      proposals: [PROPOSAL],
    });
    ...
    expect(api.mergeScanScopes).toHaveBeenCalledTimes(1);
  });
  ...
  test("merge sends the shown direction and drops the proposal on success", async () => {
    ...
    vi.mocked(api.mergeScopes).mockResolvedValue({
      target: _A,
      merged_files: 2,
      label_moved: true,
      root_moved: false,
    ...
    await waitFor(() => expect(api.mergeScopes).toHaveBeenCalledWith(_B, _A));
  ...
  test("swap flips the merge direction", async () => {
    ...
    await waitFor(() => expect(api.mergeScopes).toHaveBeenCalledWith(_A, _B));
  });
  ...
  test("a swap set on one proposal survives another proposal's merge", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: /^merge$/i }));
    await waitFor(() => expect(api.mergeScopes).toHaveBeenLastCalledWith(_C, _D));
  });
