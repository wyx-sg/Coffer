// frontend/src/kinds/knowledge/KnowledgeListLane.tsx
//
// Shared read-only "list → preview" lane for the knowledge detail page: a
// left list of files (the same shape as the entry / document trees) → select → the
...
// always flows through the frontmatter-aware <KnowledgePreviewBody>.
...
export function KnowledgeListLane({
  intro,
  listLabel,
  items,
  isLoading,
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(220px,300px)_1fr]">
        ...
        {selected ? (
          <div className="rounded-md border border-border">
            ...
            <KnowledgePreviewBody text={selected.text} className="max-h-[60vh] overflow-auto p-4" />
          </div>
        ) : (
      ...
      {onDelete && selected ? (
        <ConfirmDialog
          ...
          title={t("knowledge.detail.deleteTitle")}
          description={t("knowledge.detail.deleteConfirm", { name: selected.title })}
