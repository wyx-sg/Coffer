// frontend/src/kinds/knowledge/KnowledgeDocViewer.tsx
//
// Right-hand preview for the selected ingested document. Renders the Markdown source
  ...
  if (!doc && !isLoading) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center rounded-md border border-dashed border-border">
        <p className="text-sm text-muted-foreground">{t("knowledge.detail.selectDocument")}</p>
      </div>
    );
  }
      ...
      <div className="space-y-2 border-b border-border px-4 py-2.5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <Badge variant="outline">
            {t(`knowledge.sourceMode.${doc.source_mode}`, { defaultValue: doc.source_mode })}
          </Badge>
        </div>
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          >
            <RefreshCw className="mr-1.5 size-3.5" /> {t("knowledge.detail.reconvert")}
          </Button>
          <Button
            size="sm"
            variant="outline"
