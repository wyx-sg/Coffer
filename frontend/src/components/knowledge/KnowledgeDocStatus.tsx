// frontend/src/components/knowledge/KnowledgeDocStatus.tsx
// Maps an ingested document's embed_status (done | embedding | queued | running |
...
export function DocEmbedStatus({ status }: { status?: string | null }) {
  ...
  return (
    <StatusBadge tone={m.tone} label={t(`knowledge.detail.embedStatus.${m.key}`)} pulse={m.pulse} />
  );
}
