...
// First-run welcome card shown on /memory when no store exists yet — mirrors
// KnowledgeBaseWelcomePanel. Explains the surface and offers the single next
// step (New memory store, which opens the add dialog).
import { useTranslation } from "react-i18next";

import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

export function MemoryWelcomePanel({ onAdd }: { onAdd: () => void }) {
