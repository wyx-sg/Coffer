...
// description, and a search-mode column shows keyword vs semantic.
...
function modeOf(row: ResourceOut): "keyword" | "semantic" {
  const mode = (row.config as { search_mode?: string } | undefined)?.search_mode;
    ...
    },
    {
      ...
      cell: (r) => (
        <span className="inline-flex rounded-full border border-border/60 bg-secondary px-2 py-0.5 text-xs">
          {t(
            modeOf(r) === "semantic"
              ? "knowledgeBases.dialog.modeSemantic"
              : "knowledgeBases.dialog.modeKeyword",
          )}
        </span>
      ),
    },
