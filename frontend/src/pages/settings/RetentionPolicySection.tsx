 ...
 * The display name and description are translated by `table_name`, falling
 * back to the API-supplied text.
 */
export function RetentionPolicySection({ policy, onUpdate, updating }: Props) {
  ...
  // Toggling keep-forever persists right away: ON clears the retention window
  // (null), OFF restores the current day count.
  const toggleForever = (next: boolean) => {
    setKeepForever(next);
    onUpdate(next ? null : days);
  };
  ...
  const commitDays = () => {
    if (!keepForever && days !== policy.retention_days) onUpdate(days);
  };
        ...
        {!keepForever ? (
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              disabled={updating}
              onChange={(e) =>
                setDays(Math.min(3650, Math.max(1, parseInt(e.target.value || "0", 10) || 1)))
              }
              onBlur={commitDays}
              onKeyDown={(e) => {
