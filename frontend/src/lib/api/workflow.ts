...
// (spec workflow "Expose the engine over REST", "Show runs, their tasks and their context in
// the web UI").
//
...
//
// A workflow TEMPLATE is a resource of kind `workflow` and is read
...
/** One file under a run's directory, as the preview reads it. */
...
/** The resource kind a workflow template is registered under. */
...
export const workflowApi = {
  ...
  /**
   * Choose who runs a task's next attempt, on what, at what effort.
  ...
  /**
   * Rewrite what a run is called and what it is for. No `version`:
  ...
  // rather than by pressing Retry. The route stays; this client does
  // not call it, so it does not carry a method for it.
  /**
   * Say something to one task, whatever state it is in. The daemon
  ...
  // Inputs — what the run READS. Editable for the whole of a run's
  // life, not only at creation, so all four verbs live here.
   ...
   * directory, so this carries a body rather than a reference — a
  ...
  /**
   * Write a note of the developer's own into the run's inputs. It
  ...
  /** Idempotent: a repeated decision returns the same terminal state. */
