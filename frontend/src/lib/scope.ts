...
/** True when the two scopes name the same set of agents. */
 ...
 * agent registered on this machine is among them.
 *
 ...
 */
export function isDormantHere(scope: Scope | null, agents: string[]): boolean {
  if (scope === null || scope.agents === null) return false;
  if (scope.agents.length === 0) return true;
  return agents.length > 0 && !scope.agents.some((name) => agents.includes(name));
}
