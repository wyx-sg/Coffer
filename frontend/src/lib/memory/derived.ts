...
// came from (spec memory "Keep raw entries verbatim and hidden"). Reading a
// `.raw/` file as if it were Coffer's answer is exactly the mistake the
// surface has to prevent, so the tree marks the folder and the viewer says so
// over the file.
const RAW_DIR = ".raw";
